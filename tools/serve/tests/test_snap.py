"""Unit tests for server-side quote snapping (PoC P2, G2 v2, snap.py).

Pure-Python (no numpy / no network). Locks the core invariant: the returned quote is
always a verbatim (byte) substring of the body, and normalization is used only to LOCATE
the anchor (never returned). Covers whitespace-shifted anchors (T2), cut range (T5), and
the invariant across all cases (T6).
"""
...
def test_exact_anchor_locates_and_returns_original_bytes():
    q = SNAP.snap_quote("急迫不正の侵害に対して", BODY)
...
def test_whitespace_shifted_anchor_still_locates():
    ...
    shifted = "急迫不正の 侵害\nに対　して"
    q = SNAP.snap_quote(shifted, BODY)
    assert q is not None
...
def test_cut_extends_to_sentence_end():
    ...
    q = SNAP.snap_quote("自己又は他人の権利", body)
    assert q is not None
    assert q.endswith("。")
    assert q in body
...
def test_max_len_truncation():
    body = "あ" * 500
    q = SNAP.snap_quote("あああ", body, max_len=50)
    assert q is not None
    assert len(q) <= 50
    assert q in body
...
def test_paraphrase_anchor_not_locatable():
    assert SNAP.snap_quote("原文に存在しない言い換え表現", BODY) is None


def test_empty_anchor_returns_none():
    assert SNAP.snap_quote("", BODY) is None
    assert SNAP.snap_quote("   \n　", BODY) is None


def test_invariant_returned_quote_is_always_substring():
    # T6: いくつもの anchor で、返る引用が常に原文の byte 部分文字列であること。
    anchors = [
        "第三十六条",
        "急迫不正",
        "自己又は他人の権利を防衛",
