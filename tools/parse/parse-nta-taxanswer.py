...
_TSUTATSU_EDITION_BY_CATEGORY = {
    ...
    "sozoku": ("sochi-sozoku-tsutatsu", "sochi-sozoku-tsutatsu"),  # 相続税特例編 (FU-541)
}
...
CORPUS_UNREGISTERED_PREFIXES = {
    ...
    # --- FU-530 所得税 の 根拠法令等 に現れる corpus 未取込法令 (所得税バーティカル外)。
    # probe (212 htm 全走) で列挙。所法(article)/所基通(tsutatsu) の直後に来ると prefix 継承で
    # 偽リンクするため明示登録し unlinked (corpus_unregistered) として記録する。既存4カテゴリ
    # (hojin/sozoku/shohi/gensen) には不在 (2026-07-01 preflight 実測) ゆえ byte 非影響。
    # 【重要】衝突ゆえ意図的に除外した prefix (既存 locked baseline を変えるため追加しない):
    #   耐令 (hojin 5410 等で既に houjin 施行令へ偽リンク済 = locked)、地法 (hojin 5761 で
    #   地法法/地法令 が houjin へ偽リンク済 = locked)、日本と各国との租税条約 (gensen 2875 で
    #   実施特例法継承の unlinked = locked)。shotoku ではこの3語は tsutatsu/UNREG 継承で既に
    #   unlinked ゆえ追加不要 (偽リンクしない)。
    "震災特例法",  # 震災特例法 (東日本大震災の被災者等に係る国税関係法律の臨時特例法)
    "旧震災特例法",  # 旧震災特例法 (改正前・現行条番号と不一致)
    "旧措法",  # 旧措法 (旧租税特別措置法)
    "旧措令",  # 旧措令 (旧措置法施行令)
    "旧措規",  # 旧措規 (旧措置法施行規則)
    "旧所法",  # 旧所法 (旧所得税法)
    "旧所令",  # 旧所令 (旧所得税法施行令)
    "令元改正前所法",  # 令元改正前所法 (令和元年改正前の所得税法・条番号不一致)
    "財形法",  # 財形法 (勤労者財産形成促進法)
    "電子帳簿保存法",  # 電子帳簿保存法
    "復興所得税令",  # 復興所得税令 (復興特別所得税に関する政令)
    "実特法",  # 実特法 (実施特例法の別略記)
    "実特令",  # 実特令 (実施特例法施行令の別略記)
    "実特規",  # 実特規 (実施特例法施行規則の別略記)
    "地方税法",  # 地方税法 (フルネーム表記。略記 地法 は hojin 衝突ゆえ非登録)
    "納税貯蓄組合法",  # 納税貯蓄組合法
    "耐省令",  # 耐省令 (減価償却資産の耐用年数等に関する省令。略記 耐令 は hojin 衝突ゆえ非登録)
    "通基通",  # 通基通 (国税通則法基本通達・corpus 未取込)
    "郵便法",  # 郵便法
    "民間事業者による信書の送達に関する法律",  # 信書便法
    "災免令",  # 災免令 (災害被害者租税減免猶予法施行令)
    "オン化省令",  # オン化省令 (国税オンライン化省令の別略記)
    "国際連合の特権及び免除に関する条約",  # 国連特権免除条約
    "専門機関の特権及び免除に関する条約",  # 専門機関特権免除条約
    "アジア開発銀行を設立する協定",  # ADB 設立協定
    # --- FU-532 印紙税・その他の国税 の 根拠法令等 に現れる corpus 未取込法令。
    # probe (30 htm 全走) で列挙。印法 は FU-528 で登録済ゆえ再追加せず。印紙税本体
    # (印令/印規/印基通) と 登録免許税 (登法/登規)・その他の国税 (自法/旅客法系) は本 corpus
    # に本文が無いため常に unlinked 記録とする。明示登録することで、直前の article prefix
    # (法法/所法 等・7131 で実在) を継承して偽リンクする事故を構造的に防ぐ。既存6カテゴリ
    # (hojin/sozoku/shohi/gensen/shotoku/joto) には不在 (2026-07-02 preflight 実測) ゆえ byte 非影響。
    "印令",  # 印令 (印紙税法施行令)
    "印規",  # 印規 (印紙税法施行規則)
    "印基通",  # 印基通 (印紙税法基本通達・corpus 未取込。別表第一第N号文書 参照も本 prefix で吸収)
    "登法",  # 登法 (登録免許税法)
    "登規",  # 登規 (登録免許税法施行規則)
    "自法",  # 自法 (自動車重量税法)
    "旅客法",  # 旅客法 (国際観光旅客税法)
    "旅客令",  # 旅客令 (国際観光旅客税法施行令)
    "旅客通達",  # 旅客通達 (国際観光旅客税法通達・corpus 未取込)
}
...
# FU-530: 月区切りに中黒 ・ (U+30FB) 形 (平12・6課所4-9 / 昭62・12直所3-12 / 平13・7課個2-15)
# を追加。Why: 所得税タックスアンサーは通達番号を「元号年・月+課室記号」で書き (課所/課個/直所)、
# この中黒形は旧 regex ([.．] のみ) が取りこぼす。所法/所令/所基通 の直後に来ると条番号/通達番号
# として偽リンクするため、中黒を区切りクラスに追加して nta_notice として unlinked にする。
# 既存4カテゴリ (hojin/sozoku/shohi/gensen) に中黒形は 0 件 (2026-07-01 preflight 実測) ゆえ byte 不変。
_NOTICE_RE = re.compile(
    r"^(?:昭|平|令)(?:元|\d+)(?:[.．・]\d+)?(?:直|課)"
)  # 平元直評 / 令5課評 / 平元.3直法 / 平12・6課所
...
def _normalize_edition_remainder(remainder: str, edition: str) -> str:
    ...
    畳み込み形 61の4-1-1 へ ((N)/（N） -> -N)、山林所得・譲渡所得編 (sochi-joto) と相続税特例編
    (sochi-sozoku) は条跨ぎ共通の中黒 措通31・32共-1 / 69の6・69の7共-1 を 31_32共-1 /
    69の6_69の7共-1 へ (・ -> _)。申告所得税編 (sochi-shotoku) は款括弧も中黒も持たず (P0-2 実測)
    正規化なし。gate を編 (law_abbrev) に絞ることで、非該当編には一切触れない (非 cross-cutting・
    FU-538/539/541 の款/中黒 gate を統合)。法基通等の単一 corpus 系はこの分岐に該当せず素通し
    (byte 不変)。
    """
    if edition == "sochi-hojin-tsutatsu":
        return re.sub(r"[（(]([0-9]+)[)）]", r"-\1", remainder)
    if edition in ("sochi-joto-tsutatsu", "sochi-sozoku-tsutatsu"):
