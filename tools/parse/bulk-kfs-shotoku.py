#!/usr/bin/env python3
"""bulk-kfs-shotoku.py -- KFS 所得税 (MP/02) 全リーフの bulk fetch + dry-run 集計.

消費税 bulk (bulk-kfs-shohi.py・PR#113) の逐語横展開。fetcher
(tools/fetch-kfs/kfs-fetcher.py) と parser (tools/parse/parse-kfs-saiketsu.py) を
driver 化し、所得税法関係 (MP/02) の全リーフを enumerate -> fetch (skip-if-exists) ->
parse して集計レポートを出す。差分は税目固有の定数のみ (LAW_CATEGORY / TAX_ITEM /
STORE_PATH / APPENDIX_DIR)。

Usage:
    # dry-run (既定): fetch + parse + 集計のみ。corpus は一切書かない。
    python tools/parse/bulk-kfs-shotoku.py

    # network を使わず既存 cache だけで再集計 (offline)
    python tools/parse/bulk-kfs-shotoku.py --offline

Why (briefing case-law-shotoku-bulk-execution-handoff-2026-07-08・dry-run 先行):
    storage=B (全裁決を committed store に永続化) の前に、fetch+parse のシミュレーションで
    総数・attach 率・dup・法令分布・要旨 byte 一致率を機械算出し、佐藤承認を挟む。本 script は
    集計のみ (書込なし)。store 永続化と article 付与は承認後に別途行う。所得は既存 store 無し
...
LAW_CATEGORY = "02"  # MP/02 = 所得税法関係
INDEX_LEAF = f"MP/{LAW_CATEGORY}/index.html"
CACHE_ROOT = REPO_ROOT / "cache" / "kfs"
APPENDIX_DIR = REPO_ROOT / "build" / "chunks" / "kfs-shotoku"  # gitignored
TAX_ITEM = "所得税"
...
def enumerate_leaf_codes(index_html: bytes) -> list[str]:
    """MP/02 index HTML から所得税リーフの 10 桁コードを昇順一意で返す。"""
...
def run(*, offline: bool, sleep: float) -> int:
    ...
    print(f"index {INDEX_LEAF}: 所得税リーフ {len(codes)} 件を enumerate")
...
def _report(
    ...
    print("\n" + "=" * 64)
    print("  KFS 所得税 (MP/02) bulk-ingest DRY-RUN 集計")
    print("=" * 64)
    print(
...
STORE_PATH = REPO_ROOT / "data" / "v0.2" / "case-law" / "shotoku" / "rulings.jsonl"
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="KFS 所得税 (MP/02) bulk dry-run / write.")
