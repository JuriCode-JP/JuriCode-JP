...
| EDGE-012 | malformed HTML の入れ子吸い込み + 別表 <table> | NTA の一部ページ (例 09/09_03.htm 生命保険料) は別表 <table> 周辺で body 段落 <p> が閉じられず、後続通達 <p>/<h2>/<table> がその子として吸い込まれる。`html.parser` は <p> を自動クローズしないため `tag.find("strong")` が入れ子の番号を拾い、本文段落を誤って項開始扱い→見出し脱落・本文混線 (9-3-6 が title 空+9-3-5の2 の表本文を吸収)。さらに <table> は h1/h2/p に含まれず本文化されない | (1) <table> を巡回対象に追加しプレーンテキスト本文化 (別表保持・Bug55)。(2) CASE-A は番号が段落先頭にあるときのみ項開始 (入れ子 strong を却下)。(3) 段落本文は入れ子ブロック (`_text_excluding_nested_blocks`) を除外し二重計上を回避。整形済みページ (table/入れ子なし=shohi/9-2/ch1-2) は byte 不変。**この修正は消費税 8-1-5の2 の同型 latent バグ (8-1-6/8-1-7 を本文に重複) も是正** → shohi-corpus 修正 PR で 1 行 re-lock | `test_tsutatsu_multichapter.py::test_table_captured_as_plaintext_body` / `::test_case_a_guard_rejects_nested_strong_number` / `::test_text_excluding_nested_blocks_fastpath_and_strip` |
...
- **本文途中 (末尾でない) の改正注記** (既知の限界・将来対応): 一部の通達は注記が定義文と
  列挙項目の間に入る (例 9-2-9「…をいう。（平19年課法2-3「二十二」により追加…）(1) 役員等…」)。
  末尾アンカー (`\s*$`) のため現状は **本文テキストに忠実に残る = データ欠落はない**。構造化
  (amendment_note への分離) は次の理由で先送り: (1) 中間抽出は **9-2 sentinel の 11/35 を変更**
  し locked baseline の差分≠0 となる — バグでない理由で locked expected を改変しない規律。
  (2) 末尾束縛は安全 (裸 substring でない=Bug50 却下根拠) で、中間抽出はインライン引用との
  誤抽出リスクを伴う。**改正履歴レイヤー** (strategy.md「改正履歴レイヤー」) 着手時に schema 設計
  と一体で実装し、その時に sentinel を正式 re-lock する。それまでは本文保持で retrieval 価値は確保。
