#!/usr/bin/env python3
"""bulk-kfs-hojin.py -- KFS 法人税 (MP/03) 全リーフの bulk fetch + dry-run 集計.

パイロット (PR#107) の fetcher (tools/fetch-kfs/kfs-fetcher.py) と parser
(tools/parse/parse-kfs-saiketsu.py) を driver 化し、法人税法関係の全リーフを
enumerate -> fetch (skip-if-exists) -> parse して集計レポートを出す。

Usage:
    # dry-run (既定): fetch + parse + 集計のみ。corpus は一切書かない。
    python tools/parse/bulk-kfs-hojin.py

    # network を使わず既存 cache だけで再集計 (offline)
    python tools/parse/bulk-kfs-hojin.py --offline

Why (briefing case-law-hojin-bulk-execution-briefing-2026-07-04・dry-run 先行):
    storage=B (全裁決を committed store に永続化) の前に、fetch+parse のシミュレーションで
    総数・attach 率・dup・法令分布・要旨 byte 一致率を機械算出し、佐藤承認を挟む。本 script は
    集計のみ (書込なし)。store 永続化と article 付与は承認後に別途行う。
...
LAW_CATEGORY = "03"  # MP/03 = 法人税法関係
INDEX_LEAF = f"MP/{LAW_CATEGORY}/index.html"
CACHE_ROOT = REPO_ROOT / "cache" / "kfs"
APPENDIX_DIR = REPO_ROOT / "build" / "chunks" / "kfs-hojin"  # gitignored
TAX_ITEM = "法人税"
...
def enumerate_leaf_codes(index_html: bytes) -> list[str]:
    """MP/03 index HTML から法人税リーフの 10 桁コードを昇順一意で返す。"""
...
def _committed_pilot_summaries() -> dict[str, str]:
    """パイロットで既にコミット済の art-54 裁決 summary_ja を md から読み取り case_id->summary で返す.

    Why (補強①・2026-07-07 佐藤): パーサ修正 (list/table 取込拡張) が既にコミット済・正しい
    summary_ja を 1 件も変えないことを実証するための基準値。md frontmatter の cases: を読み、
    再パース結果と byte 一致を再 dry-run のゲートにする (「テスト不変」の主張を実測で裏取り)。
    """
    import yaml
    ...
    md = (
        ...
        / "houjin-zei-hou-shikkourei-article-54.md"
    )
    text = md.read_text(encoding="utf-8")
    fm = text.split("---", 2)[1]
    data = yaml.safe_load(fm)
    return {c["case_id"]: c.get("summary_ja", "") for c in (data.get("cases") or [])}


def _assert_pilot_idempotent(entries: list[dict]) -> tuple[int, list[str]]:
    """再パースした pilot 裁決の summary_ja が commit 済 md と byte 一致か検証する (補強①).

    Returns: (照合できた件数, 差分のあった case_id リスト)。差分ゼロ = べき等。
    """
    committed = _committed_pilot_summaries()
    checked = 0
    diffs: list[str] = []
    by_cid = {e["case_id"]: e for e in entries if e["case_id"]}
    for cid, want in committed.items():
        if cid not in by_cid:
            diffs.append(f"{cid} (再パースに現れず)")
            continue
        checked += 1
        got = by_cid[cid].get("summary_ja") or ""
...
def run(*, offline: bool, sleep: float) -> int:
    ...
    print(f"index {INDEX_LEAF}: 法人税リーフ {len(codes)} 件を enumerate")
    ...
    pilot_checked, pilot_diffs = _assert_pilot_idempotent(all_entries)
    _report(all_entries, per_leaf, byte_pass, byte_total, pilot_checked, pilot_diffs)
    ...
    green = byte_pass == byte_total and not incomplete and not null_cid and not pilot_diffs
...
def _report(
    ...
    print("\n" + "=" * 64)
    print("  KFS 法人税 (MP/03) bulk-ingest DRY-RUN 集計")
    print("=" * 64)
    print(
    ...
    print(
        f"\nべき等ゲート (補強①): pilot art-54 裁決 {pilot_checked} 件を commit 済 md と byte 照合"
        + ("  -> 差分ゼロ OK" if not pilot_diffs else f"  ★差分 {pilot_diffs}")
    )

    print("\n" + "=" * 64)
    clean = byte_pass == byte_total and not incomplete and not null_after and not pilot_diffs
    print("  DRY-RUN 完了: corpus には一切書いていません。")
    print(
        "  完全性ゲート: "
        + (
            "全て green (要旨欠落0・case_id欠落0・pilotべき等・byte100%)。"
            if clean
...
STORE_PATH = REPO_ROOT / "data" / "v0.2" / "case-law" / "hojin" / "rulings.jsonl"
DATA_V02 = REPO_ROOT / "data" / "v0.2"

# article cases: に出力するフィールド順 (pilot commit 済 md と揃える)。
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="KFS 法人税 (MP/03) bulk dry-run / write.")
