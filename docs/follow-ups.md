...
- 全角由来 **2 行**: `法人税法第２条` (全角) → houjin-art-2
...
### [x] FU-540: 申告所得税措通(801226)取込＋fallback 活性化 — ✅ 完了 2026-07-04 (PR #95, main d4df40fc)

**成果**: 措置法通達4冊目 **`sochi-shotoku-tsutatsu` corpus 新設（344 directive）**＝FU-539 逐語流用で 801226（申告所得税・41系）取込。**多編リンクを category 厳格→priority+fallback に活性化**（優先編不一致時に他 sochi-*-tsutatsu 編を fallback 照合・**一致編の law_abbrev を動的バインド＝current-context 前置による幽霊 ID 禁止**・複数編一致は fail-loud）。**shotoku +71 link**（sochi-shotoku 69＝41系/25の2/29の2・sochi-joto 2＝41の5 cross-domain・EXPECTED_DIRECTIVES 124→195・EXPECTED_UNLINKED 515→444）・**joto +1**（措通41-17＝申告provisionへ cross-domain・102→103/22→21・全 baseline byte-diff で捕捉し佐藤裁定で再ロック）・hojin/sozoku/shohi/gensen/inshi byte 不変・**誤リンク 0**（named-law 9件 unlinked 記録）・37系（株式）unlink 維持・CI 全9 green（pytest 783）。NTA stale TOC の6 dead link（17 directive）は soft-404 で `known_soft404` 機械除外＋文書化。

**残**: 37系（株式等譲渡・~30件）は 020624 tree ＝ **FU-542**。

**関連**: FU-539 (多編機構・持ち越し元) / FU-538 (shotoku baseline) / FU-542 (株式譲渡措通・37系回収)。

### [x] FU-541＋FU-543: 相続贈与税措通(080708)取込＋完全性補完 — ✅ 完了 2026-07-04 (PR #97 corpus, PR #98 corrective, main aef286ac)

**成果**: 措置法通達5冊目 **`sochi-sozoku-tsutatsu` corpus 新設（981 directive）**＝FU-539 逐語流用で 080708（相続税法の特例・69の4小規模宅地/70系）取込。sozoku タックスアンサーの措通参照を link 化（70-1-3 含む）・unlinked 48→45・他 baseline byte 不変・誤リンク0・CI 全9 green（pytest 803）。

**完全性補完（FU-543 corrective）**: FU-541（PR #97）は当初 963 で merge され、条-項-通達の3レベル現行18件（措法70条1項14/3項4）が `num_levels=2` で未収録だった。**FU-543（PR #98）で gated 可変 num_style `hier_var` を追加**（`_directive_levels_re`/`_build_directive_id_tail_re` に可変tail `{first}(?:-{level}){1,2}`＝kan_paren 流用・`_normalize` は全角 verbatim 保存）し、**現行18件を捕捉（963→981・70-1-3 link 可）**。既存 num_style 3分岐は非改変・**他6編 corpus byte 完全不変（gated）**。旧法7件は "旧"始まりで除外維持。

**関連**: FU-539 (多編機構) / FU-540 (fallback) / FU-536 (kan_paren 可変tail precedent)。

### [ ] FU-542: 株式等譲渡措通(020624)取込＋shotoku 37系リンク化 (2026-07-04 追加・FU-540 残)

**経緯**: FU-540 で shotoku 措通参照のうち 37の10/37の11 系（株式等譲渡・~30件）が別 tree `020624`（租税特別措置法 株式等に係る譲渡所得等関係）に在ると判明し unlink 維持。020624 corpus を取込み shotoku の残 unlink を回収。

**やること**: sochi-kabushiki-tsutatsu（仮）corpus 取込（FU-540 逐語流用）。**020624 は 共/中黒マーカー（`37の10・37の11共`）を含む**＝FU-539 で実装済の中点正規化を流用。多編 fallback は活性化済ゆえ純加算。shotoku baseline 再ロック。**要独立計画書＋佐藤 GO**。

**関連**: FU-540 (残の源) / FU-539 (共/中黒 num_style)。

### [ ] FU-544: resolver のカンマ継続裸番号の継承解決 (2026-07-04 追加・FU-541 由来) — ⏸ 延期 2026-07-05 (佐藤裁定・probe driven)

**経緯**: taxanswer の措通参照「69の4-27、28」の 2 つ目「28」は前の条番号（69の4）を継承した裸番号だが、resolver に継続ロジックがなく **69の4-28 が unlink 維持**。複数編・複数カテゴリに影響しうる横断課題。

**やること**: `_process_tsutatsu_remainder`（またはカンマ split 段）で、同一措通トークン内のカンマ区切り後続番号が条番号を持たない場合、直前の条を継承して解決する。全 taxanswer baseline の byte-diff で影響確認・再ロック。**要独立計画書＋佐藤 GO**。

**⏸ 延期理由 (2026-07-05 佐藤裁定)**: v2 計画の P0 probe（tracked fixture 全カテゴリ実測）で、計画前提「~102 件回収可」が **実回収 3 リンクのみ**と判明。内訳: (a) 純末尾フラグメント 6 件のうち回収可 3 件 = `措通37の11-12、13`→`37の11-13`(sochi-kabushiki) / `措通69の4-27、28`→`69の4-28`(sochi-sozoku) の clean 2 件＋ shouhi-kihon `…1-4-1~6、9`→`1-4-9`（レンジ由来の限界 1 件）。残 3 件（shohi-6113 `5,14,15`）は linked stem 無しで設計上正しく非回収。(b) bare `tsutatsu_not_in_corpus` 85 件のうち他 ~79 件は 法基通/所基通/消基通 の**未取込編 corpus gap**＝対象外。**全 tsutatsu 継続リンクを司る resolver への横断改修（FU-547 型 偽リンクリスク）を 3 リンクのために入れる価値なし**と裁定 → 延期。将来 未取込編（法基通/所基通等）を tsutatsu corpus 化すれば (b) が回収対象化し、その時点で再評価。

**関連**: FU-538 (措通リンク化) / FU-547 (同編実在ガード・偽リンク防止先例) / FU-545 (同 v2 計画で同時起票)。

### [x] FU-545: stale test test_sochi_promotion_guards.py の更新 — ✅ 完了 2026-07-05 (PR #104, main 582b0008)

**経緯**: `test_sochi_promotion_guards.py::test_sochu_still_unlinked_after_promotion` は FU-538 以前の reason `corpus_unregistered` を assert していたが、FU-538 で措通が `LAW_PREFIX_MAP` へ昇格したためその措法専用 reason には論理的にならず陳腐化。**本ファイルが CI pytest allowlist（`ci.yml` の明示ファイル列挙）外**だったため、stale assert が FU-538→FU-551 の間ローカルでのみ落ち CI を割らず放置されていた（FU-541/543 実装中に fail-loud で顕在化）。

**成果**: (1) 期待値を FU-538 後の実 reason **`tsutatsu_not_in_corpus`** へ修正（`66の4-1` は実在 directive でなく corpus は 3 階層 `66の4-1-N` のみ・main 実走 probe で確認）。(2) `_TSUTATSU_CORPUS` を実構造の最小スタブ（`66の4-1-N` のみ・2 階層なし）へ pytest `monkeypatch` し **hermetic 化**（build/chunks 非依存・CI/ローカル同一挙動、`test_taxanswer_related.py` と同方式）。(3) 本ファイルを **`ci.yml` pytest allowlist に追加**しガードを CI 実走化（root cause の封鎖）。test-only＋ci.yml のみ（parser/data/schema 不変）・CI 全9 green（pytest 870・実 CI 3.11/3.12 pass）・誤リンク 0。

**設計教訓**: CI pytest が明示ファイル allowlist 方式ゆえ、新規 test file は `ci.yml` に手動追記しないと「ローカルで走るが CI で走らない」silent gap になる（stale が長期放置された根因）。hermetic 化＝corpus スタブ注入で env 非依存にしてから allowlist 復帰。

**関連**: FU-538 (措通昇格・陳腐化の源) / FU-544 (同 v2 計画・延期) / FU-547 (CI allowlist wiring 先例)。
...
**設計教訓**: 款畳み込み正規化は corpus 構築側（`parse-nta-tsutatsu.py`）と参照解決側（`parse-nta-taxanswer.py`）の**両方に別実装で必要**（v1 計画はこれを解決側にも在ると誤認し +7 予測→config-only 実測 +3・完全版 +20）。他税目編（所得税/譲渡/相続編）の措通 corpus は未設置ゆえ当該編 措通参照（joto/shotoku/sozoku）は unlink 維持＝各編措通取込は将来 FU。

**関連**: FU-536 (措通 corpus) / FU-537 (措法系昇格) / FU-529 (所法昇格 precedent)。
...
**成果**:
- **措法/措令/措規 昇格＋parser ガード3種**: 生昇格は P0 実測で **dangling 31** (D 他法令偽リンク14 / A レンジ9 / B 等2 / C genuine gap6) を生むため、措法系 prefix 限定 (非 cross-cutting) のガードを追加＝(D 継承ガード[告示ガード同型・非数字開始の別法令名を unlink] / A・B レンジ`~`・`等` を unlink / C 措法系 corpus 実在チェック[corpus_gap])。
