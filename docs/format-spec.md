...
5. **原典忠実性** — 法令本文は e-Gov 公式テキストの完全コピー [G0-a] とし、改変・要約・修辞を一切加えない。
   **この主張は宣言ではなく機械検証で担保する**: 全条文について「空白の畳み込みを除き、
   本文 == e-Gov XML の条文本文」を [G0 忠実性ゲート](../tools/parse/v0.2/g0_fidelity_gate.py)
   が検査する(G0-a)。
   > **訂正 (2026-07-14)**: 本項は長く「宣言」に留まっており、実際には**改変が混入していた**。
   > e-Gov XML のルビ `<Ruby>瑕疵<Rt>かし</Rt></Ruby>` の**読み仮名が本文に取り込まれ**、
   > 民法101条が「代理行為の瑕疵**かし**」、道路交通法が「牽**けん**引」等となっていた
   > (29 法令・391 箇所)。混入経路は 4 つ(条文本文 / 見出し / 節名 / 表セル)あり、
   > 検証器が生成器と同じ抽出器を再利用していたため、突合しても差が出なかった。
   > 現在は除去し、G0-d(ルビ検査)を常設ゲートとして追加している。
...
| **非網羅性(データ契約)** | `amendments[]` は**本文に実質的変化をもたらした改正のみ**を収録する = 改正の完全な一覧ではない。附則のみ・omnibus 付随改正は含まない。**章単位の大改正で現行版に残らなくなった条の改廃も出ない**(帰属は現行 corpus に実在する条 `@Num` に限る)。例: 法人税法の連結納税制度 → グループ通算制度移行(2022-04-01・令和二年法律第八号)で消滅した旧第81条系枝番(81-2〜81-31)は現行 corpus 非在ゆえ非付与。削除後も番号がプレースホルダとして残る条(例 法人税法 15-2)は現行条ゆえ「本条を削除」で付与される。網羅的沿革は上記 e-Gov 一次情報を参照 |
...
### 5.2 項・号・細別の物理表現(★ロック・2026-07-13)

`## 原文 (日本語)` の中身は、**e-Gov XML の文書順どおりに、原文の文字だけで**書く。
構造の表現に使ってよいのは **見出し(項のみ)・空行・全角スペース**だけであり、
**原文に無い文字(`-`・`1.`・箇条書き記号・注釈マーカー)を本文に混ぜてはならない**。

| 単位 | XML 要素 | Markdown 表現 |
|---|---|---|
| 項 | `Paragraph` | `### 第N条第K項` 見出し(単項の条は `### 第N条`)。項番号ラベル(`ParagraphNum`)は見出しに写像するので本文には書かない |
| 項の本文 | `ParagraphSentence/Sentence` | 見出し直下に 1 段落。本文とただし書は原文どおり連結する(改行しない) |
| 号 | `Item` | 空行を挟み、`ItemTitle` ＋ **全角スペース(U+3000)** ＋ `ItemSentence` を 1 行 |
| 細別 | `Subitem1`, `Subitem2`, … | 行頭に **深さ × 全角スペース 1 個** の段付け。以降は号と同じ(`Title` ＋ 全角スペース ＋ `Sentence`) |
| 表 | `TableStruct` | GFM パイプ表(§3.5)。**XML の文書順の位置**に置く(号の中の表は、その号の直後) |
| 罰則付記等 | `SupplNote` | 条の末尾に原文のまま 1 行 |

```markdown
### 第百三十二条第一項

税務署長は、次に掲げる法人に係る法人税につき更正又は決定をする場合において、…計算することができる。
...
- **空白畳み込み後、`## 原文 (日本語)` の本文は e-Gov XML の条文本文と完全一致する**(G0-a)。
  構造の表現に空白しか使っていないため、これは自明に成立する。
- 各 chunk の本文は親条文の本文の部分列になる(G0-c)。
- 下流の正規表現(項見出し `### …` / GFM 区切り行 / 表行 `|`)と干渉しない。

**禁止事項**:

- 行頭の段付けに**半角スペースを使わない**(半角 4 個以上は Markdown がコードブロックと誤認する)。全角スペースにはこの問題がない。
- 号番号を `1.` などに振り直さない(原文の「一」をそのまま使う。振り直しは二重番号になる)。
- 本文中に `<!-- … -->` 等の内部マーカーを置かない(2026-07-13 に全 corpus から除去済み。segment のメタデータは frontmatter の `paragraphs[].segments[]` が正本)。
