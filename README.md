...
> **訂正 (2026-07-14)**: 「法令本文を一切改変しない」という主張を、以前は**宣言だけで裏付けなしに**
> 掲げていました。実際には e-Gov XML のルビ (振り仮名) の**読みが本文に取り込まれ**、
> 民法 101 条が「代理行為の瑕疵**かし**」、道路交通法が「牽**けん**引」等になっていました
> (29 法令・391 箇所)。現在は除去し、**同じ主張を機械検証 (G0) で担保**しています。
> 検出できなかった理由は、検証器が生成器と同じテキスト抽出器を再利用しており、
> 同じ欠陥が両側に現れて差が出なかったためです。

> **訂正 2 (2026-07-14)**: PR #138 で「内部マーカー 31,909 行 → **0**」と報告しましたが、
> **これは誤りでした**。正しくは **31,909 → 1** です。1 件（警察法 33 条）と、H1 見出しの
> 残骸 17 条が、**`## 原文` セクションの外**に残っていました。
> 影響範囲: **`ja_text_sha256` の対象外・retrieval chunk にも混入なし**（実測 0 件）。
> 原因は、旧パーサがマーカーを H1 行の**内部**に挿入して H1 を 2 行に割っていたためで、
> 再生成時に H1 行だけを差し替えたので割れた残り半分が居残りました。
> **なぜ気づけなかったか**: G0-a〜d は「原文セクションの**中身**」しか見ていなかったためです。
> **検査範囲の外にあるものは、ゲートが緑でも永久に見えません。**
> 対策として **G0-e**（ファイル全体を見る検査）を新設し、[CI に常設](.github/workflows/ci.yml)しました。
