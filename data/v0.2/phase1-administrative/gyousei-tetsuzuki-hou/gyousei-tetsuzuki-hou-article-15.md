---
law_id: 405AC0000000088
law_name_ja: 行政手続法
law_name_en: Administrative Procedure Act
article_number: '15'
article_id: gyousei-tetsuzuki-hou-art-15
version_date: '1993-11-12'
source_url: https://laws.e-gov.go.jp/law/405AC0000000088
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: gyousei-tetsuzuki-hou-art-15-p1-hashira
    type: hashira
    text: 行政庁は、聴聞を行うに当たっては、聴聞を行うべき期日までに相当な期間をおいて、不利益処分の名あて人となるべき者に対し、次に掲げる事項を書面により通知しなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: gyousei-tetsuzuki-hou-art-15-p2-hashira
    type: hashira
    text: 前項の書面においては、次に掲げる事項を教示しなければならない。
    modality: gimu
    references:
    - 前項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: gyousei-tetsuzuki-hou-art-15-p3-zen
    type: zen_dan
    text: 行政庁は、不利益処分の名あて人となるべき者の所在が判明しない場合においては、第一項の規定による通知を、その者の氏名、同項第三号及び第四号に掲げる事項並びに当該行政庁が同項各号に掲げる事項を記載した書面をいつでもその者に交付する旨を当該行政庁の事務所の掲示場に掲示することによって行うことができる。
    modality: kanou_kenri
    references:
    - 同項
  - id: gyousei-tetsuzuki-hou-art-15-p3-kou
    type: kou_dan
    text: この場合においては、掲示を始めた日から二週間を経過したときに、当該通知がその者に到達したものとみなす。
    modality: unspecified
    depends_on: gyousei-tetsuzuki-hou-art-15-p3-zen
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  shou: 3
  shou_name_ja: 第三章　不利益処分
  setsu: 2
  setsu_name_ja: 第二節　聴聞
---

# 行政手続法 第15条(（聴聞の通知の方式）)

## 原文 (日本語)

### 第十五条第一項

<!-- segment: hashira id: gyousei-tetsuzuki-hou-art-15-p1-hashira -->
行政庁は、聴聞を行うに当たっては、聴聞を行うべき期日までに相当な期間をおいて、不利益処分の名あて人となるべき者に対し、次に掲げる事項を書面により通知しなければならない。

### 第十五条第二項

<!-- segment: hashira id: gyousei-tetsuzuki-hou-art-15-p2-hashira -->
前項の書面においては、次に掲げる事項を教示しなければならない。

### 第十五条第三項

<!-- segment: zen_dan id: gyousei-tetsuzuki-hou-art-15-p3-zen -->
行政庁は、不利益処分の名あて人となるべき者の所在が判明しない場合においては、第一項の規定による通知を、その者の氏名、同項第三号及び第四号に掲げる事項並びに当該行政庁が同項各号に掲げる事項を記載した書面をいつでもその者に交付する旨を当該行政庁の事務所の掲示場に掲示することによって行うことができる。<!-- segment: kou_dan id: gyousei-tetsuzuki-hou-art-15-p3-kou depends_on: gyousei-tetsuzuki-hou-art-15-p3-zen -->
この場合においては、掲示を始めた日から二週間を経過したときに、当該通知がその者に到達したものとみなす。
