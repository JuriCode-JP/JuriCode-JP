---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '114'
article_id: chihou-jichi-hou-art-114
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-114-p1
    type: simple
    text: 普通地方公共団体の議会の議員の定数の半数以上の者から請求があるときは、議長は、その日の会議を開かなければならない。この場合において議長がなお会議を開かないときは、第百六条第一項又は第二項の例による。
    modality: unspecified
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-114-p2
    type: simple
    text: 前項の規定により会議を開いたとき、又は議員中に異議があるときは、議長は、会議の議決によらない限り、その日の会議を閉じ又は中止することができない。
    modality: kanou_negative
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 6
  shou_name_ja: 第六章　議会
  setsu: 6
  setsu_name_ja: 第六節　会議
---

# 地方自治法 第114条

## 原文 (日本語)

### 第百十四条第一項

普通地方公共団体の議会の議員の定数の半数以上の者から請求があるときは、議長は、その日の会議を開かなければならない。この場合において議長がなお会議を開かないときは、第百六条第一項又は第二項の例による。

### 第百十四条第二項

前項の規定により会議を開いたとき、又は議員中に異議があるときは、議長は、会議の議決によらない限り、その日の会議を閉じ又は中止することができない。
