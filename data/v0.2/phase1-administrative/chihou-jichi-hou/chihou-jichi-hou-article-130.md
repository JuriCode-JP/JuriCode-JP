---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '130'
article_id: chihou-jichi-hou-art-130
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-130-p1
    type: simple
    text: 傍聴人が公然と可否を表明し、又は騒ぎ立てる等会議を妨害するときは、普通地方公共団体の議会の議長は、これを制止し、その命令に従わないときは、これを退場させ、必要がある場合においては、これを当該警察官に引き渡すことができる。
    modality: kanou_kenri
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-130-p2
    type: simple
    text: 傍聴席が騒がしいときは、議長は、すべての傍聴人を退場させることができる。
    modality: kanou_kenri
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-130-p3
    type: simple
    text: 前二項に定めるものを除くほか、議長は、会議の傍聴に関し必要な規則を設けなければならない。
    modality: gimu
    references:
    - 前二項
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 6
  shou_name_ja: 第六章　議会
  setsu: 9
  setsu_name_ja: 第九節　紀律
---

# 地方自治法 第130条

## 原文 (日本語)

### 第百三十条第一項

傍聴人が公然と可否を表明し、又は騒ぎ立てる等会議を妨害するときは、普通地方公共団体の議会の議長は、これを制止し、その命令に従わないときは、これを退場させ、必要がある場合においては、これを当該警察官に引き渡すことができる。

### 第百三十条第二項

傍聴席が騒がしいときは、議長は、すべての傍聴人を退場させることができる。

### 第百三十条第三項

前二項に定めるものを除くほか、議長は、会議の傍聴に関し必要な規則を設けなければならない。
