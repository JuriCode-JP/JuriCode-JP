---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '235'
article_id: chihou-jichi-hou-art-235
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-235-p1
    type: simple
    text: 都道府県は、政令の定めるところにより、金融機関を指定して、都道府県の公金の収納又は支払の事務を取り扱わせなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-235-p2
    type: simple
    text: 市町村は、政令の定めるところにより、金融機関を指定して、市町村の公金の収納又は支払の事務を取り扱わせることができる。
    modality: kanou_kenri
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 9
  shou_name_ja: 第九章　財務
  setsu: 7
  setsu_name_ja: 第七節　現金及び有価証券
---

# 地方自治法 第235条(（金融機関の指定）)

## 原文 (日本語)

### 第二百三十五条第一項

都道府県は、政令の定めるところにより、金融機関を指定して、都道府県の公金の収納又は支払の事務を取り扱わせなければならない。

### 第二百三十五条第二項

市町村は、政令の定めるところにより、金融機関を指定して、市町村の公金の収納又は支払の事務を取り扱わせることができる。
