---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '217'
article_id: chihou-jichi-hou-art-217
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: true
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-217-p1-honbun
    type: honbun
    text: 予算外の支出又は予算超過の支出に充てるため、歳入歳出予算に予備費を計上しなければならない。
    modality: gimu
  - id: chihou-jichi-hou-art-217-p1-tadashi
    type: tadashi
    text: ただし、特別会計にあつては、予備費を計上しないことができる。
    modality: kanou_kenri
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-217-p2
    type: simple
    text: 予備費は、議会の否決した費途に充てることができない。
    modality: kanou_negative
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 9
  shou_name_ja: 第九章　財務
  setsu: 2
  setsu_name_ja: 第二節　予算
---

# 地方自治法 第217条(（予備費）)

## 原文 (日本語)

### 第二百十七条第一項

<!-- segment: honbun id: chihou-jichi-hou-art-217-p1-honbun -->
予算外の支出又は予算超過の支出に充てるため、歳入歳出予算に予備費を計上しなければならない。<!-- segment: tadashi id: chihou-jichi-hou-art-217-p1-tadashi -->
ただし、特別会計にあつては、予備費を計上しないことができる。

### 第二百十七条第二項

<!-- segment: simple id: chihou-jichi-hou-art-217-p2 -->
予備費は、議会の否決した費途に充てることができない。
