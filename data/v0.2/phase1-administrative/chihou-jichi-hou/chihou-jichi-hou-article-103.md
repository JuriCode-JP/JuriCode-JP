---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '103'
article_id: chihou-jichi-hou-art-103
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-103-p1
    type: simple
    text: 普通地方公共団体の議会は、議員の中から議長及び副議長一人を選挙しなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-103-p2
    type: simple
    text: 議長及び副議長の任期は、議員の任期による。
    modality: unspecified
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 6
  shou_name_ja: 第六章　議会
  setsu: 4
  setsu_name_ja: 第四節　議長及び副議長
---

# 地方自治法 第103条

## 原文 (日本語)

### 第百三条第一項

<!-- segment: simple id: chihou-jichi-hou-art-103-p1 -->
普通地方公共団体の議会は、議員の中から議長及び副議長一人を選挙しなければならない。

### 第百三条第二項

<!-- segment: simple id: chihou-jichi-hou-art-103-p2 -->
議長及び副議長の任期は、議員の任期による。
