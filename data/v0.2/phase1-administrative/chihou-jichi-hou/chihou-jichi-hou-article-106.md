---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '106'
article_id: chihou-jichi-hou-art-106
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-106-p1
    type: simple
    text: 普通地方公共団体の議会の議長に事故があるとき、又は議長が欠けたときは、副議長が議長の職務を行う。
    modality: unspecified
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-106-p2
    type: simple
    text: 議長及び副議長にともに事故があるときは、仮議長を選挙し、議長の職務を行わせる。
    modality: unspecified
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-106-p3
    type: simple
    text: 議会は、仮議長の選任を議長に委任することができる。
    modality: kanou_kenri
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 6
  shou_name_ja: 第六章　議会
  setsu: 4
  setsu_name_ja: 第四節　議長及び副議長
---

# 地方自治法 第106条

## 原文 (日本語)

### 第百六条第一項

普通地方公共団体の議会の議長に事故があるとき、又は議長が欠けたときは、副議長が議長の職務を行う。

### 第百六条第二項

議長及び副議長にともに事故があるときは、仮議長を選挙し、議長の職務を行わせる。

### 第百六条第三項

議会は、仮議長の選任を議長に委任することができる。
