---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '161'
article_id: chihou-jichi-hou-art-161
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: true
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-161-p1-honbun
    type: honbun
    text: 都道府県に副知事を、市町村に副市町村長を置く。
    modality: unspecified
  - id: chihou-jichi-hou-art-161-p1-tadashi
    type: tadashi
    text: ただし、条例で置かないことができる。
    modality: kanou_kenri
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-161-p2
    type: simple
    text: 副知事及び副市町村長の定数は、条例で定める。
    modality: unspecified
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 7
  shou_name_ja: 第七章　執行機関
  setsu: 2
  setsu_name_ja: 第二節　普通地方公共団体の長
  kan: 3
  kan_name_ja: 第三款　補助機関
---

# 地方自治法 第161条

## 原文 (日本語)

### 第百六十一条第一項

都道府県に副知事を、市町村に副市町村長を置く。ただし、条例で置かないことができる。

### 第百六十一条第二項

副知事及び副市町村長の定数は、条例で定める。
