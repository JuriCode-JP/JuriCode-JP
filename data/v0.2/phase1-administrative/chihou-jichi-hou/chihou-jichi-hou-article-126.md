---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '126'
article_id: chihou-jichi-hou-art-126
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-126-p1
    type: simple
    text: 普通地方公共団体の議会の議員は、議会の許可を得て辞職することができる。但し、閉会中においては、議長の許可を得て辞職することができる。
    modality: kanou_kenri
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 6
  shou_name_ja: 第六章　議会
  setsu: 8
  setsu_name_ja: 第八節　議員の辞職及び資格の決定
---

# 地方自治法 第126条

## 原文 (日本語)

### 第百二十六条

<!-- segment: simple id: chihou-jichi-hou-art-126-p1 -->
普通地方公共団体の議会の議員は、議会の許可を得て辞職することができる。但し、閉会中においては、議長の許可を得て辞職することができる。
