---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '6'
article_id: chihou-jichi-hou-art-6
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-6-p1
    type: simple
    text: 都道府県の廃置分合又は境界変更をしようとするときは、法律でこれを定める。
    modality: unspecified
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-6-p2
    type: simple
    text: 都道府県の境界にわたつて市町村の設置又は境界の変更があつたときは、都道府県の境界も、また、自ら変更する。従来地方公共団体の区域に属しなかつた地域を市町村の区域に編入したときも、また、同様とする。
    modality: gimu
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-6-p3
    type: simple
    text: 前二項の場合において財産処分を必要とするときは、関係地方公共団体が協議してこれを定める。但し、法律に特別の定があるときは、この限りでない。
    modality: jogai
    references:
    - 前二項
- number: 4
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-6-p4
    type: simple
    text: 前項の協議については、関係地方公共団体の議会の議決を経なければならない。
    modality: gimu
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 1
  shou_name_ja: 第一章　通則
---

# 地方自治法 第6条

## 原文 (日本語)

### 第六条第一項

<!-- segment: simple id: chihou-jichi-hou-art-6-p1 -->
都道府県の廃置分合又は境界変更をしようとするときは、法律でこれを定める。

### 第六条第二項

<!-- segment: simple id: chihou-jichi-hou-art-6-p2 -->
都道府県の境界にわたつて市町村の設置又は境界の変更があつたときは、都道府県の境界も、また、自ら変更する。従来地方公共団体の区域に属しなかつた地域を市町村の区域に編入したときも、また、同様とする。

### 第六条第三項

<!-- segment: simple id: chihou-jichi-hou-art-6-p3 -->
前二項の場合において財産処分を必要とするときは、関係地方公共団体が協議してこれを定める。但し、法律に特別の定があるときは、この限りでない。

### 第六条第四項

<!-- segment: simple id: chihou-jichi-hou-art-6-p4 -->
前項の協議については、関係地方公共団体の議会の議決を経なければならない。
