---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '181'
article_id: chihou-jichi-hou-art-181
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-181-p1
    type: simple
    text: 普通地方公共団体に選挙管理委員会を置く。
    modality: unspecified
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-181-p2
    type: simple
    text: 選挙管理委員会は、四人の選挙管理委員を以てこれを組織する。
    modality: unspecified
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 7
  shou_name_ja: 第七章　執行機関
  setsu: 3
  setsu_name_ja: 第三節　委員会及び委員
  kan: 4
  kan_name_ja: 第四款　選挙管理委員会
---

# 地方自治法 第181条

## 原文 (日本語)

### 第百八十一条第一項

普通地方公共団体に選挙管理委員会を置く。

### 第百八十一条第二項

選挙管理委員会は、四人の選挙管理委員を以てこれを組織する。
