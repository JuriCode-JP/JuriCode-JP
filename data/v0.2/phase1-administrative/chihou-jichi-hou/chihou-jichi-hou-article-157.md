---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '157'
article_id: chihou-jichi-hou-art-157
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-157-p1
    type: simple
    text: 普通地方公共団体の長は、当該普通地方公共団体の区域内の公共的団体等の活動の綜合調整を図るため、これを指揮監督することができる。
    modality: kanou_kenri
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-157-p2
    type: simple
    text: 前項の場合において必要があるときは、普通地方公共団体の長は、当該普通地方公共団体の区域内の公共的団体等をして事務の報告をさせ、書類及び帳簿を提出させ及び実地について事務を視察することができる。
    modality: kanou_kenri
    references:
    - 前項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-157-p3
    type: simple
    text: 普通地方公共団体の長は、当該普通地方公共団体の区域内の公共的団体等の監督上必要な処分をし又は当該公共的団体等の監督官庁の措置を申請することができる。
    modality: kanou_kenri
- number: 4
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-157-p4
    type: simple
    text: 前項の監督官庁は、普通地方公共団体の長の処分を取り消すことができる。
    modality: koka_torikeshi
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 7
  shou_name_ja: 第七章　執行機関
  setsu: 2
  setsu_name_ja: 第二節　普通地方公共団体の長
  kan: 2
  kan_name_ja: 第二款　権限
---

# 地方自治法 第157条

## 原文 (日本語)

### 第百五十七条第一項

普通地方公共団体の長は、当該普通地方公共団体の区域内の公共的団体等の活動の綜合調整を図るため、これを指揮監督することができる。

### 第百五十七条第二項

前項の場合において必要があるときは、普通地方公共団体の長は、当該普通地方公共団体の区域内の公共的団体等をして事務の報告をさせ、書類及び帳簿を提出させ及び実地について事務を視察することができる。

### 第百五十七条第三項

普通地方公共団体の長は、当該普通地方公共団体の区域内の公共的団体等の監督上必要な処分をし又は当該公共的団体等の監督官庁の措置を申請することができる。

### 第百五十七条第四項

前項の監督官庁は、普通地方公共団体の長の処分を取り消すことができる。
