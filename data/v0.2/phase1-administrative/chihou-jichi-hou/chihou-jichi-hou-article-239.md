---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '239'
article_id: chihou-jichi-hou-art-239
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-239-p1-hashira
    type: hashira
    text: この法律において「物品」とは、普通地方公共団体の所有に属する動産で次の各号に掲げるもの以外のもの及び普通地方公共団体が使用のために保管する動産（政令で定める動産を除く。）をいう。
    modality: teigi
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-239-p2
    type: simple
    text: 物品に関する事務に従事する職員は、その取扱いに係る物品（政令で定める物品を除く。）を普通地方公共団体から譲り受けることができない。
    modality: kanou_negative
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-239-p3
    type: simple
    text: 前項の規定に違反する行為は、これを無効とする。
    modality: koka_mukou
    references:
    - 前項
- number: 4
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-239-p4
    type: simple
    text: 前二項に定めるもののほか、物品の管理及び処分に関し必要な事項は、政令でこれを定める。
    modality: unspecified
    references:
    - 前二項
- number: 5
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-239-p5
    type: simple
    text: 普通地方公共団体の所有に属しない動産で普通地方公共団体が保管するもの（使用のために保管するものを除く。）のうち政令で定めるもの（以下「占有動産」という。）の管理に関し必要な事項は、政令でこれを定める。
    modality: unspecified
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 9
  shou_name_ja: 第九章　財務
  setsu: 9
  setsu_name_ja: 第九節　財産
  kan: 2
  kan_name_ja: 第二款　物品
---

# 地方自治法 第239条(（物品）)

## 原文 (日本語)

### 第二百三十九条第一項

<!-- segment: hashira id: chihou-jichi-hou-art-239-p1-hashira -->
この法律において「物品」とは、普通地方公共団体の所有に属する動産で次の各号に掲げるもの以外のもの及び普通地方公共団体が使用のために保管する動産（政令で定める動産を除く。）をいう。

### 第二百三十九条第二項

<!-- segment: simple id: chihou-jichi-hou-art-239-p2 -->
物品に関する事務に従事する職員は、その取扱いに係る物品（政令で定める物品を除く。）を普通地方公共団体から譲り受けることができない。

### 第二百三十九条第三項

<!-- segment: simple id: chihou-jichi-hou-art-239-p3 -->
前項の規定に違反する行為は、これを無効とする。

### 第二百三十九条第四項

<!-- segment: simple id: chihou-jichi-hou-art-239-p4 -->
前二項に定めるもののほか、物品の管理及び処分に関し必要な事項は、政令でこれを定める。

### 第二百三十九条第五項

<!-- segment: simple id: chihou-jichi-hou-art-239-p5 -->
普通地方公共団体の所有に属しない動産で普通地方公共団体が保管するもの（使用のために保管するものを除く。）のうち政令で定めるもの（以下「占有動産」という。）の管理に関し必要な事項は、政令でこれを定める。
