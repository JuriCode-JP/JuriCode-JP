---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '247'
article_id: chihou-jichi-hou-art-247
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-247-p1
    type: simple
    text: 国の行政機関又は都道府県の機関は、普通地方公共団体に対し、助言、勧告その他これらに類する行為（以下本条及び第二百五十二条の十七の三第二項において「助言等」という。）を書面によらないで行つた場合において、当該普通地方公共団体から当該助言等の趣旨及び内容を記載した書面の交付を求められたときは、これを交付しなければならない。
    modality: gimu
    references:
    - 本条
- number: 2
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-247-p2-hashira
    type: hashira
    text: 前項の規定は、次に掲げる助言等については、適用しない。
    modality: jogai
    references:
    - 前項
  - id: chihou-jichi-hou-art-247-p2-kou-1
    type: kou
    text: 一　普通地方公共団体に対しその場において完了する行為を求めるもの
    modality: unspecified
    item_number: 1
  - id: chihou-jichi-hou-art-247-p2-kou-2
    type: kou
    text: 二　既に書面により当該普通地方公共団体に通知されている事項と同一の内容であるもの
    modality: unspecified
    item_number: 2
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-247-p3
    type: simple
    text: 国又は都道府県の職員は、普通地方公共団体が国の行政機関又は都道府県の機関が行つた助言等に従わなかつたことを理由として、不利益な取扱いをしてはならない。
    modality: gimu_negative
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 12
  shou_name_ja: 第十二章　国と普通地方公共団体との関係及び普通地方公共団体相互間の関係
  setsu: 1
  setsu_name_ja: 第一節　普通地方公共団体に対する国又は都道府県の関与等
  kan: 2
  kan_name_ja: 第二款　普通地方公共団体に対する国又は都道府県の関与等の手続
---

# 地方自治法 第247条(（助言等の方式等）)

## 原文 (日本語)

### 第二百四十七条第一項

国の行政機関又は都道府県の機関は、普通地方公共団体に対し、助言、勧告その他これらに類する行為（以下本条及び第二百五十二条の十七の三第二項において「助言等」という。）を書面によらないで行つた場合において、当該普通地方公共団体から当該助言等の趣旨及び内容を記載した書面の交付を求められたときは、これを交付しなければならない。

### 第二百四十七条第二項

前項の規定は、次に掲げる助言等については、適用しない。

一　普通地方公共団体に対しその場において完了する行為を求めるもの

二　既に書面により当該普通地方公共団体に通知されている事項と同一の内容であるもの

### 第二百四十七条第三項

国又は都道府県の職員は、普通地方公共団体が国の行政機関又は都道府県の機関が行つた助言等に従わなかつたことを理由として、不利益な取扱いをしてはならない。
