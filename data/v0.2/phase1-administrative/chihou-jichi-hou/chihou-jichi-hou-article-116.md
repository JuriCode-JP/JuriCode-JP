---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '116'
article_id: chihou-jichi-hou-art-116
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-116-p1
    type: simple
    text: この法律に特別の定がある場合を除く外、普通地方公共団体の議会の議事は、出席議員の過半数でこれを決し、可否同数のときは、議長の決するところによる。
    modality: unspecified
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-116-p2
    type: simple
    text: 前項の場合においては、議長は、議員として議決に加わる権利を有しない。
    modality: unspecified
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 6
  shou_name_ja: 第六章　議会
  setsu: 6
  setsu_name_ja: 第六節　会議
---

# 地方自治法 第116条

## 原文 (日本語)

### 第百十六条第一項

<!-- segment: simple id: chihou-jichi-hou-art-116-p1 -->
この法律に特別の定がある場合を除く外、普通地方公共団体の議会の議事は、出席議員の過半数でこれを決し、可否同数のときは、議長の決するところによる。

### 第百十六条第二項

<!-- segment: simple id: chihou-jichi-hou-art-116-p2 -->
前項の場合においては、議長は、議員として議決に加わる権利を有しない。
