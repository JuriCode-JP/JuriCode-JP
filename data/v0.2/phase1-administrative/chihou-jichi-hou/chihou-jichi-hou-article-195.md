---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '195'
article_id: chihou-jichi-hou-art-195
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-195-p1
    type: simple
    text: 普通地方公共団体に監査委員を置く。
    modality: unspecified
- number: 2
  has_proviso: true
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-195-p2-honbun
    type: honbun
    text: 監査委員の定数は、都道府県及び政令で定める市にあつては四人とし、その他の市及び町村にあつては二人とする。
    modality: gimu
  - id: chihou-jichi-hou-art-195-p2-tadashi
    type: tadashi
    text: ただし、条例でその定数を増加することができる。
    modality: kanou_kenri
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 7
  shou_name_ja: 第七章　執行機関
  setsu: 3
  setsu_name_ja: 第三節　委員会及び委員
  kan: 5
  kan_name_ja: 第五款　監査委員
---

# 地方自治法 第195条

## 原文 (日本語)

### 第百九十五条第一項

普通地方公共団体に監査委員を置く。

### 第百九十五条第二項

監査委員の定数は、都道府県及び政令で定める市にあつては四人とし、その他の市及び町村にあつては二人とする。ただし、条例でその定数を増加することができる。
