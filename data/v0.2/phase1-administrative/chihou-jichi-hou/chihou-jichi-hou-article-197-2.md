---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: 197-2
article_id: chihou-jichi-hou-art-197-2
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-197-2-p1-zen
    type: zen_dan
    text: 普通地方公共団体の長は、監査委員が心身の故障のため職務の遂行に堪えないと認めるとき、又は監査委員に職務上の義務違反その他監査委員たるに適しない非行があると認めるときは、議会の同意を得て、これを罷免することができる。
    modality: kanou_kenri
  - id: chihou-jichi-hou-art-197-2-p1-kou
    type: kou_dan
    text: この場合においては、議会の常任委員会又は特別委員会において公聴会を開かなければならない。
    modality: gimu
    depends_on: chihou-jichi-hou-art-197-2-p1-zen
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-197-2-p2
    type: simple
    text: 監査委員は、前項の規定による場合を除くほか、その意に反して罷免されることがない。
    modality: unspecified
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 7
  shou_name_ja: 第七章　執行機関
  setsu: 3
  setsu_name_ja: 第三節　委員会及び委員
  kan: 5
  kan_name_ja: 第五款　監査委員
---

# 地方自治法 第197-2条

## 原文 (日本語)

### 第百九十七条の二第一項

普通地方公共団体の長は、監査委員が心身の故障のため職務の遂行に堪えないと認めるとき、又は監査委員に職務上の義務違反その他監査委員たるに適しない非行があると認めるときは、議会の同意を得て、これを罷免することができる。この場合においては、議会の常任委員会又は特別委員会において公聴会を開かなければならない。

### 第百九十七条の二第二項

監査委員は、前項の規定による場合を除くほか、その意に反して罷免されることがない。
