---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '237'
article_id: chihou-jichi-hou-art-237
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-237-p1
    type: simple
    text: この法律において「財産」とは、公有財産、物品及び債権並びに基金をいう。
    modality: teigi
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-237-p2
    type: simple
    text: 第二百三十八条の四第一項の規定の適用がある場合を除き、普通地方公共団体の財産は、条例又は議会の議決による場合でなければ、これを交換し、出資の目的とし、若しくは支払手段として使用し、又は適正な対価なくしてこれを譲渡し、若しくは貸し付けてはならない。
    modality: unspecified
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-237-p3
    type: simple
    text: 普通地方公共団体の財産は、第二百三十八条の五第二項の規定の適用がある場合で議会の議決によるとき又は同条第三項の規定の適用がある場合でなければ、これを信託してはならない。
    modality: gimu_negative
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 9
  shou_name_ja: 第九章　財務
  setsu: 9
  setsu_name_ja: 第九節　財産
---

# 地方自治法 第237条(（財産の管理及び処分）)

## 原文 (日本語)

### 第二百三十七条第一項

この法律において「財産」とは、公有財産、物品及び債権並びに基金をいう。

### 第二百三十七条第二項

第二百三十八条の四第一項の規定の適用がある場合を除き、普通地方公共団体の財産は、条例又は議会の議決による場合でなければ、これを交換し、出資の目的とし、若しくは支払手段として使用し、又は適正な対価なくしてこれを譲渡し、若しくは貸し付けてはならない。

### 第二百三十七条第三項

普通地方公共団体の財産は、第二百三十八条の五第二項の規定の適用がある場合で議会の議決によるとき又は同条第三項の規定の適用がある場合でなければ、これを信託してはならない。
