---
law_id: 322AC0000000067
law_name_ja: 地方自治法
law_name_en: Local Autonomy Act
article_number: '112'
article_id: chihou-jichi-hou-art-112
version_date: '1947-04-17'
source_url: https://laws.e-gov.go.jp/law/322AC0000000067
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: true
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-112-p1
    type: simple
    text: 普通地方公共団体の議会の議員は、議会の議決すべき事件につき、議会に議案を提出することができる。但し、予算については、この限りでない。
    modality: jogai
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-112-p2
    type: simple
    text: 前項の規定により議案を提出するに当たつては、議員の定数の十二分の一以上の者の賛成がなければならない。
    modality: gimu
    references:
    - 前項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-jichi-hou-art-112-p3
    type: simple
    text: 第一項の規定による議案の提出は、文書を以てこれをしなければならない。
    modality: gimu
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　普通地方公共団体
  shou: 6
  shou_name_ja: 第六章　議会
  setsu: 6
  setsu_name_ja: 第六節　会議
---

# 地方自治法 第112条

## 原文 (日本語)

### 第百十二条第一項

普通地方公共団体の議会の議員は、議会の議決すべき事件につき、議会に議案を提出することができる。但し、予算については、この限りでない。

### 第百十二条第二項

前項の規定により議案を提出するに当たつては、議員の定数の十二分の一以上の者の賛成がなければならない。

### 第百十二条第三項

第一項の規定による議案の提出は、文書を以てこれをしなければならない。
