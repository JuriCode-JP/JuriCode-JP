---
law_id: 415AC0000000057
law_name_ja: 個人情報の保護に関する法律
law_name_en: (English name pending)
article_number: '174'
article_id: kojin-jouhou-hogo-hou-art-174
version_date: '2003-05-30'
source_url: https://laws.e-gov.go.jp/law/415AC0000000057
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kojin-jouhou-hogo-hou-art-174-p1
    type: simple
    text: 内閣総理大臣及びこの法律の施行に関係する行政機関の長（会計検査院長を除く。）は、相互に緊密に連絡し、及び協力しなければならない。
    modality: gimu
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  shou: 7
  shou_name_ja: 第七章　雑則
---

# 個人情報の保護に関する法律 第174条(（連絡及び協力）)

## 原文 (日本語)

### 第百七十四条

<!-- segment: simple id: kojin-jouhou-hogo-hou-art-174-p1 -->
内閣総理大臣及びこの法律の施行に関係する行政機関の長（会計検査院長を除く。）は、相互に緊密に連絡し、及び協力しなければならない。
