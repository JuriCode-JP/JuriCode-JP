---
law_id: 415AC0000000057
law_name_ja: 個人情報の保護に関する法律
law_name_en: (English name pending)
article_number: '8'
article_id: kojin-jouhou-hogo-hou-art-8
version_date: '2003-05-30'
source_url: https://laws.e-gov.go.jp/law/415AC0000000057
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kojin-jouhou-hogo-hou-art-8-p1
    type: simple
    text: 国は、その機関が保有する個人情報の適正な取扱いが確保されるよう必要な措置を講ずるものとする。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kojin-jouhou-hogo-hou-art-8-p2
    type: simple
    text: 国は、独立行政法人等について、その保有する個人情報の適正な取扱いが確保されるよう必要な措置を講ずるものとする。
    modality: gimu
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  shou: 3
  shou_name_ja: 第三章　個人情報の保護に関する施策等
  setsu: 2
  setsu_name_ja: 第二節　国の施策
---

# 個人情報の保護に関する法律 第8条(（国の機関等が保有する個人情報の保護）)

## 原文 (日本語)

### 第八条第一項

<!-- segment: simple id: kojin-jouhou-hogo-hou-art-8-p1 -->
国は、その機関が保有する個人情報の適正な取扱いが確保されるよう必要な措置を講ずるものとする。

### 第八条第二項

<!-- segment: simple id: kojin-jouhou-hogo-hou-art-8-p2 -->
国は、独立行政法人等について、その保有する個人情報の適正な取扱いが確保されるよう必要な措置を講ずるものとする。
