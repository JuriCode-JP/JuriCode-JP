---
law_id: 415AC0000000057
law_name_ja: 個人情報の保護に関する法律
law_name_en: (English name pending)
article_number: '66'
article_id: kojin-jouhou-hogo-hou-art-66
version_date: '2003-05-30'
source_url: https://laws.e-gov.go.jp/law/415AC0000000057
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kojin-jouhou-hogo-hou-art-66-p1
    type: simple
    text: 行政機関の長等は、保有個人情報の漏えい、滅失又は毀損の防止その他の保有個人情報の安全管理のために必要かつ適切な措置を講じなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kojin-jouhou-hogo-hou-art-66-p2
    type: junyou
    text: 前項の規定は、次の各号に掲げる者が当該各号に定める業務を行う場合における個人情報の取扱いについて準用する。
    modality: unspecified
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  shou: 5
  shou_name_ja: 第五章　行政機関等の義務等
  setsu: 2
  setsu_name_ja: 第二節　行政機関等における個人情報等の取扱い
---

# 個人情報の保護に関する法律 第66条(（安全管理措置）)

## 原文 (日本語)

### 第六十六条第一項

<!-- segment: simple id: kojin-jouhou-hogo-hou-art-66-p1 -->
行政機関の長等は、保有個人情報の漏えい、滅失又は毀損の防止その他の保有個人情報の安全管理のために必要かつ適切な措置を講じなければならない。

### 第六十六条第二項

<!-- segment: junyou id: kojin-jouhou-hogo-hou-art-66-p2 -->
前項の規定は、次の各号に掲げる者が当該各号に定める業務を行う場合における個人情報の取扱いについて準用する。
