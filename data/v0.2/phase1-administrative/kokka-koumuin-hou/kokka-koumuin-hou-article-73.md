---
law_id: 322AC0000000120
law_name_ja: 国家公務員法
law_name_en: (English name pending)
article_number: '73'
article_id: kokka-koumuin-hou-art-73
version_date: '1947-10-21'
source_url: https://laws.e-gov.go.jp/law/322AC0000000120
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kokka-koumuin-hou-art-73-p1-hashira
    type: hashira
    text: 内閣総理大臣及び関係庁の長は、職員の勤務能率の発揮及び増進のために、次に掲げる事項について計画を樹立し、その実施に努めなければならない。
    modality: doryoku_gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kokka-koumuin-hou-art-73-p2
    type: simple
    text: 前項の計画の樹立及び実施に関し、内閣総理大臣は、その総合的企画並びに関係各庁に対する調整及び監視を行う。
    modality: unspecified
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  shou: 3
  shou_name_ja: 第三章　職員に適用される基準
  setsu: 5
  setsu_name_ja: 第五節　能率
---

# 国家公務員法 第73条(（能率増進計画）)

## 原文 (日本語)

### 第七十三条第一項

<!-- segment: hashira id: kokka-koumuin-hou-art-73-p1-hashira -->
内閣総理大臣及び関係庁の長は、職員の勤務能率の発揮及び増進のために、次に掲げる事項について計画を樹立し、その実施に努めなければならない。

### 第七十三条第二項

<!-- segment: simple id: kokka-koumuin-hou-art-73-p2 -->
前項の計画の樹立及び実施に関し、内閣総理大臣は、その総合的企画並びに関係各庁に対する調整及び監視を行う。
