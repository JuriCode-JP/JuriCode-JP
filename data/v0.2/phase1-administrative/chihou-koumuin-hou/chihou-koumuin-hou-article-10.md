---
law_id: 325AC0000000261
law_name_ja: 地方公務員法
law_name_en: (English name pending)
article_number: '10'
article_id: chihou-koumuin-hou-art-10
version_date: '1950-12-13'
source_url: https://laws.e-gov.go.jp/law/325AC0000000261
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-koumuin-hou-art-10-p1
    type: simple
    text: 人事委員会又は公平委員会は、委員のうちから委員長を選挙しなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-koumuin-hou-art-10-p2
    type: simple
    text: 委員長は、委員会に関する事務を処理し、委員会を代表する。
    modality: unspecified
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-koumuin-hou-art-10-p3
    type: simple
    text: 委員長に事故があるとき、又は委員長が欠けたときは、委員長の指定する委員が、その職務を代理する。
    modality: unspecified
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  shou: 2
  shou_name_ja: 第二章　人事機関
---

# 地方公務員法 第10条(（人事委員会又は公平委員会の委員長）)

## 原文 (日本語)

### 第十条第一項

<!-- segment: simple id: chihou-koumuin-hou-art-10-p1 -->
人事委員会又は公平委員会は、委員のうちから委員長を選挙しなければならない。

### 第十条第二項

<!-- segment: simple id: chihou-koumuin-hou-art-10-p2 -->
委員長は、委員会に関する事務を処理し、委員会を代表する。

### 第十条第三項

<!-- segment: simple id: chihou-koumuin-hou-art-10-p3 -->
委員長に事故があるとき、又は委員長が欠けたときは、委員長の指定する委員が、その職務を代理する。
