---
law_id: 325AC0000000261
law_name_ja: 地方公務員法
law_name_en: (English name pending)
article_number: '4'
article_id: chihou-koumuin-hou-art-4
version_date: '1950-12-13'
source_url: https://laws.e-gov.go.jp/law/325AC0000000261
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-koumuin-hou-art-4-p1
    type: simple
    text: この法律の規定は、一般職に属するすべての地方公務員（以下「職員」という。）に適用する。
    modality: unspecified
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-koumuin-hou-art-4-p2
    type: simple
    text: この法律の規定は、法律に特別の定がある場合を除く外、特別職に属する地方公務員には適用しない。
    modality: jogai
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  shou: 1
  shou_name_ja: 第一章　総則
---

# 地方公務員法 第4条(（この法律の適用を受ける地方公務員）)

## 原文 (日本語)

### 第四条第一項

<!-- segment: simple id: chihou-koumuin-hou-art-4-p1 -->
この法律の規定は、一般職に属するすべての地方公務員（以下「職員」という。）に適用する。

### 第四条第二項

<!-- segment: simple id: chihou-koumuin-hou-art-4-p2 -->
この法律の規定は、法律に特別の定がある場合を除く外、特別職に属する地方公務員には適用しない。
