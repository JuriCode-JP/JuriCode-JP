---
law_id: 426AC0000000068
law_name_ja: 行政不服審査法
law_name_en: Administrative Complaint Review Act
article_number: '28'
article_id: gyousei-fufuku-shinsa-hou-art-28
version_date: '2014-06-13'
source_url: https://laws.e-gov.go.jp/law/426AC0000000068
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: gyousei-fufuku-shinsa-hou-art-28-p1
    type: simple
    text: 審査請求人、参加人及び処分庁等（以下「審理関係人」という。）並びに審理員は、簡易迅速かつ公正な審理の実現のため、審理において、相互に協力するとともに、審理手続の計画的な進行を図らなければならない。
    modality: gimu
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  shou: 2
  shou_name_ja: 第二章　審査請求
  setsu: 3
  setsu_name_ja: 第三節　審理手続
---

# 行政不服審査法 第28条(（審理手続の計画的進行）)

## 原文 (日本語)

### 第二十八条

審査請求人、参加人及び処分庁等（以下「審理関係人」という。）並びに審理員は、簡易迅速かつ公正な審理の実現のため、審理において、相互に協力するとともに、審理手続の計画的な進行を図らなければならない。
