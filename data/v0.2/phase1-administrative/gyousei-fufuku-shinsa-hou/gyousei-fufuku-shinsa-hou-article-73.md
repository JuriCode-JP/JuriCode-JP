---
law_id: 426AC0000000068
law_name_ja: 行政不服審査法
law_name_en: Administrative Complaint Review Act
article_number: '73'
article_id: gyousei-fufuku-shinsa-hou-art-73
version_date: '2014-06-13'
source_url: https://laws.e-gov.go.jp/law/426AC0000000068
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: gyousei-fufuku-shinsa-hou-art-73-p1
    type: simple
    text: 審査会の事務を処理させるため、審査会に事務局を置く。
    modality: unspecified
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: gyousei-fufuku-shinsa-hou-art-73-p2
    type: simple
    text: 事務局に、事務局長のほか、所要の職員を置く。
    modality: unspecified
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: gyousei-fufuku-shinsa-hou-art-73-p3
    type: simple
    text: 事務局長は、会長の命を受けて、局務を掌理する。
    modality: unspecified
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  shou: 5
  shou_name_ja: 第五章　行政不服審査会等
  setsu: 1
  setsu_name_ja: 第一節　行政不服審査会
  kan: 1
  kan_name_ja: 第一款　設置及び組織
---

# 行政不服審査法 第73条(（事務局）)

## 原文 (日本語)

### 第七十三条第一項

審査会の事務を処理させるため、審査会に事務局を置く。

### 第七十三条第二項

事務局に、事務局長のほか、所要の職員を置く。

### 第七十三条第三項

事務局長は、会長の命を受けて、局務を掌理する。
