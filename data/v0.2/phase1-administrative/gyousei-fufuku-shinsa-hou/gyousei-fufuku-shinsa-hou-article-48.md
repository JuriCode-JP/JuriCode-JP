---
law_id: 426AC0000000068
law_name_ja: 行政不服審査法
law_name_en: Administrative Complaint Review Act
article_number: '48'
article_id: gyousei-fufuku-shinsa-hou-art-48
version_date: '2014-06-13'
source_url: https://laws.e-gov.go.jp/law/426AC0000000068
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: gyousei-fufuku-shinsa-hou-art-48-p1
    type: simple
    text: 第四十六条第一項本文又は前条の場合において、審査庁は、審査請求人の不利益に当該処分を変更し、又は当該事実上の行為を変更すべき旨を命じ、若しくはこれを変更することはできない。
    modality: kanou_negative
    references:
    - 前条
cases: []
amendments: []
tags:
- phase1-administrative
- auto-generated
parent_section:
  shou: 2
  shou_name_ja: 第二章　審査請求
  setsu: 5
  setsu_name_ja: 第五節　裁決
---

# 行政不服審査法 第48条(（不利益変更の禁止）)

## 原文 (日本語)

### 第四十八条

<!-- segment: simple id: gyousei-fufuku-shinsa-hou-art-48-p1 -->
第四十六条第一項本文又は前条の場合において、審査庁は、審査請求人の不利益に当該処分を変更し、又は当該事実上の行為を変更すべき旨を命じ、若しくはこれを変更することはできない。
