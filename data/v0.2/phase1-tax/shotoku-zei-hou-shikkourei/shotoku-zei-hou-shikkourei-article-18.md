---
law_id: 340CO0000000096
law_name_ja: 所得税法施行令
law_name_en: (English name pending)
article_number: '18'
article_id: shotoku-zei-hou-shikkourei-art-18
version_date: '1965-03-31'
source_url: https://laws.e-gov.go.jp/law/340CO0000000096
source_format: e-gov-xml
last_verified: '2026-06-01'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: shotoku-zei-hou-shikkourei-art-18-p1
    type: simple
    text: 法第九条第一項第一号（非課税所得）に規定する政令で定める利子は、年一パーセントを超える利率の利子を付された当座預金の利子とする。
    modality: gimu
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
parent_section:
  hen: 1
  hen_name_ja: 第一編　総則
  shou: 2
  shou_name_ja: 第二章　課税所得の範囲
  setsu: 2
  setsu_name_ja: 第二節　非課税所得
---

# 所得税法施行令 第18条(（非課税とされない当座預金の利子）)

## 原文 (日本語)

### 第十八条

<!-- segment: simple id: shotoku-zei-hou-shikkourei-art-18-p1 -->
法第九条第一項第一号（非課税所得）に規定する政令で定める利子は、年一パーセントを超える利率の利子を付された当座預金の利子とする。
