---
law_id: 340M50000040011
law_name_ja: 所得税法施行規則
law_name_en: (English name pending)
article_number: '80'
article_id: shotoku-zei-hou-shikoukisoku-art-80
version_date: '1965-03-31'
source_url: https://laws.e-gov.go.jp/law/340M50000040011
source_format: e-gov-xml
last_verified: '2026-06-01'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: shotoku-zei-hou-shikoukisoku-art-80-p1
    type: simple
    text: 法第二百二十条（源泉徴収に係る所得税の納付手続）に規定する計算書の書式は、別表第三（一）から別表第三（六）までによる。
    modality: unspecified
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
parent_section:
  hen: 4
  hen_name_ja: 第四編　源泉徴収
  shou: 6
  shou_name_ja: 第六章　源泉徴収に係る所得税の納付
---

# 所得税法施行規則 第80条(（計算書の書式）)

## 原文 (日本語)

### 第八十条

<!-- segment: simple id: shotoku-zei-hou-shikoukisoku-art-80-p1 -->
法第二百二十条（源泉徴収に係る所得税の納付手続）に規定する計算書の書式は、別表第三（一）から別表第三（六）までによる。
