---
law_id: 340M50000040011
law_name_ja: 所得税法施行規則
law_name_en: (English name pending)
article_number: '52'
article_id: shotoku-zei-hou-shikoukisoku-art-52
version_date: '1965-03-31'
source_url: https://laws.e-gov.go.jp/law/340M50000040011
source_format: e-gov-xml
last_verified: '2026-06-01'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: shotoku-zei-hou-shikoukisoku-art-52-p1-hashira
    type: hashira
    text: 法第百三十四条第一項（延払条件付譲渡に係る所得税額の延納条件の変更の手続）に規定する財務省令で定める事項は、次に掲げる事項とする。
    modality: gimu
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　居住者の納税義務
  shou: 3
  shou_name_ja: 第三章　申告、納付及び還付
  setsu: 2
  setsu_name_ja: 第二節　確定申告並びにこれに伴う納付及び還付
  kan: 2
  kan_name_ja: 第二款　延納
---

# 所得税法施行規則 第52条(（延払条件付譲渡に係る所得税額の延納条件の変更の申請書の記載事項）)

## 原文 (日本語)

### 第五十二条

<!-- segment: hashira id: shotoku-zei-hou-shikoukisoku-art-52-p1-hashira -->
法第百三十四条第一項（延払条件付譲渡に係る所得税額の延納条件の変更の手続）に規定する財務省令で定める事項は、次に掲げる事項とする。
