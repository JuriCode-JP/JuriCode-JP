---
law_id: 340M50000040011
law_name_ja: 所得税法施行規則
law_name_en: (English name pending)
article_number: '22'
article_id: shotoku-zei-hou-shikoukisoku-art-22
version_date: '1965-03-31'
source_url: https://laws.e-gov.go.jp/law/340M50000040011
source_format: e-gov-xml
last_verified: '2026-06-01'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: shotoku-zei-hou-shikoukisoku-art-22-p1
    type: simple
    text: 令第九十九条の二第二項（棚卸資産の特別な評価の方法）に規定する財務省令で定める事項は、同項に規定する申請書を提出する者の氏名及び住所（国内に住所がない場合には、居所。以下この款及び第二款において同じ。）その他参考となるべき事項とする。
    modality: gimu
    references:
    - 同項
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　居住者の納税義務
  shou: 1
  shou_name_ja: 第一章　各種所得の金額の計算
  setsu: 3
  setsu_name_ja: 第三節　必要経費等の計算
  kan: 12
  kan_name_ja: 第一款の二　棚卸資産の評価
---

# 所得税法施行規則 第22条(（特別な評価の方法の承認申請書の記載事項）)

## 原文 (日本語)

### 第二十二条

<!-- segment: simple id: shotoku-zei-hou-shikoukisoku-art-22-p1 -->
令第九十九条の二第二項（棚卸資産の特別な評価の方法）に規定する財務省令で定める事項は、同項に規定する申請書を提出する者の氏名及び住所（国内に住所がない場合には、居所。以下この款及び第二款において同じ。）その他参考となるべき事項とする。
