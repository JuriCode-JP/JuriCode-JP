---
law_id: 340M50000040011
law_name_ja: 所得税法施行規則
law_name_en: (English name pending)
article_number: 34-3
article_id: shotoku-zei-hou-shikoukisoku-art-34-3
version_date: '1965-03-31'
source_url: https://laws.e-gov.go.jp/law/340M50000040011
source_format: e-gov-xml
last_verified: '2026-06-01'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: shotoku-zei-hou-shikoukisoku-art-34-3-p1
    type: junyou
    text: 前条の規定は、令第百三十九条第一項（一括償却資産の必要経費算入）に規定する主要な業務として行われる貸付けに該当するかどうかの判定について準用する。
    modality: unspecified
    applies_provisions:
    - 前条
    references:
    - 前条
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　居住者の納税義務
  shou: 1
  shou_name_ja: 第一章　各種所得の金額の計算
  setsu: 3
  setsu_name_ja: 第三節　必要経費等の計算
  kan: 2
  kan_name_ja: 第二款　減価償却資産の償却
---

# 所得税法施行規則 第34-3条(（一括償却資産の主要な業務として行う貸付けの判定）)

## 原文 (日本語)

### 第三十四条の三

<!-- segment: junyou id: shotoku-zei-hou-shikoukisoku-art-34-3-p1 applies_provisions: 前条 -->
前条の規定は、令第百三十九条第一項（一括償却資産の必要経費算入）に規定する主要な業務として行われる貸付けに該当するかどうかの判定について準用する。
