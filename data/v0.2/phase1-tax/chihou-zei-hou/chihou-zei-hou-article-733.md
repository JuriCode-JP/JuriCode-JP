---
law_id: 325AC0000000226
law_name_ja: 地方税法
law_name_en: (English name pending)
article_number: '733'
article_id: chihou-zei-hou-art-733
version_date: '1950-07-31'
source_url: https://laws.e-gov.go.jp/law/325AC0000000226
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: chihou-zei-hou-art-733-p1-hashira
    type: hashira
    text: 総務大臣は、第七百三十一条第二項の規定による協議の申出を受けた場合には、当該協議の申出に係る法定外目的税について次に掲げる事由のいずれかがあると認める場合を除き、これに同意しなければならない。
    modality: gimu
  - id: chihou-zei-hou-art-733-p1-kou-1
    type: kou
    text: 一　国税又は他の地方税と課税標準を同じくし、かつ、住民の負担が著しく過重となること。
    modality: unspecified
    item_number: 1
  - id: chihou-zei-hou-art-733-p1-kou-2
    type: kou
    text: 二　地方団体間における物の流通に重大な障害を与えること。
    modality: unspecified
    item_number: 2
  - id: chihou-zei-hou-art-733-p1-kou-3
    type: kou
    text: 三　前二号に掲げるものを除くほか、国の経済施策に照らして適当でないこと。
    modality: unspecified
    item_number: 3
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
parent_section:
  shou: 4
  shou_name_ja: 第四章　目的税
  setsu: 8
  setsu_name_ja: 第八節　法定外目的税
---

# 地方税法 第733条(（総務大臣の同意）)

## 原文 (日本語)

### 第七百三十三条

総務大臣は、第七百三十一条第二項の規定による協議の申出を受けた場合には、当該協議の申出に係る法定外目的税について次に掲げる事由のいずれかがあると認める場合を除き、これに同意しなければならない。

一　国税又は他の地方税と課税標準を同じくし、かつ、住民の負担が著しく過重となること。

二　地方団体間における物の流通に重大な障害を与えること。

三　前二号に掲げるものを除くほか、国の経済施策に照らして適当でないこと。
