---
law_id: 325AC0000000226
law_name_ja: 地方税法
law_name_en: (English name pending)
article_number: '198'
article_id: chihou-zei-hou-art-198
version_date: '1950-07-31'
source_url: https://laws.e-gov.go.jp/law/325AC0000000226
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-zei-hou-art-198-p1
    type: simple
    text: 納税者が納期限までに鉱区税に係る地方団体の徴収金を完納しない場合においては、道府県の徴税吏員は、納期限後二十日以内に、督促状を発しなければならない。但し、繰上徴収をする場合においては、この限りでない。
    modality: jogai
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-zei-hou-art-198-p2
    type: simple
    text: 特別の事情がある道府県においては、当該道府県の条例で前項に規定する期間と異なる期間を定めることができる。
    modality: kanou_kenri
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  shou: 2
  shou_name_ja: 第二章　道府県の普通税
  setsu: 9
  setsu_name_ja: 第九節　鉱区税
---

# 地方税法 第198条(（鉱区税に係る督促）)

## 原文 (日本語)

### 第百九十八条第一項

<!-- segment: simple id: chihou-zei-hou-art-198-p1 -->
納税者が納期限までに鉱区税に係る地方団体の徴収金を完納しない場合においては、道府県の徴税吏員は、納期限後二十日以内に、督促状を発しなければならない。但し、繰上徴収をする場合においては、この限りでない。

### 第百九十八条第二項

<!-- segment: simple id: chihou-zei-hou-art-198-p2 -->
特別の事情がある道府県においては、当該道府県の条例で前項に規定する期間と異なる期間を定めることができる。
