---
law_id: 325AC0000000226
law_name_ja: 地方税法
law_name_en: (English name pending)
article_number: 24-5
article_id: chihou-zei-hou-art-24-5
version_date: '1950-07-31'
source_url: https://laws.e-gov.go.jp/law/325AC0000000226
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: true
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: chihou-zei-hou-art-24-5-p1-hashira
    type: hashira
    text: 道府県は、次の各号のいずれかに該当する者に対しては、道府県民税の均等割及び所得割（第二号に該当する者にあつては、第五十条の二の規定により課する所得割（以下この款及び次款において「分離課税に係る所得割」という。）を除く。）を課することができない。ただし、この法律の施行地に住所を有しない者については、この限りでない。
    modality: jogai
  - id: chihou-zei-hou-art-24-5-p1-kou-1
    type: kou
    text: 生活保護法（昭和二十五年法律第百四十四号）の規定による生活扶助を受けている者
    modality: unspecified
    item_number: 1
  - id: chihou-zei-hou-art-24-5-p1-kou-2
    type: kou
    text: 障害者、未成年者、寡婦又はひとり親（これらの者の前年の合計所得金額が百三十五万円を超える場合を除く。）
    modality: unspecified
    item_number: 2
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-zei-hou-art-24-5-p2
    type: simple
    text: 分離課税に係る所得割につき前項第一号の規定を適用する場合における同号に掲げる者であるかどうかの判定は、退職手当等の支払を受けるべき日の属する年の一月一日の現況によるものとする。
    modality: gimu
    references:
    - 前項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-zei-hou-art-24-5-p3
    type: simple
    text: 道府県は、第二百九十五条第三項の規定により個人の市町村民税の均等割を課することができないこととされる者に対しては、当該均等割と併せて賦課徴収すべき個人の道府県民税の均等割を課することができない。
    modality: kanou_negative
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
parent_section:
  shou: 2
  shou_name_ja: 第二章　道府県の普通税
  setsu: 1
  setsu_name_ja: 第一節　道府県民税
  kan: 1
  kan_name_ja: 第一款　通則
---

# 地方税法 第24-5条(（個人の道府県民税の非課税の範囲）)

## 原文 (日本語)

### 第二十四条の五第一項

道府県は、次の各号のいずれかに該当する者に対しては、道府県民税の均等割及び所得割（第二号に該当する者にあつては、第五十条の二の規定により課する所得割（以下この款及び次款において「分離課税に係る所得割」という。）を除く。）を課することができない。ただし、この法律の施行地に住所を有しない者については、この限りでない。

一　生活保護法（昭和二十五年法律第百四十四号）の規定による生活扶助を受けている者

二　障害者、未成年者、寡婦又はひとり親（これらの者の前年の合計所得金額が百三十五万円を超える場合を除く。）

### 第二十四条の五第二項

分離課税に係る所得割につき前項第一号の規定を適用する場合における同号に掲げる者であるかどうかの判定は、退職手当等の支払を受けるべき日の属する年の一月一日の現況によるものとする。

### 第二十四条の五第三項

道府県は、第二百九十五条第三項の規定により個人の市町村民税の均等割を課することができないこととされる者に対しては、当該均等割と併せて賦課徴収すべき個人の道府県民税の均等割を課することができない。
