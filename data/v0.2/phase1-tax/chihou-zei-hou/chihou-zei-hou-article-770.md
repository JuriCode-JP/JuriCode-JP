---
law_id: 325AC0000000226
law_name_ja: 地方税法
law_name_en: (English name pending)
article_number: '770'
article_id: chihou-zei-hou-art-770
version_date: '1950-07-31'
source_url: https://laws.e-gov.go.jp/law/325AC0000000226
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-zei-hou-art-770-p1
    type: simple
    text: 代表者会議に議長を置き、委員の互選によりこれを定める。
    modality: unspecified
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-zei-hou-art-770-p2
    type: simple
    text: 議長は、会務を総理し、代表者会議を代表する。
    modality: unspecified
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-zei-hou-art-770-p3
    type: simple
    text: 議長に事故があるとき、又は議長が欠けたときは、議長のあらかじめ指定する委員がその職務を行う。
    modality: unspecified
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  shou: 9
  shou_name_ja: 第九章　地方税共同機構
  setsu: 2
  setsu_name_ja: 第二節　代表者会議
---

# 地方税法 第770条(（代表者会議の議長）)

## 原文 (日本語)

### 第七百七十条第一項

<!-- segment: simple id: chihou-zei-hou-art-770-p1 -->
代表者会議に議長を置き、委員の互選によりこれを定める。

### 第七百七十条第二項

<!-- segment: simple id: chihou-zei-hou-art-770-p2 -->
議長は、会務を総理し、代表者会議を代表する。

### 第七百七十条第三項

<!-- segment: simple id: chihou-zei-hou-art-770-p3 -->
議長に事故があるとき、又は議長が欠けたときは、議長のあらかじめ指定する委員がその職務を行う。
