---
law_id: 325AC0000000226
law_name_ja: 地方税法
law_name_en: (English name pending)
article_number: '76'
article_id: chihou-zei-hou-art-76
version_date: '1950-07-31'
source_url: https://laws.e-gov.go.jp/law/325AC0000000226
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-zei-hou-art-76-p1
    type: simple
    text: ゴルフ場利用税の標準税率は、一人一日につき八百円とする。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-zei-hou-art-76-p2
    type: simple
    text: 道府県は、前項に定める標準税率を超える税率でゴルフ場利用税を課する場合には、千二百円を超える税率で課することができない。
    modality: kanou_negative
    references:
    - 前項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: chihou-zei-hou-art-76-p3-zen
    type: zen_dan
    text: 道府県は、ゴルフ場の整備の状況等に応じて、ゴルフ場利用税の税率に差等を設けることができる。
    modality: kanou_kenri
  - id: chihou-zei-hou-art-76-p3-kou
    type: kou_dan
    text: この場合においては、前項の規定を準用する。
    modality: unspecified
    references:
    - 前項
    depends_on: chihou-zei-hou-art-76-p3-zen
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
parent_section:
  shou: 2
  shou_name_ja: 第二章　道府県の普通税
  setsu: 6
  setsu_name_ja: 第六節　ゴルフ場利用税
  kan: 1
  kan_name_ja: 第一款　通則
---

# 地方税法 第76条(（ゴルフ場利用税の税率）)

## 原文 (日本語)

### 第七十六条第一項

<!-- segment: simple id: chihou-zei-hou-art-76-p1 -->
ゴルフ場利用税の標準税率は、一人一日につき八百円とする。

### 第七十六条第二項

<!-- segment: simple id: chihou-zei-hou-art-76-p2 -->
道府県は、前項に定める標準税率を超える税率でゴルフ場利用税を課する場合には、千二百円を超える税率で課することができない。

### 第七十六条第三項

<!-- segment: zen_dan id: chihou-zei-hou-art-76-p3-zen -->
道府県は、ゴルフ場の整備の状況等に応じて、ゴルフ場利用税の税率に差等を設けることができる。<!-- segment: kou_dan id: chihou-zei-hou-art-76-p3-kou depends_on: chihou-zei-hou-art-76-p3-zen -->
この場合においては、前項の規定を準用する。
