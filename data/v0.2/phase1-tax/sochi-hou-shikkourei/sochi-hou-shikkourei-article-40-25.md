---
law_id: 332CO0000000043
law_name_ja: 租税特別措置法施行令
law_name_en: (English name pending)
article_number: 40-25
article_id: sochi-hou-shikkourei-art-40-25
version_date: '1957-03-31'
source_url: https://laws.e-gov.go.jp/law/332CO0000000043
source_format: e-gov-xml
last_verified: '2026-07-03'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: sochi-hou-shikkourei-art-40-25-p1-hashira
    type: hashira
    text: 法第七十一条の十六第一項に規定する政令で定める無線設備は、電波法（昭和二十五年法律第百三十一号）第二条第四号に規定する無線設備（以下この項において「無線設備」という。）のうち次に掲げるすべての要件を満たすものとする。
    modality: gimu
  - id: sochi-hou-shikkourei-art-40-25-p1-kou-1
    type: kou
    text: 電波法第二条第一号に規定する電波を空間へ放射する無線設備で財務省令で定めるものに該当するものであること。
    modality: unspecified
    item_number: 1
  - id: sochi-hou-shikkourei-art-40-25-p1-kou-2
    type: kou
    text: 当該無線設備の用に供されている土地等の利用に相当の制約を伴うものであること。
    modality: unspecified
    item_number: 2
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: sochi-hou-shikkourei-art-40-25-p2
    type: simple
    text: 法第七十一条の十六第一項に規定する政令で定める土地等は、同項に規定する特定の放送用施設の用以外の用にも供されている土地等に係る面積の当該特定の放送用施設の用に供されている土地等に係る面積に対する割合が十分の一未満であるものとする。
    modality: gimu
    references:
    - 同項
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
parent_section:
  shou: 33
  shou_name_ja: 第三章の三　地価税法の特例
---

# 租税特別措置法施行令 第40-25条(（特定の放送用施設の用に供されている土地等について課税価格の計算の特例）)

## 原文 (日本語)

### 第四十条の二十五第一項

法第七十一条の十六第一項に規定する政令で定める無線設備は、電波法（昭和二十五年法律第百三十一号）第二条第四号に規定する無線設備（以下この項において「無線設備」という。）のうち次に掲げるすべての要件を満たすものとする。

一　電波法第二条第一号に規定する電波を空間へ放射する無線設備で財務省令で定めるものに該当するものであること。

二　当該無線設備の用に供されている土地等の利用に相当の制約を伴うものであること。

### 第四十条の二十五第二項

法第七十一条の十六第一項に規定する政令で定める土地等は、同項に規定する特定の放送用施設の用以外の用にも供されている土地等に係る面積の当該特定の放送用施設の用に供されている土地等に係る面積に対する割合が十分の一未満であるものとする。
