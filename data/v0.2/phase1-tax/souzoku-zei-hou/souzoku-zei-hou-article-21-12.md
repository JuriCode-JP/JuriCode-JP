---
law_id: 325AC0000000073
law_name_ja: 相続税法
law_name_en: (English name pending)
article_number: 21-12
article_id: souzoku-zei-hou-art-21-12
version_date: '1950-03-31'
source_url: https://laws.e-gov.go.jp/law/325AC0000000073
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: souzoku-zei-hou-art-21-12-p1-hashira
    type: hashira
    text: 相続時精算課税適用者がその年中において特定贈与者からの贈与により取得した財産に係るその年分の贈与税については、特定贈与者ごとの前条第一項の規定による控除後の贈与税の課税価格からそれぞれ次に掲げる金額のうちいずれか低い金額を控除する。
    modality: unspecified
    references:
    - 前条
  - id: souzoku-zei-hou-art-21-12-p1-kou-1
    type: kou
    text: 一　二千五百万円（既にこの条の規定の適用を受けて控除した金額がある場合には、その金額の合計額を控除した残額）
    modality: unspecified
    item_number: 1
  - id: souzoku-zei-hou-art-21-12-p1-kou-2
    type: kou
    text: 二　特定贈与者ごとの前条第一項の規定による控除後の贈与税の課税価格
    modality: unspecified
    item_number: 2
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: souzoku-zei-hou-art-21-12-p2
    type: simple
    text: 前項の規定は、期限内申告書に同項の規定により控除を受ける金額、既に同項の規定の適用を受けて控除した金額がある場合の控除した金額その他財務省令で定める事項の記載がある場合に限り、適用する。
    modality: unspecified
    references:
    - 前項
    - 同項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: souzoku-zei-hou-art-21-12-p3
    type: simple
    text: 税務署長は、第一項の財産について前項の記載がない期限内申告書の提出があつた場合において、その記載がなかつたことについてやむを得ない事情があると認めるときは、その記載をした書類の提出があつた場合に限り、第一項の規定を適用することができる。
    modality: kanou_kenri
    references:
    - 前項
cases: []
amendments:
- effective_date: '2024-01-01'
  law_num: 令和五年法律第三号
  law_name: 所得税法等の一部を改正する法律
  description: 本条を改正（（追加）→「前条第一項の規定による控除後の」ほか）
tags:
- phase1-tax
- auto-generated
parent_section:
  shou: 2
  shou_name_ja: 第二章　課税価格、税率及び控除
  setsu: 3
  setsu_name_ja: 第三節　相続時精算課税
---

# 相続税法 第21-12条(（相続時精算課税に係る贈与税の特別控除）)

## 原文 (日本語)

### 第二十一条の十二第一項

相続時精算課税適用者がその年中において特定贈与者からの贈与により取得した財産に係るその年分の贈与税については、特定贈与者ごとの前条第一項の規定による控除後の贈与税の課税価格からそれぞれ次に掲げる金額のうちいずれか低い金額を控除する。

一　二千五百万円（既にこの条の規定の適用を受けて控除した金額がある場合には、その金額の合計額を控除した残額）

二　特定贈与者ごとの前条第一項の規定による控除後の贈与税の課税価格

### 第二十一条の十二第二項

前項の規定は、期限内申告書に同項の規定により控除を受ける金額、既に同項の規定の適用を受けて控除した金額がある場合の控除した金額その他財務省令で定める事項の記載がある場合に限り、適用する。

### 第二十一条の十二第三項

税務署長は、第一項の財産について前項の記載がない期限内申告書の提出があつた場合において、その記載がなかつたことについてやむを得ない事情があると認めるときは、その記載をした書類の提出があつた場合に限り、第一項の規定を適用することができる。
