---
law_id: 332AC0000000026
law_name_ja: 租税特別措置法
law_name_en: (English name pending)
article_number: '5'
article_id: sochi-hou-art-5
version_date: '1957-03-31'
source_url: https://laws.e-gov.go.jp/law/332AC0000000026
source_format: e-gov-xml
last_verified: '2026-07-03'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: true
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: sochi-hou-art-5-p1-honbun
    type: honbun
    text: 納税準備預金の利子については、所得税を課さない。
    modality: unspecified
  - id: sochi-hou-art-5-p1-tadashi
    type: tadashi
    text: ただし、当該預金から租税の納付の目的以外の目的のために引き出された金額がある場合には、その引出しの日の属する利子の計算期間に対応する利子については、所得税を課する。
    modality: unspecified
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: sochi-hou-art-5-p2
    type: simple
    text: 前項に規定する納税準備預金とは、租税の納付に充てることを目的として銀行その他の政令で定める金融機関に対してした預金で当該金融機関が他の預金と区分して経理しているものをいう。
    modality: teigi
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
parent_section:
  shou: 2
  shou_name_ja: 第二章　所得税法の特例
  setsu: 1
  setsu_name_ja: 第一節　利子所得及び配当所得
---

# 租税特別措置法 第5条(（納税準備預金の利子の非課税）)

## 原文 (日本語)

### 第五条第一項

納税準備預金の利子については、所得税を課さない。ただし、当該預金から租税の納付の目的以外の目的のために引き出された金額がある場合には、その引出しの日の属する利子の計算期間に対応する利子については、所得税を課する。

### 第五条第二項

前項に規定する納税準備預金とは、租税の納付に充てることを目的として銀行その他の政令で定める金融機関に対してした預金で当該金融機関が他の預金と区分して経理しているものをいう。
