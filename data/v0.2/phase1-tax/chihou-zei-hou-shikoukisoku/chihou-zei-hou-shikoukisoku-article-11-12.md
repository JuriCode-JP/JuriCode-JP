---
law_id: 329M50000002023
law_name_ja: 地方税法施行規則
law_name_en: (English name pending)
article_number: 11-12
article_id: chihou-zei-hou-shikoukisoku-art-11-12
version_date: '1954-05-13'
source_url: https://laws.e-gov.go.jp/law/329M50000002023
source_format: e-gov-xml
last_verified: '2026-06-01'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: chihou-zei-hou-shikoukisoku-art-11-12-p1
    type: simple
    text: 法第三百四十九条の三第二十六項に規定する総務省令で定めるコンテナーは、次の要件に該当するコンテナー（当該要件に該当することについて地方運輸局（運輸監理部を含む。）又はその運輸支局若しくは海事事務所の長が証明したものに限る。）とする。
    modality: gimu
  - id: chihou-zei-hou-shikoukisoku-art-11-12-p1-kou-1
    type: kou
    text: その長さが六メートル以上のものであり、かつ、その幅及び高さがいずれも二・四メートル以上のものであること又はその最大積載重量が十八トン以上のものであること。
    modality: unspecified
    item_number: 1
  - id: chihou-zei-hou-shikoukisoku-art-11-12-p1-kou-2
    type: kou
    text: 当該年度の初日の属する年の前年中における外国貿易のために使用された日数の全使用日数に対する割合が八十パーセントを超えるものであること。
    modality: unspecified
    item_number: 2
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
---

# 地方税法施行規則 第11-12条(（法第三百四十九条の三第二十六項のコンテナー）)

## 原文 (日本語)

### 第十一条の十二

法第三百四十九条の三第二十六項に規定する総務省令で定めるコンテナーは、次の要件に該当するコンテナー（当該要件に該当することについて地方運輸局（運輸監理部を含む。）又はその運輸支局若しくは海事事務所の長が証明したものに限る。）とする。

一　その長さが六メートル以上のものであり、かつ、その幅及び高さがいずれも二・四メートル以上のものであること又はその最大積載重量が十八トン以上のものであること。

二　当該年度の初日の属する年の前年中における外国貿易のために使用された日数の全使用日数に対する割合が八十パーセントを超えるものであること。
