---
law_id: 337AC0000000066
law_name_ja: 国税通則法
law_name_en: (English name pending)
article_number: '82'
article_id: kokuzei-tsuusoku-hou-art-82
version_date: '1962-04-02'
source_url: https://laws.e-gov.go.jp/law/337AC0000000066
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kokuzei-tsuusoku-hou-art-82-p1-zen
    type: zen_dan
    text: 第七十五条第二項（第一号に係る部分に限る。）（国税局の職員の調査に係る処分についての再調査の請求）の規定による再調査の請求は、当該再調査の請求に係る処分をした税務署長を経由してすることもできる。
    modality: unspecified
  - id: kokuzei-tsuusoku-hou-art-82-p1-kou
    type: kou_dan
    text: この場合において、再調査の請求人は、当該税務署長に再調査の請求書を提出してするものとする。
    modality: gimu
    depends_on: kokuzei-tsuusoku-hou-art-82-p1-zen
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kokuzei-tsuusoku-hou-art-82-p2
    type: simple
    text: 前項の場合には、同項の税務署長は、直ちに、再調査の請求書を当該税務署長の管轄区域を所轄する国税局長に送付しなければならない。
    modality: gimu
    references:
    - 前項
    - 同項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kokuzei-tsuusoku-hou-art-82-p3
    type: simple
    text: 第一項の場合における再調査の請求期間の計算については、同項の税務署長に再調査の請求書が提出された時に再調査の請求がされたものとみなす。
    modality: unspecified
    references:
    - 同項
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
parent_section:
  shou: 8
  shou_name_ja: 第八章　不服審査及び訴訟
  setsu: 1
  setsu_name_ja: 第一節　不服審査
  kan: 2
  kan_name_ja: 第二款　再調査の請求
---

# 国税通則法 第82条(（税務署長を経由する再調査の請求）)

## 原文 (日本語)

### 第八十二条第一項

第七十五条第二項（第一号に係る部分に限る。）（国税局の職員の調査に係る処分についての再調査の請求）の規定による再調査の請求は、当該再調査の請求に係る処分をした税務署長を経由してすることもできる。この場合において、再調査の請求人は、当該税務署長に再調査の請求書を提出してするものとする。

### 第八十二条第二項

前項の場合には、同項の税務署長は、直ちに、再調査の請求書を当該税務署長の管轄区域を所轄する国税局長に送付しなければならない。

### 第八十二条第三項

第一項の場合における再調査の請求期間の計算については、同項の税務署長に再調査の請求書が提出された時に再調査の請求がされたものとみなす。
