---
law_id: 337AC0000000066
law_name_ja: 国税通則法
law_name_en: (English name pending)
article_number: 34-3
article_id: kokuzei-tsuusoku-hou-art-34-3
version_date: '1962-04-02'
source_url: https://laws.e-gov.go.jp/law/337AC0000000066
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kokuzei-tsuusoku-hou-art-34-3-p1-hashira
    type: hashira
    text: 国税を納付しようとする者は、その税額が財務省令で定める金額以下である場合であつて、次の各号のいずれかに該当するときは、納付受託者（次条第一項に規定する納付受託者をいう。以下この条において同じ。）に納付を委託することができる。
    modality: kanou_kenri
    references:
    - 次条
  - id: kokuzei-tsuusoku-hou-art-34-3-p1-kou-1
    type: kou
    text: 一　第三十四条第一項（納付の手続）に規定する納付書で財務省令で定めるものに基づき納付しようとするとき。
    modality: unspecified
    item_number: 1
  - id: kokuzei-tsuusoku-hou-art-34-3-p1-kou-2
    type: kou
    text: 二　電子情報処理組織を使用して行う納付受託者に対する通知で財務省令で定めるものに基づき納付しようとするとき。
    modality: unspecified
    item_number: 2
- number: 2
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kokuzei-tsuusoku-hou-art-34-3-p2-hashira
    type: hashira
    text: 次の各号に掲げるときは、当該各号に定める日に当該各号に規定する国税の納付があつたものとみなして、延納、物納及び附帯税に関する規定を適用する。
    modality: unspecified
  - id: kokuzei-tsuusoku-hou-art-34-3-p2-kou-1
    type: kou
    text: 一　国税を納付しようとする者が、前項第一号の納付書を添えて、納付受託者に納付しようとする税額に相当する金銭の交付をしたとき　当該交付をした日
    modality: unspecified
    item_number: 1
  - id: kokuzei-tsuusoku-hou-art-34-3-p2-kou-2
    type: kou
    text: 二　国税を納付しようとする者が前項第二号の通知に基づき当該国税を納付しようとする場合において、納付受託者が当該国税を納付しようとする者の委託を受けたとき　当該委託を受けた日
    modality: unspecified
    item_number: 2
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
parent_section:
  shou: 3
  shou_name_ja: 第三章　国税の納付及び徴収
  setsu: 1
  setsu_name_ja: 第一節　国税の納付
---

# 国税通則法 第34-3条(（納付受託者に対する納付の委託）)

## 原文 (日本語)

### 第三十四条の三第一項

国税を納付しようとする者は、その税額が財務省令で定める金額以下である場合であつて、次の各号のいずれかに該当するときは、納付受託者（次条第一項に規定する納付受託者をいう。以下この条において同じ。）に納付を委託することができる。

一　第三十四条第一項（納付の手続）に規定する納付書で財務省令で定めるものに基づき納付しようとするとき。

二　電子情報処理組織を使用して行う納付受託者に対する通知で財務省令で定めるものに基づき納付しようとするとき。

### 第三十四条の三第二項

次の各号に掲げるときは、当該各号に定める日に当該各号に規定する国税の納付があつたものとみなして、延納、物納及び附帯税に関する規定を適用する。

一　国税を納付しようとする者が、前項第一号の納付書を添えて、納付受託者に納付しようとする税額に相当する金銭の交付をしたとき　当該交付をした日

二　国税を納付しようとする者が前項第二号の通知に基づき当該国税を納付しようとする場合において、納付受託者が当該国税を納付しようとする者の委託を受けたとき　当該委託を受けた日
