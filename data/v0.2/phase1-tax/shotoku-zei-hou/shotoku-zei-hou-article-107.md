---
law_id: 340AC0000000033
law_name_ja: 所得税法
law_name_en: (English name pending)
article_number: '107'
article_id: shotoku-zei-hou-art-107
version_date: '1965-03-31'
source_url: https://laws.e-gov.go.jp/law/340AC0000000033
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: shotoku-zei-hou-art-107-p1-hashira
    type: hashira
    text: 次に掲げる居住者は、予定納税基準額が十五万円以上である場合には、第二期において、その予定納税基準額の二分の一に相当する金額の所得税を国に納付しなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: shotoku-zei-hou-art-107-p2
    type: simple
    text: 国税通則法第十一条（災害等による期限の延長）の規定による納付に関する期限の延長（以下この項において「期限延長」という。）により、前項に規定する居住者が同項の規定により第二期において納付すべき予定納税額の納期限がその年十二月三十一日後となる場合は、当該期限延長に係る予定納税額は、ないものとする。
    modality: gimu
    references:
    - 同項
    - 前項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: shotoku-zei-hou-art-107-p3
    type: simple
    text: 第一項の場合において、同項に規定する予定納税基準額の二分の一に相当する金額に百円未満の端数があるときは、その端数を切り捨てる。
    modality: unspecified
    references:
    - 同項
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　居住者の納税義務
  shou: 5
  shou_name_ja: 第五章　申告、納付及び還付
  setsu: 1
  setsu_name_ja: 第一節　予定納税
  kan: 2
  kan_name_ja: 第二款　特別農業所得者の予定納税の特例
---

# 所得税法 第107条(（特別農業所得者の予定納税額の納付）)

## 原文 (日本語)

### 第百七条第一項

<!-- segment: hashira id: shotoku-zei-hou-art-107-p1-hashira -->
次に掲げる居住者は、予定納税基準額が十五万円以上である場合には、第二期において、その予定納税基準額の二分の一に相当する金額の所得税を国に納付しなければならない。

### 第百七条第二項

<!-- segment: simple id: shotoku-zei-hou-art-107-p2 -->
国税通則法第十一条（災害等による期限の延長）の規定による納付に関する期限の延長（以下この項において「期限延長」という。）により、前項に規定する居住者が同項の規定により第二期において納付すべき予定納税額の納期限がその年十二月三十一日後となる場合は、当該期限延長に係る予定納税額は、ないものとする。

### 第百七条第三項

<!-- segment: simple id: shotoku-zei-hou-art-107-p3 -->
第一項の場合において、同項に規定する予定納税基準額の二分の一に相当する金額に百円未満の端数があるときは、その端数を切り捨てる。
