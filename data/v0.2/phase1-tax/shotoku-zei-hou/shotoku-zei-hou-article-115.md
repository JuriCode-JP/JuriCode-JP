---
law_id: 340AC0000000033
law_name_ja: 所得税法
law_name_en: (English name pending)
article_number: '115'
article_id: shotoku-zei-hou-art-115
version_date: '1965-03-31'
source_url: https://laws.e-gov.go.jp/law/340AC0000000033
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: shotoku-zei-hou-art-115-p1
    type: simple
    text: 第百四条第一項（予定納税額の納付）又は第百七条第一項（特別農業所得者の予定納税額の納付）の規定により予定納税額を納付すべき居住者は、これらの規定に規定する納期限前に出国をする場合には、これらの規定にかかわらず、その出国後に当該納期限の到来する予定納税額に相当する所得税を、その出国の時までに国に納付しなければならない。
    modality: gimu
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　居住者の納税義務
  shou: 5
  shou_name_ja: 第五章　申告、納付及び還付
  setsu: 1
  setsu_name_ja: 第一節　予定納税
  kan: 4
  kan_name_ja: 第四款　予定納税額の納付及び徴収に関する特例
---

# 所得税法 第115条(（出国をする場合の予定納税額の納期限の特例）)

## 原文 (日本語)

### 第百十五条

<!-- segment: simple id: shotoku-zei-hou-art-115-p1 -->
第百四条第一項（予定納税額の納付）又は第百七条第一項（特別農業所得者の予定納税額の納付）の規定により予定納税額を納付すべき居住者は、これらの規定に規定する納期限前に出国をする場合には、これらの規定にかかわらず、その出国後に当該納期限の到来する予定納税額に相当する所得税を、その出国の時までに国に納付しなければならない。
