---
law_id: 340CO0000000097
law_name_ja: 法人税法施行令
law_name_en: (English name pending)
article_number: 119-8
article_id: houjin-zei-hou-shikkourei-art-119-8
version_date: '1965-03-31'
source_url: https://laws.e-gov.go.jp/law/340CO0000000097
source_format: e-gov-xml
last_verified: '2026-06-01'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: houjin-zei-hou-shikkourei-art-119-8-p1
    type: simple
    text: 法第六十一条の二第四項（有価証券の譲渡益又は譲渡損の益金又は損金算入）に規定する政令で定めるところにより計算した金額は、同項に規定する所有株式を発行した法人の行つた分割型分割の直前の当該所有株式の帳簿価額に当該分割型分割に係る第二十三条第一項第二号（所有株式に対応する資本金等の額の計算方法等）に規定する割合を乗じて計算した金額とする。
    modality: gimu
    references:
    - 同項
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: houjin-zei-hou-shikkourei-art-119-8-p2
    type: simple
    text: 前項に規定する所有株式を発行した法人は、分割型分割を行つた場合には、当該所有株式を有していた法人に対し、当該分割型分割に係る同項に規定する割合を通知しなければならない。
    modality: gimu
    references:
    - 前項
    - 同項
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　内国法人の法人税
  shou: 1
  shou_name_ja: 第一章　各事業年度の所得に対する法人税
  setsu: 1
  setsu_name_ja: 第一節　各事業年度の所得の金額の計算
  kan: 22
  kan_name_ja: 第二款の二　利益の額又は損失の額の計算
  moku: 12
  moku_name_ja: 第一目の二　有価証券の一単位当たりの帳簿価額及び時価評価金額
---

# 法人税法施行令 第119-8条(（分割型分割の場合の譲渡対価の額及び譲渡原価の額等）)

## 原文 (日本語)

### 第百十九条の八第一項

<!-- segment: simple id: houjin-zei-hou-shikkourei-art-119-8-p1 -->
法第六十一条の二第四項（有価証券の譲渡益又は譲渡損の益金又は損金算入）に規定する政令で定めるところにより計算した金額は、同項に規定する所有株式を発行した法人の行つた分割型分割の直前の当該所有株式の帳簿価額に当該分割型分割に係る第二十三条第一項第二号（所有株式に対応する資本金等の額の計算方法等）に規定する割合を乗じて計算した金額とする。

### 第百十九条の八第二項

<!-- segment: simple id: houjin-zei-hou-shikkourei-art-119-8-p2 -->
前項に規定する所有株式を発行した法人は、分割型分割を行つた場合には、当該所有株式を有していた法人に対し、当該分割型分割に係る同項に規定する割合を通知しなければならない。
