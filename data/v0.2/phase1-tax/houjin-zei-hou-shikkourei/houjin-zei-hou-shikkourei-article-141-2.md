---
law_id: 340CO0000000097
law_name_ja: 法人税法施行令
law_name_en: (English name pending)
article_number: 141-2
article_id: houjin-zei-hou-shikkourei-art-141-2
version_date: '1965-03-31'
source_url: https://laws.e-gov.go.jp/law/340CO0000000097
source_format: e-gov-xml
last_verified: '2026-06-01'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: houjin-zei-hou-shikkourei-art-141-2-p1-hashira
    type: hashira
    text: 法第六十九条第一項（外国税額の控除）に規定する政令で定める金額は、内国法人の各事業年度の次に掲げる国外源泉所得（同項に規定する国外源泉所得をいう。以下この款において同じ。）に係る所得の金額の合計額（当該合計額が零を下回る場合には、零）とする。
    modality: gimu
    references:
    - 同項
cases: []
amendments: []
tags:
- phase1-tax
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　内国法人の法人税
  shou: 1
  shou_name_ja: 第一章　各事業年度の所得に対する法人税
  setsu: 2
  setsu_name_ja: 第二節　税額の計算
  kan: 2
  kan_name_ja: 第二款　税額控除
---

# 法人税法施行令 第141-2条(（国外所得金額）)

## 原文 (日本語)

### 第百四十一条の二

<!-- segment: hashira id: houjin-zei-hou-shikkourei-art-141-2-p1-hashira -->
法第六十九条第一項（外国税額の控除）に規定する政令で定める金額は、内国法人の各事業年度の次に掲げる国外源泉所得（同項に規定する国外源泉所得をいう。以下この款において同じ。）に係る所得の金額の合計額（当該合計額が零を下回る場合には、零）とする。
