...
- number: 2
  ...
  - id: houjin-zei-hou-shikkourei-art-201-2-p2-kou-1
    type: kou
    text: 集団投資信託（合同運用信託、所得税法第二条第一項第十五号（定義）に規定する公社債投資信託及び同項第十五号の二に規定する公社債等運用投資信託（第百四十条の二第一項第一号（法人税額から控除する所得税額の計算）に規定する特定公社債等運用投資信託を除く。）を除く。）の収益の分配に係る分配時調整外国税相当額　その元本を所有していた期間に対応するものとして計算される分配時調整外国税相当額
    modality: unspecified
    item_number: 1
  - id: houjin-zei-hou-shikkourei-art-201-2-p2-kou-2
    type: kou
    text: 前号に掲げるもの以外の分配時調整外国税相当額　その分配時調整外国税相当額の全額
    modality: unspecified
    item_number: 2
- number: 3
  has_proviso: false
