---
law_id: 129AC0000000089
law_name_ja: 民法
law_name_en: Civil Code
article_number: '496'
article_id: minpou-art-496
version_date: '1896-04-27'
source_url: https://laws.e-gov.go.jp/law/129AC0000000089
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-496-p1-zen
    type: zen_dan
    text: 債権者が供託を受諾せず、又は供託を有効と宣告した判決が確定しない間は、弁済者は、供託物を取り戻すことができる。
    modality: kanou_kenri
  - id: minpou-art-496-p1-kou
    type: kou_dan
    text: この場合においては、供託をしなかったものとみなす。
    modality: unspecified
    depends_on: minpou-art-496-p1-zen
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-496-p2
    type: simple
    text: 前項の規定は、供託によって質権又は抵当権が消滅した場合には、適用しない。
    modality: jogai
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-practitioner
- auto-generated
parent_section:
  hen: 3
  hen_name_ja: 第三編　債権
  shou: 1
  shou_name_ja: 第一章　総則
  setsu: 6
  setsu_name_ja: 第六節　債権の消滅
  kan: 1
  kan_name_ja: 第一款　弁済
  moku: 2
  moku_name_ja: 第二目　弁済の目的物の供託
---

# 民法 第496条(（供託物の取戻し）)

## 原文 (日本語)

### 第四百九十六条第一項

債権者が供託を受諾せず、又は供託を有効と宣告した判決が確定しない間は、弁済者は、供託物を取り戻すことができる。この場合においては、供託をしなかったものとみなす。

### 第四百九十六条第二項

前項の規定は、供託によって質権又は抵当権が消滅した場合には、適用しない。
