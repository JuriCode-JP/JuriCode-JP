---
law_id: 129AC0000000089
law_name_ja: 民法
law_name_en: Civil Code
article_number: '1045'
article_id: minpou-art-1045
version_date: '1896-04-27'
source_url: https://laws.e-gov.go.jp/law/129AC0000000089
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-1045-p1
    type: simple
    text: 負担付贈与がされた場合における第千四十三条第一項に規定する贈与した財産の価額は、その目的の価額から負担の価額を控除した額とする。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-1045-p2
    type: simple
    text: 不相当な対価をもってした有償行為は、当事者双方が遺留分権利者に損害を加えることを知ってしたものに限り、当該対価を負担の価額とする負担付贈与とみなす。
    modality: unspecified
cases: []
amendments: []
tags:
- phase1-practitioner
- auto-generated
parent_section:
  hen: 5
  hen_name_ja: 第五編　相続
  shou: 9
  shou_name_ja: 第九章　遺留分
---

# 民法 第1045条

## 原文 (日本語)

### 第千四十五条第一項

負担付贈与がされた場合における第千四十三条第一項に規定する贈与した財産の価額は、その目的の価額から負担の価額を控除した額とする。

### 第千四十五条第二項

不相当な対価をもってした有償行為は、当事者双方が遺留分権利者に損害を加えることを知ってしたものに限り、当該対価を負担の価額とする負担付贈与とみなす。
