---
law_id: 129AC0000000089
law_name_ja: 民法
law_name_en: Civil Code
article_number: '272'
article_id: minpou-art-272
version_date: '1896-04-27'
source_url: https://laws.e-gov.go.jp/law/129AC0000000089
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: true
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-272-p1-honbun
    type: honbun
    text: 永小作人は、その権利を他人に譲り渡し、又はその権利の存続期間内において耕作若しくは牧畜のため土地を賃貸することができる。
    modality: kanou_kenri
  - id: minpou-art-272-p1-tadashi
    type: tadashi
    text: ただし、設定行為で禁じたときは、この限りでない。
    modality: jogai
cases: []
amendments: []
tags:
- phase1-practitioner
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　物権
  shou: 5
  shou_name_ja: 第五章　永小作権
---

# 民法 第272条(（永小作権の譲渡又は土地の賃貸）)

## 原文 (日本語)

### 第二百七十二条

永小作人は、その権利を他人に譲り渡し、又はその権利の存続期間内において耕作若しくは牧畜のため土地を賃貸することができる。ただし、設定行為で禁じたときは、この限りでない。
