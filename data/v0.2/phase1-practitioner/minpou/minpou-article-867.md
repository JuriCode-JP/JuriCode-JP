---
law_id: 129AC0000000089
law_name_ja: 民法
law_name_en: Civil Code
article_number: '867'
article_id: minpou-art-867
version_date: '1896-04-27'
source_url: https://laws.e-gov.go.jp/law/129AC0000000089
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-867-p1
    type: simple
    text: 未成年後見人は、未成年被後見人に代わって親権を行う。
    modality: unspecified
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-867-p2
    type: junyou
    text: 第八百五十三条から第八百五十七条まで及び第八百六十一条から前条までの規定は、前項の場合について準用する。
    modality: unspecified
    references:
    - 前条
    - 前項
cases: []
amendments: []
tags:
- phase1-practitioner
- auto-generated
parent_section:
  hen: 4
  hen_name_ja: 第四編　親族
  shou: 5
  shou_name_ja: 第五章　後見
  setsu: 3
  setsu_name_ja: 第三節　後見の事務
---

# 民法 第867条(（未成年被後見人に代わる親権の行使）)

## 原文 (日本語)

### 第八百六十七条第一項

未成年後見人は、未成年被後見人に代わって親権を行う。

### 第八百六十七条第二項

第八百五十三条から第八百五十七条まで及び第八百六十一条から前条までの規定は、前項の場合について準用する。
