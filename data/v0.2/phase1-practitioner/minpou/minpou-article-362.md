---
law_id: 129AC0000000089
law_name_ja: 民法
law_name_en: Civil Code
article_number: '362'
article_id: minpou-art-362
version_date: '1896-04-27'
source_url: https://laws.e-gov.go.jp/law/129AC0000000089
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-362-p1
    type: simple
    text: 質権は、財産権をその目的とすることができる。
    modality: kanou_kenri
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-362-p2
    type: simple
    text: 前項の質権については、この節に定めるもののほか、その性質に反しない限り、前三節（総則、動産質及び不動産質）の規定を準用する。
    modality: unspecified
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-practitioner
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　物権
  shou: 9
  shou_name_ja: 第九章　質権
  setsu: 4
  setsu_name_ja: 第四節　権利質
---

# 民法 第362条(（権利質の目的等）)

## 原文 (日本語)

### 第三百六十二条第一項

質権は、財産権をその目的とすることができる。

### 第三百六十二条第二項

前項の質権については、この節に定めるもののほか、その性質に反しない限り、前三節（総則、動産質及び不動産質）の規定を準用する。
