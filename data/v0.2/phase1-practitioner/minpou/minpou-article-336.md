---
law_id: 129AC0000000089
law_name_ja: 民法
law_name_en: Civil Code
article_number: '336'
article_id: minpou-art-336
version_date: '1896-04-27'
source_url: https://laws.e-gov.go.jp/law/129AC0000000089
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: true
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-336-p1-honbun
    type: honbun
    text: 一般の先取特権は、不動産について登記をしなくても、特別担保を有しない債権者に対抗することができる。
    modality: kanou_kenri
  - id: minpou-art-336-p1-tadashi
    type: tadashi
    text: ただし、登記をした第三者に対しては、この限りでない。
    modality: jogai
cases: []
amendments: []
tags:
- phase1-practitioner
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　物権
  shou: 8
  shou_name_ja: 第八章　先取特権
  setsu: 4
  setsu_name_ja: 第四節　先取特権の効力
---

# 民法 第336条(（一般の先取特権の対抗力）)

## 原文 (日本語)

### 第三百三十六条

一般の先取特権は、不動産について登記をしなくても、特別担保を有しない債権者に対抗することができる。ただし、登記をした第三者に対しては、この限りでない。
