---
law_id: 129AC0000000089
law_name_ja: 民法
law_name_en: Civil Code
article_number: '918'
article_id: minpou-art-918
version_date: '1896-04-27'
source_url: https://laws.e-gov.go.jp/law/129AC0000000089
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: true
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-918-p1-honbun
    type: honbun
    text: 相続人は、その固有財産におけるのと同一の注意をもって、相続財産を管理しなければならない。
    modality: gimu
  - id: minpou-art-918-p1-tadashi
    type: tadashi
    text: ただし、相続の承認又は放棄をしたときは、この限りでない。
    modality: jogai
cases: []
amendments: []
tags:
- phase1-practitioner
- auto-generated
parent_section:
  hen: 5
  hen_name_ja: 第五編　相続
  shou: 4
  shou_name_ja: 第四章　相続の承認及び放棄
  setsu: 1
  setsu_name_ja: 第一節　総則
---

# 民法 第918条(（相続人による管理）)

## 原文 (日本語)

### 第九百十八条

相続人は、その固有財産におけるのと同一の注意をもって、相続財産を管理しなければならない。ただし、相続の承認又は放棄をしたときは、この限りでない。
