---
law_id: 129AC0000000089
law_name_ja: 民法
law_name_en: Civil Code
article_number: '775'
article_id: minpou-art-775
version_date: '1896-04-27'
source_url: https://laws.e-gov.go.jp/law/129AC0000000089
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: minpou-art-775-p1-hashira
    type: hashira
    text: 次の各号に掲げる否認権は、それぞれ当該各号に定める者に対する嫡出否認の訴えによって行う。
    modality: unspecified
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-775-p2
    type: simple
    text: 前項第一号又は第四号に掲げる否認権を親権を行う母に対し行使しようとする場合において、親権を行う母がないときは、家庭裁判所は、特別代理人を選任しなければならない。
    modality: gimu
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-practitioner
- auto-generated
parent_section:
  hen: 4
  hen_name_ja: 第四編　親族
  shou: 3
  shou_name_ja: 第三章　親子
  setsu: 1
  setsu_name_ja: 第一節　実子
---

# 民法 第775条(（嫡出否認の訴え）)

## 原文 (日本語)

### 第七百七十五条第一項

<!-- segment: hashira id: minpou-art-775-p1-hashira -->
次の各号に掲げる否認権は、それぞれ当該各号に定める者に対する嫡出否認の訴えによって行う。

### 第七百七十五条第二項

<!-- segment: simple id: minpou-art-775-p2 -->
前項第一号又は第四号に掲げる否認権を親権を行う母に対し行使しようとする場合において、親権を行う母がないときは、家庭裁判所は、特別代理人を選任しなければならない。
