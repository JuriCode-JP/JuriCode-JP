---
law_id: 129AC0000000089
law_name_ja: 民法
law_name_en: Civil Code
article_number: '658'
article_id: minpou-art-658
version_date: '1896-04-27'
source_url: https://laws.e-gov.go.jp/law/129AC0000000089
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-658-p1
    type: simple
    text: 受寄者は、寄託者の承諾を得なければ、寄託物を使用することができない。
    modality: kanou_negative
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-658-p2
    type: simple
    text: 受寄者は、寄託者の承諾を得たとき、又はやむを得ない事由があるときでなければ、寄託物を第三者に保管させることができない。
    modality: kanou_negative
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-658-p3
    type: simple
    text: 再受寄者は、寄託者に対して、その権限の範囲内において、受寄者と同一の権利を有し、義務を負う。
    modality: unspecified
cases: []
amendments: []
tags:
- phase1-practitioner
- auto-generated
parent_section:
  hen: 3
  hen_name_ja: 第三編　債権
  shou: 2
  shou_name_ja: 第二章　契約
  setsu: 11
  setsu_name_ja: 第十一節　寄託
---

# 民法 第658条(（寄託物の使用及び第三者による保管）)

## 原文 (日本語)

### 第六百五十八条第一項

受寄者は、寄託者の承諾を得なければ、寄託物を使用することができない。

### 第六百五十八条第二項

受寄者は、寄託者の承諾を得たとき、又はやむを得ない事由があるときでなければ、寄託物を第三者に保管させることができない。

### 第六百五十八条第三項

再受寄者は、寄託者に対して、その権限の範囲内において、受寄者と同一の権利を有し、義務を負う。
