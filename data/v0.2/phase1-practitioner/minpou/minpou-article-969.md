---
law_id: 129AC0000000089
law_name_ja: 民法
law_name_en: Civil Code
article_number: '969'
article_id: minpou-art-969
version_date: '1896-04-27'
source_url: https://laws.e-gov.go.jp/law/129AC0000000089
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: minpou-art-969-p1-hashira
    type: hashira
    text: 公正証書によって遺言をするには、次に掲げる方式に従わなければならない。
    modality: gimu
  - id: minpou-art-969-p1-kou-1
    type: kou
    text: 一　証人二人以上の立会いがあること。
    modality: unspecified
    item_number: 1
  - id: minpou-art-969-p1-kou-2
    type: kou
    text: 二　遺言者が遺言の趣旨を公証人に口授すること。
    modality: unspecified
    item_number: 2
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-969-p2
    type: simple
    text: 前項の公正証書は、公証人法（明治四十一年法律第五十三号）の定めるところにより作成するものとする。
    modality: gimu
    references:
    - 前項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: minpou-art-969-p3
    type: simple
    text: 第一項第一号の証人については、公証人法第三十条に規定する証人とみなして、同法の規定（同法第三十五条第三項の規定を除く。）を適用する。
    modality: unspecified
cases: []
amendments: []
tags:
- phase1-practitioner
- auto-generated
parent_section:
  hen: 5
  hen_name_ja: 第五編　相続
  shou: 7
  shou_name_ja: 第七章　遺言
  setsu: 2
  setsu_name_ja: 第二節　遺言の方式
  kan: 1
  kan_name_ja: 第一款　普通の方式
---

# 民法 第969条(（公正証書遺言）)

## 原文 (日本語)

### 第九百六十九条第一項

公正証書によって遺言をするには、次に掲げる方式に従わなければならない。

一　証人二人以上の立会いがあること。

二　遺言者が遺言の趣旨を公証人に口授すること。

### 第九百六十九条第二項

前項の公正証書は、公証人法（明治四十一年法律第五十三号）の定めるところにより作成するものとする。

### 第九百六十九条第三項

第一項第一号の証人については、公証人法第三十条に規定する証人とみなして、同法の規定（同法第三十五条第三項の規定を除く。）を適用する。
