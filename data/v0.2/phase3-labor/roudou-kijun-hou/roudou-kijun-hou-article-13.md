---
law_id: 322AC0000000049
law_name_ja: 労働基準法
law_name_en: (English name pending)
article_number: '13'
article_id: roudou-kijun-hou-art-13
version_date: '1947-04-07'
source_url: https://laws.e-gov.go.jp/law/322AC0000000049
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: roudou-kijun-hou-art-13-p1-zen
    type: zen_dan
    text: この法律で定める基準に達しない労働条件を定める労働契約は、その部分については無効とする。
    modality: koka_mukou
  - id: roudou-kijun-hou-art-13-p1-kou
    type: kou_dan
    text: この場合において、無効となつた部分は、この法律で定める基準による。
    modality: unspecified
    depends_on: roudou-kijun-hou-art-13-p1-zen
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  shou: 2
  shou_name_ja: 第二章　労働契約
---

# 労働基準法 第13条(（この法律違反の契約）)

## 原文 (日本語)

### 第十三条

<!-- segment: zen_dan id: roudou-kijun-hou-art-13-p1-zen -->
この法律で定める基準に達しない労働条件を定める労働契約は、その部分については無効とする。<!-- segment: kou_dan id: roudou-kijun-hou-art-13-p1-kou depends_on: roudou-kijun-hou-art-13-p1-zen -->
この場合において、無効となつた部分は、この法律で定める基準による。
