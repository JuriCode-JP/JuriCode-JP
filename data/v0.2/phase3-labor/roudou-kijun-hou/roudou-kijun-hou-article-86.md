---
law_id: 322AC0000000049
law_name_ja: 労働基準法
law_name_en: (English name pending)
article_number: '86'
article_id: roudou-kijun-hou-art-86
version_date: '1947-04-07'
source_url: https://laws.e-gov.go.jp/law/322AC0000000049
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: roudou-kijun-hou-art-86-p1
    type: simple
    text: 前条の規定による審査及び仲裁の結果に不服のある者は、労働者災害補償保険審査官の審査又は仲裁を申し立てることができる。
    modality: kanou_kenri
    references:
    - 前条
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: roudou-kijun-hou-art-86-p2
    type: simple
    text: 前条第三項の規定は、前項の規定により審査又は仲裁の申立てがあつた場合に、これを準用する。
    modality: unspecified
    references:
    - 前条
    - 前項
cases: []
amendments: []
tags:
- phase3-labor
- auto-generated
parent_section:
  shou: 8
  shou_name_ja: 第八章　災害補償
---

# 労働基準法 第86条

## 原文 (日本語)

### 第八十六条第一項

<!-- segment: simple id: roudou-kijun-hou-art-86-p1 -->
前条の規定による審査及び仲裁の結果に不服のある者は、労働者災害補償保険審査官の審査又は仲裁を申し立てることができる。

### 第八十六条第二項

<!-- segment: simple id: roudou-kijun-hou-art-86-p2 -->
前条第三項の規定は、前項の規定により審査又は仲裁の申立てがあつた場合に、これを準用する。
