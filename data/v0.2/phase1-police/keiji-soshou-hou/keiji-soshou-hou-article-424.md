---
law_id: 323AC0000000131
law_name_ja: 刑事訴訟法
law_name_en: Code of Criminal Procedure
article_number: '424'
article_id: keiji-soshou-hou-art-424
version_date: '1948-07-10'
source_url: https://laws.e-gov.go.jp/law/323AC0000000131
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: true
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-424-p1
    type: simple
    text: 抗告は、即時抗告を除いては、裁判の執行を停止する効力を有しない。但し、原裁判所は、決定で、抗告の裁判があるまで執行を停止することができる。
    modality: kanou_kenri
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-424-p2
    type: simple
    text: 抗告裁判所は、決定で裁判の執行を停止することができる。
    modality: kanou_kenri
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 3
  hen_name_ja: 第三編　上訴
  shou: 4
  shou_name_ja: 第四章　抗告
---

# 刑事訴訟法 第424条

## 原文 (日本語)

### 第四百二十四条第一項

抗告は、即時抗告を除いては、裁判の執行を停止する効力を有しない。但し、原裁判所は、決定で、抗告の裁判があるまで執行を停止することができる。

### 第四百二十四条第二項

抗告裁判所は、決定で裁判の執行を停止することができる。
