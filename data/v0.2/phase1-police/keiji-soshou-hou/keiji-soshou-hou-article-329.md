---
law_id: 323AC0000000131
law_name_ja: 刑事訴訟法
law_name_en: Code of Criminal Procedure
article_number: '329'
article_id: keiji-soshou-hou-art-329
version_date: '1948-07-10'
source_url: https://laws.e-gov.go.jp/law/323AC0000000131
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-329-p1
    type: simple
    text: 被告事件が裁判所の管轄に属しないときは、判決で管轄違の言渡をしなければならない。但し、第二百六十六条第二号の規定により地方裁判所の審判に付された事件については、管轄違の言渡をすることはできない。
    modality: kanou_negative
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　第一審
  shou: 3
  shou_name_ja: 第三章　公判
  setsu: 5
  setsu_name_ja: 第五節　公判の裁判
---

# 刑事訴訟法 第329条

## 原文 (日本語)

### 第三百二十九条

<!-- segment: simple id: keiji-soshou-hou-art-329-p1 -->
被告事件が裁判所の管轄に属しないときは、判決で管轄違の言渡をしなければならない。但し、第二百六十六条第二号の規定により地方裁判所の審判に付された事件については、管轄違の言渡をすることはできない。
