---
law_id: 323AC0000000131
law_name_ja: 刑事訴訟法
law_name_en: Code of Criminal Procedure
article_number: '403'
article_id: keiji-soshou-hou-art-403
version_date: '1948-07-10'
source_url: https://laws.e-gov.go.jp/law/323AC0000000131
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-403-p1
    type: simple
    text: 原裁判所が不法に公訴棄却の決定をしなかつたときは、決定で公訴を棄却しなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-403-p2
    type: simple
    text: 第三百八十五条第二項の規定は、前項の決定についてこれを準用する。
    modality: unspecified
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 3
  hen_name_ja: 第三編　上訴
  shou: 2
  shou_name_ja: 第二章　控訴
---

# 刑事訴訟法 第403条

## 原文 (日本語)

### 第四百三条第一項

原裁判所が不法に公訴棄却の決定をしなかつたときは、決定で公訴を棄却しなければならない。

### 第四百三条第二項

第三百八十五条第二項の規定は、前項の決定についてこれを準用する。
