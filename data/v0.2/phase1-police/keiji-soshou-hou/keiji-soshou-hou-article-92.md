---
law_id: 323AC0000000131
law_name_ja: 刑事訴訟法
law_name_en: Code of Criminal Procedure
article_number: '92'
article_id: keiji-soshou-hou-art-92
version_date: '1948-07-10'
source_url: https://laws.e-gov.go.jp/law/323AC0000000131
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-92-p1
    type: simple
    text: 裁判所は、保釈を許す決定又は保釈の請求を却下する決定をするには、検察官の意見を聴かなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-92-p2
    type: simple
    text: 検察官の請求による場合を除いて、勾留を取り消す決定をするときも、前項と同様である。但し、急速を要する場合は、この限りでない。
    modality: jogai
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 1
  hen_name_ja: 第一編　総則
  shou: 8
  shou_name_ja: 第八章　被告人の召喚、勾引及び勾留
---

# 刑事訴訟法 第92条

## 原文 (日本語)

### 第九十二条第一項

<!-- segment: simple id: keiji-soshou-hou-art-92-p1 -->
裁判所は、保釈を許す決定又は保釈の請求を却下する決定をするには、検察官の意見を聴かなければならない。

### 第九十二条第二項

<!-- segment: simple id: keiji-soshou-hou-art-92-p2 -->
検察官の請求による場合を除いて、勾留を取り消す決定をするときも、前項と同様である。但し、急速を要する場合は、この限りでない。
