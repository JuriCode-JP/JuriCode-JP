---
law_id: 323AC0000000131
law_name_ja: 刑事訴訟法
law_name_en: Code of Criminal Procedure
article_number: '241'
article_id: keiji-soshou-hou-art-241
version_date: '1948-07-10'
source_url: https://laws.e-gov.go.jp/law/323AC0000000131
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-241-p1
    type: simple
    text: 告訴又は告発は、書面又は口頭で検察官又は司法警察員にこれをしなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-241-p2
    type: simple
    text: 検察官又は司法警察員は、口頭による告訴又は告発を受けたときは調書を作らなければならない。
    modality: gimu
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　第一審
  shou: 1
  shou_name_ja: 第一章　捜査
---

# 刑事訴訟法 第241条

## 原文 (日本語)

### 第二百四十一条第一項

<!-- segment: simple id: keiji-soshou-hou-art-241-p1 -->
告訴又は告発は、書面又は口頭で検察官又は司法警察員にこれをしなければならない。

### 第二百四十一条第二項

<!-- segment: simple id: keiji-soshou-hou-art-241-p2 -->
検察官又は司法警察員は、口頭による告訴又は告発を受けたときは調書を作らなければならない。
