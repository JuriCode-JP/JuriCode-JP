---
law_id: 323AC0000000131
law_name_ja: 刑事訴訟法
law_name_en: Code of Criminal Procedure
article_number: '253'
article_id: keiji-soshou-hou-art-253
version_date: '1948-07-10'
source_url: https://laws.e-gov.go.jp/law/323AC0000000131
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-253-p1
    type: simple
    text: 時効は、犯罪行為が終つた時から進行する。
    modality: unspecified
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-253-p2
    type: simple
    text: 共犯の場合には、最終の行為が終つた時から、すべての共犯に対して時効の期間を起算する。
    modality: unspecified
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　第一審
  shou: 2
  shou_name_ja: 第二章　公訴
---

# 刑事訴訟法 第253条

## 原文 (日本語)

### 第二百五十三条第一項

時効は、犯罪行為が終つた時から進行する。

### 第二百五十三条第二項

共犯の場合には、最終の行為が終つた時から、すべての共犯に対して時効の期間を起算する。
