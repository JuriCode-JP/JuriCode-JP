---
law_id: 323AC0000000131
law_name_ja: 刑事訴訟法
law_name_en: Code of Criminal Procedure
article_number: '237'
article_id: keiji-soshou-hou-art-237
version_date: '1948-07-10'
source_url: https://laws.e-gov.go.jp/law/323AC0000000131
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-237-p1
    type: simple
    text: 告訴は、公訴の提起があるまでこれを取り消すことができる。
    modality: koka_torikeshi
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-237-p2
    type: simple
    text: 告訴の取消をした者は、更に告訴をすることができない。
    modality: kanou_negative
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-237-p3
    type: simple
    text: 前二項の規定は、請求を待つて受理すべき事件についての請求についてこれを準用する。
    modality: unspecified
    references:
    - 前二項
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　第一審
  shou: 1
  shou_name_ja: 第一章　捜査
---

# 刑事訴訟法 第237条

## 原文 (日本語)

### 第二百三十七条第一項

告訴は、公訴の提起があるまでこれを取り消すことができる。

### 第二百三十七条第二項

告訴の取消をした者は、更に告訴をすることができない。

### 第二百三十七条第三項

前二項の規定は、請求を待つて受理すべき事件についての請求についてこれを準用する。
