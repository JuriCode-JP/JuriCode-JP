---
law_id: 323AC0000000131
law_name_ja: 刑事訴訟法
law_name_en: Code of Criminal Procedure
article_number: '417'
article_id: keiji-soshou-hou-art-417
version_date: '1948-07-10'
source_url: https://laws.e-gov.go.jp/law/323AC0000000131
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-417-p1
    type: simple
    text: 上告裁判所は、訂正の判決をしないときは、速やかに決定で申立を棄却しなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-417-p2
    type: simple
    text: 訂正の判決に対しては、第四百十五条第一項の申立をすることはできない。
    modality: kanou_negative
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 3
  hen_name_ja: 第三編　上訴
  shou: 3
  shou_name_ja: 第三章　上告
---

# 刑事訴訟法 第417条

## 原文 (日本語)

### 第四百十七条第一項

上告裁判所は、訂正の判決をしないときは、速やかに決定で申立を棄却しなければならない。

### 第四百十七条第二項

訂正の判決に対しては、第四百十五条第一項の申立をすることはできない。
