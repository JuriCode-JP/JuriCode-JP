---
law_id: 323AC0000000131
law_name_ja: 刑事訴訟法
law_name_en: Code of Criminal Procedure
article_number: '347'
article_id: keiji-soshou-hou-art-347
version_date: '1948-07-10'
source_url: https://laws.e-gov.go.jp/law/323AC0000000131
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-347-p1
    type: simple
    text: 押収した贓物で被害者に還付すべき理由が明らかなものは、これを被害者に還付する言渡をしなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-347-p2
    type: simple
    text: 贓物の対価として得た物について、被害者から交付の請求があつたときは、前項の例による。
    modality: unspecified
    references:
    - 前項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-347-p3
    type: simple
    text: 仮に還付した物について、別段の言渡がないときは、還付の言渡があつたものとする。
    modality: gimu
- number: 4
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-347-p4
    type: simple
    text: 前三項の規定は、民事訴訟の手続に従い、利害関係人がその権利を主張することを妨げない。
    modality: jogai
    references:
    - 前三項
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　第一審
  shou: 3
  shou_name_ja: 第三章　公判
  setsu: 5
  setsu_name_ja: 第五節　公判の裁判
---

# 刑事訴訟法 第347条

## 原文 (日本語)

### 第三百四十七条第一項

<!-- segment: simple id: keiji-soshou-hou-art-347-p1 -->
押収した贓物で被害者に還付すべき理由が明らかなものは、これを被害者に還付する言渡をしなければならない。

### 第三百四十七条第二項

<!-- segment: simple id: keiji-soshou-hou-art-347-p2 -->
贓物の対価として得た物について、被害者から交付の請求があつたときは、前項の例による。

### 第三百四十七条第三項

<!-- segment: simple id: keiji-soshou-hou-art-347-p3 -->
仮に還付した物について、別段の言渡がないときは、還付の言渡があつたものとする。

### 第三百四十七条第四項

<!-- segment: simple id: keiji-soshou-hou-art-347-p4 -->
前三項の規定は、民事訴訟の手続に従い、利害関係人がその権利を主張することを妨げない。
