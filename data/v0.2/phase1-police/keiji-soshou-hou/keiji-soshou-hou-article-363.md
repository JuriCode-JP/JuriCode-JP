---
law_id: 323AC0000000131
law_name_ja: 刑事訴訟法
law_name_en: Code of Criminal Procedure
article_number: '363'
article_id: keiji-soshou-hou-art-363
version_date: '1948-07-10'
source_url: https://laws.e-gov.go.jp/law/323AC0000000131
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-363-p1
    type: simple
    text: 上訴権回復の請求は、事由が止んだ日から上訴の提起期間に相当する期間内にこれをしなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-363-p2
    type: simple
    text: 上訴権回復の請求をする者は、その請求と同時に上訴の申立をしなければならない。
    modality: gimu
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 3
  hen_name_ja: 第三編　上訴
  shou: 1
  shou_name_ja: 第一章　通則
---

# 刑事訴訟法 第363条

## 原文 (日本語)

### 第三百六十三条第一項

<!-- segment: simple id: keiji-soshou-hou-art-363-p1 -->
上訴権回復の請求は、事由が止んだ日から上訴の提起期間に相当する期間内にこれをしなければならない。

### 第三百六十三条第二項

<!-- segment: simple id: keiji-soshou-hou-art-363-p2 -->
上訴権回復の請求をする者は、その請求と同時に上訴の申立をしなければならない。
