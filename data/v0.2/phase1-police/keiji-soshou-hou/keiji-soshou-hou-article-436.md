---
law_id: 323AC0000000131
law_name_ja: 刑事訴訟法
law_name_en: Code of Criminal Procedure
article_number: '436'
article_id: keiji-soshou-hou-art-436
version_date: '1948-07-10'
source_url: https://laws.e-gov.go.jp/law/323AC0000000131
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-436-p1
    type: simple
    text: 再審の請求は、左の場合において、控訴又は上告を棄却した確定判決に対して、その言渡を受けた者の利益のために、これをすることができる。
    modality: kanou_kenri
  - id: keiji-soshou-hou-art-436-p1-kou-1
    type: kou
    text: 前条第一号又は第二号に規定する事由があるとき。
    modality: unspecified
    item_number: 1
  - id: keiji-soshou-hou-art-436-p1-kou-2
    type: kou
    text: 原判決又はその証拠となつた証拠書類の作成に関与した裁判官について前条第七号に規定する事由があるとき。
    modality: unspecified
    item_number: 2
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-436-p2
    type: simple
    text: 第一審の確定判決に対して再審の請求をした事件について再審の判決があつた後は、控訴棄却の判決に対しては、再審の請求をすることはできない。
    modality: kanou_negative
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-436-p3
    type: simple
    text: 第一審又は第二審の確定判決に対して再審の請求をした事件について再審の判決があつた後は、上告棄却の判決に対しては、再審の請求をすることはできない。
    modality: kanou_negative
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 4
  hen_name_ja: 第四編　再審
---

# 刑事訴訟法 第436条

## 原文 (日本語)

### 第四百三十六条第一項

再審の請求は、左の場合において、控訴又は上告を棄却した確定判決に対して、その言渡を受けた者の利益のために、これをすることができる。

一　前条第一号又は第二号に規定する事由があるとき。

二　原判決又はその証拠となつた証拠書類の作成に関与した裁判官について前条第七号に規定する事由があるとき。

### 第四百三十六条第二項

第一審の確定判決に対して再審の請求をした事件について再審の判決があつた後は、控訴棄却の判決に対しては、再審の請求をすることはできない。

### 第四百三十六条第三項

第一審又は第二審の確定判決に対して再審の請求をした事件について再審の判決があつた後は、上告棄却の判決に対しては、再審の請求をすることはできない。
