---
law_id: 323AC0000000131
law_name_ja: 刑事訴訟法
law_name_en: Code of Criminal Procedure
article_number: '333'
article_id: keiji-soshou-hou-art-333
version_date: '1948-07-10'
source_url: https://laws.e-gov.go.jp/law/323AC0000000131
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-333-p1
    type: simple
    text: 被告事件について犯罪の証明があつたときは、第三百三十四条の場合を除いては、判決で刑の言渡をしなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-333-p2
    type: simple
    text: 刑の執行猶予は、刑の言渡しと同時に、判決でその言渡しをしなければならない。猶予の期間中保護観察に付する場合も、同様とする。
    modality: gimu
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　第一審
  shou: 3
  shou_name_ja: 第三章　公判
  setsu: 5
  setsu_name_ja: 第五節　公判の裁判
---

# 刑事訴訟法 第333条

## 原文 (日本語)

### 第三百三十三条第一項

被告事件について犯罪の証明があつたときは、第三百三十四条の場合を除いては、判決で刑の言渡をしなければならない。

### 第三百三十三条第二項

刑の執行猶予は、刑の言渡しと同時に、判決でその言渡しをしなければならない。猶予の期間中保護観察に付する場合も、同様とする。
