---
law_id: 323AC0000000131
law_name_ja: 刑事訴訟法
law_name_en: Code of Criminal Procedure
article_number: '381'
article_id: keiji-soshou-hou-art-381
version_date: '1948-07-10'
source_url: https://laws.e-gov.go.jp/law/323AC0000000131
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keiji-soshou-hou-art-381-p1
    type: simple
    text: 刑の量定が不当であることを理由として控訴の申立をした場合には、控訴趣意書に、訴訟記録及び原裁判所において取り調べた証拠に現われている事実であつて刑の量定が不当であることを信ずるに足りるものを援用しなければならない。
    modality: gimu
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 3
  hen_name_ja: 第三編　上訴
  shou: 2
  shou_name_ja: 第二章　控訴
---

# 刑事訴訟法 第381条

## 原文 (日本語)

### 第三百八十一条

<!-- segment: simple id: keiji-soshou-hou-art-381-p1 -->
刑の量定が不当であることを理由として控訴の申立をした場合には、控訴趣意書に、訴訟記録及び原裁判所において取り調べた証拠に現われている事実であつて刑の量定が不当であることを信ずるに足りるものを援用しなければならない。
