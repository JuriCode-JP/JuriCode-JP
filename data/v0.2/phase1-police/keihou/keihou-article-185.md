---
law_id: 140AC0000000045
law_name_ja: 刑法
law_name_en: Penal Code
article_number: '185'
article_id: keihou-art-185
version_date: '1907-04-24'
source_url: https://laws.e-gov.go.jp/law/140AC0000000045
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: true
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keihou-art-185-p1-honbun
    type: honbun
    text: 賭と博をした者は、五十万円以下の罰金又は科料に処する。
    modality: gimu_kei
  - id: keihou-art-185-p1-tadashi
    type: tadashi
    text: ただし、一時の娯楽に供する物を賭かけたにとどまるときは、この限りでない。
    modality: jogai
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　罪
  shou: 23
  shou_name_ja: 第二十三章　賭と博及び富くじに関する罪
---

# 刑法 第185条(（賭と博）)

## 原文 (日本語)

### 第百八十五条

賭と博をした者は、五十万円以下の罰金又は科料に処する。ただし、一時の娯楽に供する物を賭かけたにとどまるときは、この限りでない。
