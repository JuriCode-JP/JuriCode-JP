---
law_id: 140AC0000000045
law_name_ja: 刑法
law_name_en: Penal Code
article_number: '56'
article_id: keihou-art-56
version_date: '1907-04-24'
source_url: https://laws.e-gov.go.jp/law/140AC0000000045
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keihou-art-56-p1
    type: simple
    text: 拘禁刑に処せられた者がその執行を終わった日又はその執行の免除を得た日から五年以内に更に罪を犯した場合において、その者を有期拘禁刑に処するときは、再犯とする。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keihou-art-56-p2
    type: simple
    text: 死刑に処せられた者がその執行の免除を得た日又は減刑により拘禁刑に減軽されてその執行を終わった日若しくはその執行の免除を得た日から五年以内に更に罪を犯した場合において、その者を有期拘禁刑に処するときも、前項と同様とする。
    modality: gimu
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 1
  hen_name_ja: 第一編　総則
  shou: 10
  shou_name_ja: 第十章　累犯
---

# 刑法 第56条(（再犯）)

## 原文 (日本語)

### 第五十六条第一項

<!-- segment: simple id: keihou-art-56-p1 -->
拘禁刑に処せられた者がその執行を終わった日又はその執行の免除を得た日から五年以内に更に罪を犯した場合において、その者を有期拘禁刑に処するときは、再犯とする。

### 第五十六条第二項

<!-- segment: simple id: keihou-art-56-p2 -->
死刑に処せられた者がその執行の免除を得た日又は減刑により拘禁刑に減軽されてその執行を終わった日若しくはその執行の免除を得た日から五年以内に更に罪を犯した場合において、その者を有期拘禁刑に処するときも、前項と同様とする。
