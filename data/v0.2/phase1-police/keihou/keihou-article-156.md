---
law_id: 140AC0000000045
law_name_ja: 刑法
law_name_en: Penal Code
article_number: '156'
article_id: keihou-art-156
version_date: '1907-04-24'
source_url: https://laws.e-gov.go.jp/law/140AC0000000045
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keihou-art-156-p1
    type: simple
    text: 公務員が、その職務に関し、行使の目的で、虚偽の文書等若しくは電磁的記録文書等を作成し、又は文書等若しくは電磁的記録文書等を変造したときは、印章等又は電磁的記録印章等の有無により区別して、前二条の例による。
    modality: unspecified
    references:
    - 前二条
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　罪
  shou: 17
  shou_name_ja: 第十七章　文書偽造の罪
---

# 刑法 第156条(（虚偽公文書作成等）)

## 原文 (日本語)

### 第百五十六条

<!-- segment: simple id: keihou-art-156-p1 -->
公務員が、その職務に関し、行使の目的で、虚偽の文書等若しくは電磁的記録文書等を作成し、又は文書等若しくは電磁的記録文書等を変造したときは、印章等又は電磁的記録印章等の有無により区別して、前二条の例による。
