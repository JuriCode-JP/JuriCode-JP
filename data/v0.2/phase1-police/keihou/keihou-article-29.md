---
law_id: 140AC0000000045
law_name_ja: 刑法
law_name_en: Penal Code
article_number: '29'
article_id: keihou-art-29
version_date: '1907-04-24'
source_url: https://laws.e-gov.go.jp/law/140AC0000000045
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: keihou-art-29-p1-hashira
    type: hashira
    text: 次に掲げる場合においては、仮釈放の処分を取り消すことができる。
    modality: koka_torikeshi
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keihou-art-29-p2
    type: simple
    text: 刑の一部の執行猶予の言渡しを受け、その刑について仮釈放の処分を受けた場合において、当該仮釈放中に当該執行猶予の言渡しを取り消されたときは、その処分は、効力を失う。
    modality: unspecified
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keihou-art-29-p3
    type: simple
    text: 仮釈放の処分を取り消したとき、又は前項の規定により仮釈放の処分が効力を失ったときは、釈放中の日数は、刑期に算入しない。
    modality: unspecified
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 1
  hen_name_ja: 第一編　総則
  shou: 5
  shou_name_ja: 第五章　仮釈放
---

# 刑法 第29条(（仮釈放の取消し等）)

## 原文 (日本語)

### 第二十九条第一項

<!-- segment: hashira id: keihou-art-29-p1-hashira -->
次に掲げる場合においては、仮釈放の処分を取り消すことができる。

### 第二十九条第二項

<!-- segment: simple id: keihou-art-29-p2 -->
刑の一部の執行猶予の言渡しを受け、その刑について仮釈放の処分を受けた場合において、当該仮釈放中に当該執行猶予の言渡しを取り消されたときは、その処分は、効力を失う。

### 第二十九条第三項

<!-- segment: simple id: keihou-art-29-p3 -->
仮釈放の処分を取り消したとき、又は前項の規定により仮釈放の処分が効力を失ったときは、釈放中の日数は、刑期に算入しない。
