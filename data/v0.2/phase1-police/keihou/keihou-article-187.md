---
law_id: 140AC0000000045
law_name_ja: 刑法
law_name_en: Penal Code
article_number: '187'
article_id: keihou-art-187
version_date: '1907-04-24'
source_url: https://laws.e-gov.go.jp/law/140AC0000000045
source_format: e-gov-xml
last_verified: '2026-05-19'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keihou-art-187-p1
    type: simple
    text: 富くじを発売した者は、二年以下の拘禁刑又は百五十万円以下の罰金に処する。
    modality: gimu_kei
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keihou-art-187-p2
    type: simple
    text: 富くじ発売の取次ぎをした者は、一年以下の拘禁刑又は百万円以下の罰金に処する。
    modality: gimu_kei
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: keihou-art-187-p3
    type: simple
    text: 前二項に規定するもののほか、富くじを授受した者は、二十万円以下の罰金又は科料に処する。
    modality: gimu_kei
    references:
    - 前二項
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　罪
  shou: 23
  shou_name_ja: 第二十三章　賭博及び富くじに関する罪
---

# 刑法 第187条(（富くじ発売等）)

## 原文 (日本語)

### 第百八十七条第一項

富くじを発売した者は、二年以下の拘禁刑又は百五十万円以下の罰金に処する。

### 第百八十七条第二項

富くじ発売の取次ぎをした者は、一年以下の拘禁刑又は百万円以下の罰金に処する。

### 第百八十七条第三項

前二項に規定するもののほか、富くじを授受した者は、二十万円以下の罰金又は科料に処する。
