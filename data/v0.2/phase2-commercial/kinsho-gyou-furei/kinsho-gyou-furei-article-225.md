---
law_id: 419M60000002052
law_name_ja: 金融商品取引業等に関する内閣府令
law_name_en: (English name pending)
article_number: '225'
article_id: kinsho-gyou-furei-art-225
version_date: '2007-08-06'
source_url: https://laws.e-gov.go.jp/law/419M60000002052
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kinsho-gyou-furei-art-225-p1-hashira
    type: hashira
    text: 法第六十条の六において準用する法第四十六条の二の規定により取引所取引許可業者が作成し、保存しなければならない帳簿書類は、第百五十七条第一項第三号、第四号、第六号、第九号、第十号及び第十三号に掲げる帳簿書類又は外国の法令に基づいて作成される書類であってこれらの帳簿書類に類するもの（以下この項において「外国帳簿書類」といい、外国帳簿書類が外国語で作成される場合にあっては、次に掲げる書類（次項において「外国帳簿書類等」という。））とする。
    modality: gimu
    references:
    - 次項
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-gyou-furei-art-225-p2
    type: simple
    text: 前項に規定する帳簿書類は、第百五十七条第一項第三号に掲げる帳簿書類及びこれに類する外国帳簿書類等にあっては、その作成の日から七年間、同項第四号、第六号、第九号、第十号及び第十三号に掲げる帳簿書類並びにこれらに類する外国帳簿書類等にあっては、その作成の日から十年間保存しなければならない。
    modality: gimu
    references:
    - 前項
    - 同項
cases: []
amendments: []
tags:
- phase2-commercial
- auto-generated
parent_section:
  shou: 2
  shou_name_ja: 第二章　金融商品取引業者等
  setsu: 5
  setsu_name_ja: 第五節　外国業者に関する特例
  kan: 3
  kan_name_ja: 第三款　取引所取引業務の許可
---

# 金融商品取引業等に関する内閣府令 第225条(（業務に関する帳簿書類）)

## 原文 (日本語)

### 第二百二十五条第一項

<!-- segment: hashira id: kinsho-gyou-furei-art-225-p1-hashira -->
法第六十条の六において準用する法第四十六条の二の規定により取引所取引許可業者が作成し、保存しなければならない帳簿書類は、第百五十七条第一項第三号、第四号、第六号、第九号、第十号及び第十三号に掲げる帳簿書類又は外国の法令に基づいて作成される書類であってこれらの帳簿書類に類するもの（以下この項において「外国帳簿書類」といい、外国帳簿書類が外国語で作成される場合にあっては、次に掲げる書類（次項において「外国帳簿書類等」という。））とする。

### 第二百二十五条第二項

<!-- segment: simple id: kinsho-gyou-furei-art-225-p2 -->
前項に規定する帳簿書類は、第百五十七条第一項第三号に掲げる帳簿書類及びこれに類する外国帳簿書類等にあっては、その作成の日から七年間、同項第四号、第六号、第九号、第十号及び第十三号に掲げる帳簿書類並びにこれらに類する外国帳簿書類等にあっては、その作成の日から十年間保存しなければならない。
