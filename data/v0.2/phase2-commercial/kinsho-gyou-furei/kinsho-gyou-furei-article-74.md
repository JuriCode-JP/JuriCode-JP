---
law_id: 419M60000002052
law_name_ja: 金融商品取引業等に関する内閣府令
law_name_en: (English name pending)
article_number: '74'
article_id: kinsho-gyou-furei-art-74
version_date: '2007-08-06'
source_url: https://laws.e-gov.go.jp/law/419M60000002052
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: true
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-gyou-furei-art-74-p1-honbun
    type: honbun
    text: 令第十六条第一項第一号に規定する内閣府令で定めるものは、手数料、報酬、費用その他いかなる名称によるかを問わず、金融商品取引契約に関して顧客が支払うべき対価（有価証券の価格又は保証金等の額（同項第三号に規定する保証金等の額をいう。第二百六十八条第一項において同じ。）を除く。以下この款において「手数料等」という。）の種類ごとの金額若しくはその上限額又はこれらの計算方法（当該金融商品取引契約に係る有価証券の価格、令第十六条第一項第三号に規定するデリバティブ取引等の額若しくは運用財産の額に対する割合又は金融商品取引行為を行うことにより生じた利益に対する割合を含む。以下この項において同じ。）の概要及び当該金額の合計額若しくはその上限額又はこれらの計算方法の概要とする。
    modality: gimu
    references:
    - 同項
  - id: kinsho-gyou-furei-art-74-p1-tadashi
    type: tadashi
    text: ただし、これらの表示をすることができない場合にあっては、その旨及びその理由とする。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-gyou-furei-art-74-p2
    type: simple
    text: 前項の金融商品取引契約が法第二条第一項第十号若しくは第十一号に掲げる有価証券に表示されるべき権利又は同条第二項第五号若しくは第六号に掲げる権利（以下この条及び第二百六十八条において「投資信託受益権等」という。）の取得に係るものであって、当該投資信託受益権等に係る財産が他の投資信託受益権等（以下この条において「出資対象投資信託受益権等」という。）に対して出資され、又は拠出されるものである場合には、前項の手数料等には、当該出資対象投資信託受益権等に係る信託報酬その他の手数料等を含むものとする。
    modality: gimu
    references:
    - 前項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-gyou-furei-art-74-p3
    type: simple
    text: 前項の出資対象投資信託受益権等に係る財産が他の投資信託受益権等に対して出資され、又は拠出される場合には、当該他の投資信託受益権等を出資対象投資信託受益権等とみなして、前二項の規定を適用する。
    modality: unspecified
    references:
    - 前項
    - 前二項
- number: 4
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-gyou-furei-art-74-p4
    type: junyou
    text: 前項の規定は、同項（この項において準用する場合を含む。）の規定により出資対象投資信託受益権等とみなされた投資信託受益権等に係る財産が他の投資信託受益権等に対して出資され、又は拠出される場合について準用する。
    modality: unspecified
    references:
    - 前項
    - 同項
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  shou: 2
  shou_name_ja: 第二章　金融商品取引業者等
  setsu: 2
  setsu_name_ja: 第二節　業務
  kan: 1
  kan_name_ja: 第一款　通則
---

# 金融商品取引業等に関する内閣府令 第74条(（顧客が支払うべき対価に関する事項）)

## 原文 (日本語)

### 第七十四条第一項

<!-- segment: honbun id: kinsho-gyou-furei-art-74-p1-honbun -->
令第十六条第一項第一号に規定する内閣府令で定めるものは、手数料、報酬、費用その他いかなる名称によるかを問わず、金融商品取引契約に関して顧客が支払うべき対価（有価証券の価格又は保証金等の額（同項第三号に規定する保証金等の額をいう。第二百六十八条第一項において同じ。）を除く。以下この款において「手数料等」という。）の種類ごとの金額若しくはその上限額又はこれらの計算方法（当該金融商品取引契約に係る有価証券の価格、令第十六条第一項第三号に規定するデリバティブ取引等の額若しくは運用財産の額に対する割合又は金融商品取引行為を行うことにより生じた利益に対する割合を含む。以下この項において同じ。）の概要及び当該金額の合計額若しくはその上限額又はこれらの計算方法の概要とする。<!-- segment: tadashi id: kinsho-gyou-furei-art-74-p1-tadashi -->
ただし、これらの表示をすることができない場合にあっては、その旨及びその理由とする。

### 第七十四条第二項

<!-- segment: simple id: kinsho-gyou-furei-art-74-p2 -->
前項の金融商品取引契約が法第二条第一項第十号若しくは第十一号に掲げる有価証券に表示されるべき権利又は同条第二項第五号若しくは第六号に掲げる権利（以下この条及び第二百六十八条において「投資信託受益権等」という。）の取得に係るものであって、当該投資信託受益権等に係る財産が他の投資信託受益権等（以下この条において「出資対象投資信託受益権等」という。）に対して出資され、又は拠出されるものである場合には、前項の手数料等には、当該出資対象投資信託受益権等に係る信託報酬その他の手数料等を含むものとする。

### 第七十四条第三項

<!-- segment: simple id: kinsho-gyou-furei-art-74-p3 -->
前項の出資対象投資信託受益権等に係る財産が他の投資信託受益権等に対して出資され、又は拠出される場合には、当該他の投資信託受益権等を出資対象投資信託受益権等とみなして、前二項の規定を適用する。

### 第七十四条第四項

<!-- segment: junyou id: kinsho-gyou-furei-art-74-p4 -->
前項の規定は、同項（この項において準用する場合を含む。）の規定により出資対象投資信託受益権等とみなされた投資信託受益権等に係る財産が他の投資信託受益権等に対して出資され、又は拠出される場合について準用する。
