---
law_id: 419M60000002052
law_name_ja: 金融商品取引業等に関する内閣府令
law_name_en: (English name pending)
article_number: '48'
article_id: kinsho-gyou-furei-art-48
version_date: '2007-08-06'
source_url: https://laws.e-gov.go.jp/law/419M60000002052
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-gyou-furei-art-48-p1
    type: simple
    text: 管轄財務局長等は、その登録をした登録金融機関に係る金融機関登録簿を当該登録金融機関の本店等の所在地を管轄する財務局（当該所在地が福岡財務支局の管轄区域内にある場合にあっては、福岡財務支局）に備え置き、公衆の縦覧に供するものとする。
    modality: gimu
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  shou: 2
  shou_name_ja: 第二章　金融商品取引業者等
  setsu: 1
  setsu_name_ja: 第一節　総則
  kan: 4
  kan_name_ja: 第四款　登録金融機関
---

# 金融商品取引業等に関する内閣府令 第48条(（金融機関登録簿の縦覧）)

## 原文 (日本語)

### 第四十八条

<!-- segment: simple id: kinsho-gyou-furei-art-48-p1 -->
管轄財務局長等は、その登録をした登録金融機関に係る金融機関登録簿を当該登録金融機関の本店等の所在地を管轄する財務局（当該所在地が福岡財務支局の管轄区域内にある場合にあっては、福岡財務支局）に備え置き、公衆の縦覧に供するものとする。
