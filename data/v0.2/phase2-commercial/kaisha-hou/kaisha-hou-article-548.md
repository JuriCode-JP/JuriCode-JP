---
law_id: 417AC0000000086
law_name_ja: 会社法
law_name_en: Companies Act
article_number: '548'
article_id: kaisha-hou-art-548
version_date: '2005-07-26'
source_url: https://laws.e-gov.go.jp/law/417AC0000000086
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-548-p1-hashira
    type: hashira
    text: 債権者集会を招集する者（以下この款において「招集者」という。）は、債権者集会を招集する場合には、次に掲げる事項を定めなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-548-p2
    type: simple
    text: 清算株式会社が債権者集会を招集する場合には、当該清算株式会社は、各協定債権について債権者集会における議決権の行使の許否及びその額を定めなければならない。
    modality: gimu
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-548-p3-zen
    type: zen_dan
    text: 清算株式会社以外の者が債権者集会を招集する場合には、その招集者は、清算株式会社に対し、前項に規定する事項を定めることを請求しなければならない。
    modality: gimu
    references:
    - 前項
  - id: kaisha-hou-art-548-p3-kou
    type: kou_dan
    text: この場合において、その請求があったときは、清算株式会社は、同項に規定する事項を定めなければならない。
    modality: gimu
    references:
    - 同項
    depends_on: kaisha-hou-art-548-p3-zen
- number: 4
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-548-p4
    type: simple
    text: 清算株式会社の財産につき第五百二十二条第二項に規定する担保権を有する協定債権者は、その担保権の行使によって弁済を受けることができる協定債権の額については、議決権を有しない。
    modality: unspecified
- number: 5
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-548-p5
    type: simple
    text: 協定債権者は、共助対象外国租税の請求権については、議決権を有しない。
    modality: unspecified
cases: []
amendments: []
tags:
- phase2-commercial
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　株式会社
  shou: 9
  shou_name_ja: 第九章　清算
  setsu: 2
  setsu_name_ja: 第二節　特別清算
  kan: 8
  kan_name_ja: 第八款　債権者集会
---

# 会社法 第548条(（債権者集会の招集等の決定）)

## 原文 (日本語)

### 第五百四十八条第一項

<!-- segment: hashira id: kaisha-hou-art-548-p1-hashira -->
債権者集会を招集する者（以下この款において「招集者」という。）は、債権者集会を招集する場合には、次に掲げる事項を定めなければならない。

### 第五百四十八条第二項

<!-- segment: simple id: kaisha-hou-art-548-p2 -->
清算株式会社が債権者集会を招集する場合には、当該清算株式会社は、各協定債権について債権者集会における議決権の行使の許否及びその額を定めなければならない。

### 第五百四十八条第三項

<!-- segment: zen_dan id: kaisha-hou-art-548-p3-zen -->
清算株式会社以外の者が債権者集会を招集する場合には、その招集者は、清算株式会社に対し、前項に規定する事項を定めることを請求しなければならない。<!-- segment: kou_dan id: kaisha-hou-art-548-p3-kou depends_on: kaisha-hou-art-548-p3-zen -->
この場合において、その請求があったときは、清算株式会社は、同項に規定する事項を定めなければならない。

### 第五百四十八条第四項

<!-- segment: simple id: kaisha-hou-art-548-p4 -->
清算株式会社の財産につき第五百二十二条第二項に規定する担保権を有する協定債権者は、その担保権の行使によって弁済を受けることができる協定債権の額については、議決権を有しない。

### 第五百四十八条第五項

<!-- segment: simple id: kaisha-hou-art-548-p5 -->
協定債権者は、共助対象外国租税の請求権については、議決権を有しない。
