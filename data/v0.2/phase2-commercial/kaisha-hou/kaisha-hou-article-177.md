---
law_id: 417AC0000000086
law_name_ja: 会社法
law_name_en: Companies Act
article_number: '177'
article_id: kaisha-hou-art-177
version_date: '2005-07-26'
source_url: https://laws.e-gov.go.jp/law/417AC0000000086
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-177-p1
    type: simple
    text: 前条第一項の規定による請求があった場合には、第百七十五条第一項第一号の株式の売買価格は、株式会社と同項第二号の者との協議によって定める。
    modality: unspecified
    references:
    - 同項
    - 前条
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-177-p2
    type: simple
    text: 株式会社又は第百七十五条第一項第二号の者は、前条第一項の規定による請求があった日から二十日以内に、裁判所に対し、売買価格の決定の申立てをすることができる。
    modality: kanou_kenri
    references:
    - 前条
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-177-p3
    type: simple
    text: 裁判所は、前項の決定をするには、前条第一項の規定による請求の時における株式会社の資産状態その他一切の事情を考慮しなければならない。
    modality: gimu
    references:
    - 前項
    - 前条
- number: 4
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-177-p4
    type: tokusoku
    text: 第一項の規定にかかわらず、第二項の期間内に同項の申立てがあったときは、当該申立てにより裁判所が定めた額をもって第百七十五条第一項第一号の株式の売買価格とする。
    modality: gimu
    override_flag: true
    override_target:
    - 第一項
    references:
    - 同項
- number: 5
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-177-p5
    type: simple
    text: 第二項の期間内に同項の申立てがないとき（当該期間内に第一項の協議が調った場合を除く。）は、前条第一項の規定による請求は、その効力を失う。
    modality: unspecified
    references:
    - 同項
    - 前条
cases: []
amendments: []
tags:
- phase2-commercial
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　株式会社
  shou: 2
  shou_name_ja: 第二章　株式
  setsu: 4
  setsu_name_ja: 第四節　株式会社による自己の株式の取得
  kan: 5
  kan_name_ja: 第五款　相続人等に対する売渡しの請求
---

# 会社法 第177条(（売買価格の決定）)

## 原文 (日本語)

### 第百七十七条第一項

<!-- segment: simple id: kaisha-hou-art-177-p1 -->
前条第一項の規定による請求があった場合には、第百七十五条第一項第一号の株式の売買価格は、株式会社と同項第二号の者との協議によって定める。

### 第百七十七条第二項

<!-- segment: simple id: kaisha-hou-art-177-p2 -->
株式会社又は第百七十五条第一項第二号の者は、前条第一項の規定による請求があった日から二十日以内に、裁判所に対し、売買価格の決定の申立てをすることができる。

### 第百七十七条第三項

<!-- segment: simple id: kaisha-hou-art-177-p3 -->
裁判所は、前項の決定をするには、前条第一項の規定による請求の時における株式会社の資産状態その他一切の事情を考慮しなければならない。

### 第百七十七条第四項

<!-- segment: tokusoku id: kaisha-hou-art-177-p4 override_flag: true override_target: 第一項 -->
第一項の規定にかかわらず、第二項の期間内に同項の申立てがあったときは、当該申立てにより裁判所が定めた額をもって第百七十五条第一項第一号の株式の売買価格とする。

### 第百七十七条第五項

<!-- segment: simple id: kaisha-hou-art-177-p5 -->
第二項の期間内に同項の申立てがないとき（当該期間内に第一項の協議が調った場合を除く。）は、前条第一項の規定による請求は、その効力を失う。
