---
law_id: 417AC0000000086
law_name_ja: 会社法
law_name_en: Companies Act
article_number: '618'
article_id: kaisha-hou-art-618
version_date: '2005-07-26'
source_url: https://laws.e-gov.go.jp/law/417AC0000000086
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-618-p1-hashira
    type: hashira
    text: 持分会社の社員は、当該持分会社の営業時間内は、いつでも、次に掲げる請求をすることができる。
    modality: kanou_kenri
- number: 2
  has_proviso: true
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-618-p2-honbun
    type: honbun
    text: 前項の規定は、定款で別段の定めをすることを妨げない。
    modality: jogai
    references:
    - 前項
  - id: kaisha-hou-art-618-p2-tadashi
    type: tadashi
    text: ただし、定款によっても、社員が事業年度の終了時に同項各号に掲げる請求をすることを制限する旨を定めることができない。
    modality: kanou_negative
    references:
    - 同項
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 3
  hen_name_ja: 第三編　持分会社
  shou: 5
  shou_name_ja: 第五章　計算等
  setsu: 3
  setsu_name_ja: 第三節　計算書類
---

# 会社法 第618条(（計算書類の閲覧等）)

## 原文 (日本語)

### 第六百十八条第一項

<!-- segment: hashira id: kaisha-hou-art-618-p1-hashira -->
持分会社の社員は、当該持分会社の営業時間内は、いつでも、次に掲げる請求をすることができる。

### 第六百十八条第二項

<!-- segment: honbun id: kaisha-hou-art-618-p2-honbun -->
前項の規定は、定款で別段の定めをすることを妨げない。<!-- segment: tadashi id: kaisha-hou-art-618-p2-tadashi -->
ただし、定款によっても、社員が事業年度の終了時に同項各号に掲げる請求をすることを制限する旨を定めることができない。
