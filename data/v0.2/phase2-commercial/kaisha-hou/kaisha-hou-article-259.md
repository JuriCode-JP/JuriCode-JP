---
law_id: 417AC0000000086
law_name_ja: 会社法
law_name_en: Companies Act
article_number: '259'
article_id: kaisha-hou-art-259
version_date: '2005-07-26'
source_url: https://laws.e-gov.go.jp/law/417AC0000000086
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-259-p1-hashira
    type: hashira
    text: 株式会社は、次の各号に掲げる場合には、当該各号の新株予約権の新株予約権者に係る新株予約権原簿記載事項を新株予約権原簿に記載し、又は記録しなければならない。
    modality: gimu
  - id: kaisha-hou-art-259-p1-kou-1
    type: kou
    text: 当該株式会社の新株予約権を取得した場合
    modality: unspecified
    item_number: 1
  - id: kaisha-hou-art-259-p1-kou-2
    type: kou
    text: 自己新株予約権を処分した場合
    modality: unspecified
    item_number: 2
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-259-p2
    type: simple
    text: 前項の規定は、無記名新株予約権及び無記名新株予約権付社債に付された新株予約権については、適用しない。
    modality: jogai
    references:
    - 前項
cases: []
amendments: []
tags:
- phase2-commercial
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　株式会社
  shou: 3
  shou_name_ja: 第三章　新株予約権
  setsu: 4
  setsu_name_ja: 第四節　新株予約権の譲渡等
  kan: 1
  kan_name_ja: 第一款　新株予約権の譲渡
---

# 会社法 第259条(（新株予約権者の請求によらない新株予約権原簿記載事項の記載又は記録）)

## 原文 (日本語)

### 第二百五十九条第一項

株式会社は、次の各号に掲げる場合には、当該各号の新株予約権の新株予約権者に係る新株予約権原簿記載事項を新株予約権原簿に記載し、又は記録しなければならない。

一　当該株式会社の新株予約権を取得した場合

二　自己新株予約権を処分した場合

### 第二百五十九条第二項

前項の規定は、無記名新株予約権及び無記名新株予約権付社債に付された新株予約権については、適用しない。
