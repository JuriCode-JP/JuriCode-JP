---
law_id: 417AC0000000086
law_name_ja: 会社法
law_name_en: Companies Act
article_number: '112'
article_id: kaisha-hou-art-112
version_date: '2005-07-26'
source_url: https://laws.e-gov.go.jp/law/417AC0000000086
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-112-p1
    type: simple
    text: 第百八条第二項第九号に掲げる事項（取締役に関するものに限る。）についての定款の定めは、この法律又は定款で定めた取締役の員数を欠いた場合において、そのために当該員数に足りる数の取締役を選任することができないときは、廃止されたものとみなす。
    modality: unspecified
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-112-p2
    type: junyou
    text: 前項の規定は、第百八条第二項第九号に掲げる事項（監査役に関するものに限る。）についての定款の定めについて準用する。
    modality: unspecified
    references:
    - 前項
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　株式会社
  shou: 2
  shou_name_ja: 第二章　株式
  setsu: 1
  setsu_name_ja: 第一節　総則
---

# 会社法 第112条(（取締役の選任等に関する種類株式の定款の定めの廃止の特則）)

## 原文 (日本語)

### 第百十二条第一項

<!-- segment: simple id: kaisha-hou-art-112-p1 -->
第百八条第二項第九号に掲げる事項（取締役に関するものに限る。）についての定款の定めは、この法律又は定款で定めた取締役の員数を欠いた場合において、そのために当該員数に足りる数の取締役を選任することができないときは、廃止されたものとみなす。

### 第百十二条第二項

<!-- segment: junyou id: kaisha-hou-art-112-p2 -->
前項の規定は、第百八条第二項第九号に掲げる事項（監査役に関するものに限る。）についての定款の定めについて準用する。
