---
law_id: 417AC0000000086
law_name_ja: 会社法
law_name_en: Companies Act
article_number: '898'
article_id: kaisha-hou-art-898
version_date: '2005-07-26'
source_url: https://laws.e-gov.go.jp/law/417AC0000000086
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-898-p1-hashira
    type: hashira
    text: 裁判所は、次に掲げる裁判を変更し、又は取り消すことができる。
    modality: koka_torikeshi
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-898-p2
    type: simple
    text: 前項各号に掲げる裁判及び同項の規定による決定に対しては、即時抗告をすることができる。
    modality: kanou_kenri
    references:
    - 前項
    - 同項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-898-p3
    type: simple
    text: 前項の即時抗告は、執行停止の効力を有しない。
    modality: unspecified
    references:
    - 前項
- number: 4
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-898-p4
    type: simple
    text: 第二項に規定する裁判及び同項の即時抗告についての裁判があった場合には、その裁判書を当事者に送達しなければならない。
    modality: gimu
    references:
    - 同項
- number: 5
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-898-p5
    type: simple
    text: 裁判所は、第一項第二号に掲げる裁判をしたときは、直ちに、その旨を公告しなければならない。当該裁判を変更し、又は取り消す決定があったときも、同様とする。
    modality: gimu
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  hen: 7
  hen_name_ja: 第七編　雑則
  shou: 3
  shou_name_ja: 第三章　非訟
  setsu: 3
  setsu_name_ja: 第三節　特別清算の手続に関する特則
  kan: 3
  kan_name_ja: 第三款　特別清算の実行の手続に関する特則
---

# 会社法 第898条(（清算株式会社の財産に関する保全処分等）)

## 原文 (日本語)

### 第八百九十八条第一項

<!-- segment: hashira id: kaisha-hou-art-898-p1-hashira -->
裁判所は、次に掲げる裁判を変更し、又は取り消すことができる。

### 第八百九十八条第二項

<!-- segment: simple id: kaisha-hou-art-898-p2 -->
前項各号に掲げる裁判及び同項の規定による決定に対しては、即時抗告をすることができる。

### 第八百九十八条第三項

<!-- segment: simple id: kaisha-hou-art-898-p3 -->
前項の即時抗告は、執行停止の効力を有しない。

### 第八百九十八条第四項

<!-- segment: simple id: kaisha-hou-art-898-p4 -->
第二項に規定する裁判及び同項の即時抗告についての裁判があった場合には、その裁判書を当事者に送達しなければならない。

### 第八百九十八条第五項

<!-- segment: simple id: kaisha-hou-art-898-p5 -->
裁判所は、第一項第二号に掲げる裁判をしたときは、直ちに、その旨を公告しなければならない。当該裁判を変更し、又は取り消す決定があったときも、同様とする。
