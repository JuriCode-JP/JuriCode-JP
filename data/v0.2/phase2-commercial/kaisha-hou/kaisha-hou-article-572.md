---
law_id: 417AC0000000086
law_name_ja: 会社法
law_name_en: Companies Act
article_number: '572'
article_id: kaisha-hou-art-572
version_date: '2005-07-26'
source_url: https://laws.e-gov.go.jp/law/417AC0000000086
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kaisha-hou-art-572-p1-zen
    type: zen_dan
    text: 協定の実行上必要があるときは、協定の内容を変更することができる。
    modality: kanou_kenri
  - id: kaisha-hou-art-572-p1-kou
    type: kou_dan
    text: この場合においては、第五百六十三条から前条までの規定を準用する。
    modality: unspecified
    references:
    - 前条
    depends_on: kaisha-hou-art-572-p1-zen
cases: []
amendments: []
tags:
- phase2-commercial
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　株式会社
  shou: 9
  shou_name_ja: 第九章　清算
  setsu: 2
  setsu_name_ja: 第二節　特別清算
  kan: 9
  kan_name_ja: 第九款　協定
---

# 会社法 第572条(（協定の内容の変更）)

## 原文 (日本語)

### 第五百七十二条

<!-- segment: zen_dan id: kaisha-hou-art-572-p1-zen -->
協定の実行上必要があるときは、協定の内容を変更することができる。<!-- segment: kou_dan id: kaisha-hou-art-572-p1-kou depends_on: kaisha-hou-art-572-p1-zen -->
この場合においては、第五百六十三条から前条までの規定を準用する。
