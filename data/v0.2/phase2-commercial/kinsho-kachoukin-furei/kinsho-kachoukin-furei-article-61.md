---
law_id: 417M60000002017
law_name_ja: 金融商品取引法第六章の二の規定による課徴金に関する内閣府令
law_name_en: (English name pending)
article_number: '61'
article_id: kinsho-kachoukin-furei-art-61
version_date: '2005-03-04'
source_url: https://laws.e-gov.go.jp/law/417M60000002017
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kinsho-kachoukin-furei-art-61-p1-hashira
    type: hashira
    text: 法第百八十五条の七第一項、第二項、第四項から第八項まで及び第十項から第十七項までの決定には、次に掲げる事項を記載しなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-kachoukin-furei-art-61-p2
    type: simple
    text: 前項第一号に掲げる事項には、納付すべき課徴金の額及び納付期限を記載しなければならない。
    modality: gimu
    references:
    - 前項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-kachoukin-furei-art-61-p3-zen
    type: zen_dan
    text: 第一項第二号に掲げる事項には、課徴金に係る法第百七十八条第一項各号に掲げる事実、法令の適用及び課徴金の計算の基礎を記載しなければならない。
    modality: gimu
  - id: kinsho-kachoukin-furei-art-61-p3-kou
    type: kou_dan
    text: この場合においては、審判手続開始決定書を引用することができる。
    modality: kanou_kenri
    depends_on: kinsho-kachoukin-furei-art-61-p3-zen
- number: 4
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kinsho-kachoukin-furei-art-61-p4-hashira
    type: hashira
    text: 法第百八十五条の七第十八項の決定には、次の各号のいずれかに該当する旨及びその理由を記載しなければならない。
    modality: gimu
cases: []
amendments: []
tags:
- phase2-commercial
- auto-generated
parent_section:
  shou: 2
  shou_name_ja: 第二章　審判手続
  setsu: 5
  setsu_name_ja: 第五節　決定
---

# 金融商品取引法第六章の二の規定による課徴金に関する内閣府令 第61条(（決定の記載事項）)

## 原文 (日本語)

### 第六十一条第一項

<!-- segment: hashira id: kinsho-kachoukin-furei-art-61-p1-hashira -->
法第百八十五条の七第一項、第二項、第四項から第八項まで及び第十項から第十七項までの決定には、次に掲げる事項を記載しなければならない。

### 第六十一条第二項

<!-- segment: simple id: kinsho-kachoukin-furei-art-61-p2 -->
前項第一号に掲げる事項には、納付すべき課徴金の額及び納付期限を記載しなければならない。

### 第六十一条第三項

<!-- segment: zen_dan id: kinsho-kachoukin-furei-art-61-p3-zen -->
第一項第二号に掲げる事項には、課徴金に係る法第百七十八条第一項各号に掲げる事実、法令の適用及び課徴金の計算の基礎を記載しなければならない。<!-- segment: kou_dan id: kinsho-kachoukin-furei-art-61-p3-kou depends_on: kinsho-kachoukin-furei-art-61-p3-zen -->
この場合においては、審判手続開始決定書を引用することができる。

### 第六十一条第四項

<!-- segment: hashira id: kinsho-kachoukin-furei-art-61-p4-hashira -->
法第百八十五条の七第十八項の決定には、次の各号のいずれかに該当する旨及びその理由を記載しなければならない。
