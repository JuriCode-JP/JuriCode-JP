---
law_id: 132AC0000000048
law_name_ja: 商法
law_name_en: Commercial Code
article_number: '758'
article_id: shouhou-art-758
version_date: '1899-03-09'
source_url: https://laws.e-gov.go.jp/law/132AC0000000048
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: shouhou-art-758-p1-hashira
    type: hashira
    text: 船荷証券には、次に掲げる事項（受取船荷証券にあっては、第七号及び第八号に掲げる事項を除く。）を記載し、運送人又は船長がこれに署名し、又は記名押印しなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: shouhou-art-758-p2-zen
    type: zen_dan
    text: 受取船荷証券と引換えに船積船荷証券の交付の請求があったときは、その受取船荷証券に船積みがあった旨を記載し、かつ、署名し、又は記名押印して、船積船荷証券の作成に代えることができる。
    modality: kanou_kenri
  - id: shouhou-art-758-p2-kou
    type: kou_dan
    text: この場合においては、前項第七号及び第八号に掲げる事項をも記載しなければならない。
    modality: gimu
    references:
    - 前項
    depends_on: shouhou-art-758-p2-zen
cases: []
amendments: []
tags:
- phase2-commercial
- auto-generated
parent_section:
  hen: 3
  hen_name_ja: 第三編　海商
  shou: 3
  shou_name_ja: 第三章　海上物品運送に関する特則
  setsu: 3
  setsu_name_ja: 第三節　船荷証券等
---

# 商法 第758条(（船荷証券の記載事項）)

## 原文 (日本語)

### 第七百五十八条第一項

<!-- segment: hashira id: shouhou-art-758-p1-hashira -->
船荷証券には、次に掲げる事項（受取船荷証券にあっては、第七号及び第八号に掲げる事項を除く。）を記載し、運送人又は船長がこれに署名し、又は記名押印しなければならない。

### 第七百五十八条第二項

<!-- segment: zen_dan id: shouhou-art-758-p2-zen -->
受取船荷証券と引換えに船積船荷証券の交付の請求があったときは、その受取船荷証券に船積みがあった旨を記載し、かつ、署名し、又は記名押印して、船積船荷証券の作成に代えることができる。<!-- segment: kou_dan id: shouhou-art-758-p2-kou depends_on: shouhou-art-758-p2-zen -->
この場合においては、前項第七号及び第八号に掲げる事項をも記載しなければならない。
