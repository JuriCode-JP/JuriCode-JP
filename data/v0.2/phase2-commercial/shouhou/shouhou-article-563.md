---
law_id: 132AC0000000048
law_name_ja: 商法
law_name_en: Commercial Code
article_number: '563'
article_id: shouhou-art-563
version_date: '1899-03-09'
source_url: https://laws.e-gov.go.jp/law/132AC0000000048
source_format: e-gov-xml
last_verified: '2026-05-20'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: shouhou-art-563-p1-zen
    type: zen_dan
    text: 運送取扱人は、自ら運送をすることができる。
    modality: kanou_kenri
  - id: shouhou-art-563-p1-kou
    type: kou_dan
    text: この場合において、運送取扱人は、運送人と同一の権利義務を有する。
    modality: unspecified
    depends_on: shouhou-art-563-p1-zen
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: shouhou-art-563-p2
    type: simple
    text: 運送取扱人が委託者の請求によって船荷証券又は複合運送証券を作成したときは、自ら運送をするものとみなす。
    modality: unspecified
cases: []
amendments: []
tags:
- phase2-commercial
- auto-generated
parent_section:
  hen: 2
  hen_name_ja: 第二編　商行為
  shou: 7
  shou_name_ja: 第七章　運送取扱営業
---

# 商法 第563条(（介入権）)

## 原文 (日本語)

### 第五百六十三条第一項

<!-- segment: zen_dan id: shouhou-art-563-p1-zen -->
運送取扱人は、自ら運送をすることができる。<!-- segment: kou_dan id: shouhou-art-563-p1-kou depends_on: shouhou-art-563-p1-zen -->
この場合において、運送取扱人は、運送人と同一の権利義務を有する。

### 第五百六十三条第二項

<!-- segment: simple id: shouhou-art-563-p2 -->
運送取扱人が委託者の請求によって船荷証券又は複合運送証券を作成したときは、自ら運送をするものとみなす。
