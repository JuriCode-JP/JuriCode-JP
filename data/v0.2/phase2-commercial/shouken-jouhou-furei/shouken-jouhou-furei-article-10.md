---
law_id: 420M60000002078
law_name_ja: 証券情報等の提供又は公表に関する内閣府令
law_name_en: (English name pending)
article_number: '10'
article_id: shouken-jouhou-furei-art-10
version_date: '2008-12-05'
source_url: https://laws.e-gov.go.jp/law/420M60000002078
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: shouken-jouhou-furei-art-10-p1-hashira
    type: hashira
    text: 法第二十七条の三十二第四項に規定する内閣府令で定める場合は、次の各号に掲げる場合とし、同項に規定する内閣府令で定める期間は、それぞれ当該各号に定める期間とする。
    modality: gimu
    references:
    - 同項
cases: []
amendments: []
tags:
- phase2-commercial
- auto-generated
parent_section:
  shou: 2
  shou_name_ja: 第二章　特定証券情報等の提供又は公表
---

# 証券情報等の提供又は公表に関する内閣府令 第10条(（発行者等情報を公表しなければならない期間）)

## 原文 (日本語)

### 第十条

<!-- segment: hashira id: shouken-jouhou-furei-art-10-p1-hashira -->
法第二十七条の三十二第四項に規定する内閣府令で定める場合は、次の各号に掲げる場合とし、同項に規定する内閣府令で定める期間は、それぞれ当該各号に定める期間とする。
