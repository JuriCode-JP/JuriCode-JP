---
law_id: 323AC0000000025
law_name_ja: 金融商品取引法
law_name_en: (English name pending)
article_number: '137'
article_id: kinsho-hou-art-137
version_date: '1948-04-13'
source_url: https://laws.e-gov.go.jp/law/323AC0000000025
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kinsho-hou-art-137-p1-hashira
    type: hashira
    text: 会員金融商品取引所と会員金融商品取引所とが吸収合併をする場合には、吸収合併契約において、次に掲げる事項を定めなければならない。
    modality: gimu
  - id: kinsho-hou-art-137-p1-kou-1
    type: kou
    text: 一　吸収合併後存続する会員金融商品取引所（以下この款において「吸収合併存続会員金融商品取引所」という。）及び吸収合併により消滅する会員金融商品取引所（以下この款において「吸収合併消滅会員金融商品取引所」という。）の名称及び住所
    modality: unspecified
    item_number: 1
  - id: kinsho-hou-art-137-p1-kou-2
    type: kou
    text: 二　吸収合併がその効力を生ずる日（以下この款において「効力発生日」という。）その他内閣府令で定める事項
    modality: unspecified
    item_number: 2
cases: []
amendments: []
tags:
- phase2-commercial
- auto-generated
parent_section:
  shou: 5
  shou_name_ja: 第五章　金融商品取引所
  setsu: 4
  setsu_name_ja: 第四節　金融商品取引所の解散等
  kan: 2
  kan_name_ja: 第二款　合併
  moku: 2
  moku_name_ja: 第二目　会員金融商品取引所と会員金融商品取引所との合併
---

# 金融商品取引法 第137条(（会員金融商品取引所と会員金融商品取引所との吸収合併契約）)
会員金融商品取引所と会員金融商品取引所との吸収合併契約）)

## 原文 (日本語)

### 第百三十七条

会員金融商品取引所と会員金融商品取引所とが吸収合併をする場合には、吸収合併契約において、次に掲げる事項を定めなければならない。

一　吸収合併後存続する会員金融商品取引所（以下この款において「吸収合併存続会員金融商品取引所」という。）及び吸収合併により消滅する会員金融商品取引所（以下この款において「吸収合併消滅会員金融商品取引所」という。）の名称及び住所

二　吸収合併がその効力を生ずる日（以下この款において「効力発生日」という。）その他内閣府令で定める事項
