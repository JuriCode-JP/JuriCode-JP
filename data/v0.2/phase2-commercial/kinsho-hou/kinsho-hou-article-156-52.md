---
law_id: 323AC0000000025
law_name_ja: 金融商品取引法
law_name_en: (English name pending)
article_number: 156-52
article_id: kinsho-hou-art-156-52
version_date: '1948-04-13'
source_url: https://laws.e-gov.go.jp/law/323AC0000000025
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kinsho-hou-art-156-52-p1-hashira
    type: hashira
    text: 金融商品取引業等業務関連紛争について当該金融商品取引業等業務関連紛争の当事者間に訴訟が係属する場合において、次の各号のいずれかに掲げる事由があり、かつ、当該金融商品取引業等業務関連紛争の当事者の共同の申立てがあるときは、受訴裁判所は、四月以内の期間を定めて訴訟手続を中止する旨の決定をすることができる。
    modality: kanou_kenri
  - id: kinsho-hou-art-156-52-p1-kou-1
    type: kou
    text: 一　当該金融商品取引業等業務関連紛争について、当該金融商品取引業等業務関連紛争の当事者間において紛争解決手続が実施されていること。
    modality: unspecified
    item_number: 1
  - id: kinsho-hou-art-156-52-p1-kou-2
    type: kou
    text: 二　前号の場合のほか、当該金融商品取引業等業務関連紛争の当事者間に紛争解決手続によつて当該金融商品取引業等業務関連紛争の解決を図る旨の合意があること。
    modality: unspecified
    item_number: 2
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-hou-art-156-52-p2
    type: simple
    text: 受訴裁判所は、いつでも前項の決定を取り消すことができる。
    modality: koka_torikeshi
    references:
    - 前項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-hou-art-156-52-p3
    type: simple
    text: 第一項の申立てを却下する決定及び前項の規定により第一項の決定を取り消す決定に対しては、不服を申し立てることができない。
    modality: kanou_negative
    references:
    - 前項
cases: []
amendments: []
tags:
- phase2-commercial
- auto-generated
parent_section:
  shou: 55
  shou_name_ja: 第五章の五　指定紛争解決機関
  setsu: 2
  setsu_name_ja: 第二節　業務
---

# 金融商品取引法 第156-52条(（訴訟手続の中止）)

## 原文 (日本語)

### 第百五十六条の五十二第一項

金融商品取引業等業務関連紛争について当該金融商品取引業等業務関連紛争の当事者間に訴訟が係属する場合において、次の各号のいずれかに掲げる事由があり、かつ、当該金融商品取引業等業務関連紛争の当事者の共同の申立てがあるときは、受訴裁判所は、四月以内の期間を定めて訴訟手続を中止する旨の決定をすることができる。

一　当該金融商品取引業等業務関連紛争について、当該金融商品取引業等業務関連紛争の当事者間において紛争解決手続が実施されていること。

二　前号の場合のほか、当該金融商品取引業等業務関連紛争の当事者間に紛争解決手続によつて当該金融商品取引業等業務関連紛争の解決を図る旨の合意があること。

### 第百五十六条の五十二第二項

受訴裁判所は、いつでも前項の決定を取り消すことができる。

### 第百五十六条の五十二第三項

第一項の申立てを却下する決定及び前項の規定により第一項の決定を取り消す決定に対しては、不服を申し立てることができない。
