---
law_id: 323AC0000000025
law_name_ja: 金融商品取引法
law_name_en: (English name pending)
article_number: '54'
article_id: kinsho-hou-art-54
version_date: '1948-04-13'
source_url: https://laws.e-gov.go.jp/law/323AC0000000025
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-hou-art-54-p1
    type: simple
    text: 内閣総理大臣は、金融商品取引業者等が正当な理由がないのに、金融商品取引業等を行うことができることとなつた日から三月以内に業務を開始しないとき、又は引き続き三月以上その業務を休止したときは、当該金融商品取引業者等の第二十九条又は第三十三条の二の登録を取り消すことができる。
    modality: koka_torikeshi
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  shou: 3
  shou_name_ja: 第三章　金融商品取引業者等
  setsu: 4
  setsu_name_ja: 第四節　監督
---

# 金融商品取引法 第54条(（業務の不開始又は休止に基づく登録の取消し）)

## 原文 (日本語)

### 第五十四条

<!-- segment: simple id: kinsho-hou-art-54-p1 -->
内閣総理大臣は、金融商品取引業者等が正当な理由がないのに、金融商品取引業等を行うことができることとなつた日から三月以内に業務を開始しないとき、又は引き続き三月以上その業務を休止したときは、当該金融商品取引業者等の第二十九条又は第三十三条の二の登録を取り消すことができる。
