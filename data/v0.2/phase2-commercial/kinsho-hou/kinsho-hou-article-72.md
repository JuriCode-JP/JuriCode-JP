---
law_id: 323AC0000000025
law_name_ja: 金融商品取引法
law_name_en: (English name pending)
article_number: '72'
article_id: kinsho-hou-art-72
version_date: '1948-04-13'
source_url: https://laws.e-gov.go.jp/law/323AC0000000025
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-hou-art-72-p1
    type: simple
    text: 認可協会の役員若しくは職員又はこれらの職にあつた者は、その職務に関して知り得た秘密を漏らし、又は盗用してはならない。
    modality: gimu_negative
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-hou-art-72-p2
    type: simple
    text: 認可協会の役員若しくは職員又はこれらの職にあつた者は、その職務に関して知り得た情報を、認可協会の業務の用に供する目的以外に利用してはならない。
    modality: gimu_negative
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  shou: 4
  shou_name_ja: 第四章　金融商品取引業協会
  setsu: 1
  setsu_name_ja: 第一節　認可金融商品取引業協会
  kan: 3
  kan_name_ja: 第三款　管理
---

# 金融商品取引法 第72条(（役職員の秘密保持義務等）)

## 原文 (日本語)

### 第七十二条第一項

<!-- segment: simple id: kinsho-hou-art-72-p1 -->
<!-- segment: simple id: kinsho-hou-art-72-p2 -->
認可協会の役員若しくは職員又はこれらの職にあつた者は、その職務に関して知り得た秘密を漏らし、又は盗用してはならない。

### 第七十二条第二項

認可協会の役員若しくは職員又はこれらの職にあつた者は、その職務に関して知り得た情報を、認可協会の業務の用に供する目的以外に利用してはならない。
