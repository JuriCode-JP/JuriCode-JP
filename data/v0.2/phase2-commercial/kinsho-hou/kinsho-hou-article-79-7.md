---
law_id: 323AC0000000025
law_name_ja: 金融商品取引法
law_name_en: (English name pending)
article_number: 79-7
article_id: kinsho-hou-art-79-7
version_date: '1948-04-13'
source_url: https://laws.e-gov.go.jp/law/323AC0000000025
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kinsho-hou-art-79-7-p1-hashira
    type: hashira
    text: 有価証券の売買その他の取引及びデリバティブ取引等を公正かつ円滑にし、並びに金融商品取引業の健全な発展及び投資者の保護に資することを目的として、次の各号に掲げる業務を行おうとする法人（法人でない団体で代表者又は管理人の定めのあるものを含み、認可協会及び認定協会を除く。次条第三号ロにおいて同じ。）は、内閣総理大臣の認定を受けることができる。
    modality: kanou_kenri
    references:
    - 次条
  - id: kinsho-hou-art-79-7-p1-kou-1
    type: kou
    text: 金融商品取引業者又は金融商品仲介業者の行う金融商品取引業に対する苦情の解決
    modality: unspecified
    item_number: 1
  - id: kinsho-hou-art-79-7-p1-kou-2
    type: kou
    text: 金融商品取引業者又は金融商品仲介業者の行う金融商品取引業に争いがある場合のあつせん
    modality: unspecified
    item_number: 2
  - id: kinsho-hou-art-79-7-p1-kou-3
    type: kou
    text: 前二号に掲げるもののほか、金融商品取引業の健全な発展又は投資者の保護に資する業務
    modality: unspecified
    item_number: 3
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-hou-art-79-7-p2
    type: simple
    text: 前項の認定を受けようとする者は、政令で定めるところにより、内閣総理大臣に対し申請をしなければならない。
    modality: gimu
    references:
    - 前項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-hou-art-79-7-p3
    type: simple
    text: 内閣総理大臣は、第一項の認定をしたときは、その旨を公示しなければならない。
    modality: gimu
cases: []
amendments: []
tags:
- phase2-commercial
- auto-generated
parent_section:
  shou: 4
  shou_name_ja: 第四章　金融商品取引業協会
  setsu: 3
  setsu_name_ja: 第三節　認定投資者保護団体
---

# 金融商品取引法 第79-7条(（認定投資者保護団体の目的及び業務）)

## 原文 (日本語)

### 第七十九条の七第一項

有価証券の売買その他の取引及びデリバティブ取引等を公正かつ円滑にし、並びに金融商品取引業の健全な発展及び投資者の保護に資することを目的として、次の各号に掲げる業務を行おうとする法人（法人でない団体で代表者又は管理人の定めのあるものを含み、認可協会及び認定協会を除く。次条第三号ロにおいて同じ。）は、内閣総理大臣の認定を受けることができる。

一　金融商品取引業者又は金融商品仲介業者の行う金融商品取引業に対する苦情の解決

二　金融商品取引業者又は金融商品仲介業者の行う金融商品取引業に争いがある場合のあつせん

三　前二号に掲げるもののほか、金融商品取引業の健全な発展又は投資者の保護に資する業務

### 第七十九条の七第二項

前項の認定を受けようとする者は、政令で定めるところにより、内閣総理大臣に対し申請をしなければならない。

### 第七十九条の七第三項

内閣総理大臣は、第一項の認定をしたときは、その旨を公示しなければならない。
