---
law_id: 405M50000040014
law_name_ja: 金融商品取引法第二条に規定する定義に関する内閣府令
law_name_en: (English name pending)
article_number: '24'
article_id: kinsho-teigi-furei-art-24
version_date: '1993-03-03'
source_url: https://laws.e-gov.go.jp/law/405M50000040014
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kinsho-teigi-furei-art-24-p1-hashira
    type: hashira
    text: 法第二条第三十四項に規定する法人に類するものとして内閣府令で定めるものは、次に掲げるものとする。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-teigi-furei-art-24-p2
    type: simple
    text: 法第二条第三十四項に規定する記号又は数字に類するものとして内閣府令で定めるものは、順序を示す簡易な文章又は文字とする。
    modality: gimu
- number: 3
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kinsho-teigi-furei-art-24-p3-hashira
    type: hashira
    text: 法第二条第三十四項に規定する主として信用評価以外の事項を勘案して定められる等級として内閣府令で定めるものは、次に掲げるものとする。
    modality: gimu
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
---

# 金融商品取引法第二条に規定する定義に関する内閣府令 第24条(（信用格付の範囲）)

## 原文 (日本語)

### 第二十四条第一項

<!-- segment: hashira id: kinsho-teigi-furei-art-24-p1-hashira -->
法第二条第三十四項に規定する法人に類するものとして内閣府令で定めるものは、次に掲げるものとする。

### 第二十四条第二項

<!-- segment: simple id: kinsho-teigi-furei-art-24-p2 -->
法第二条第三十四項に規定する記号又は数字に類するものとして内閣府令で定めるものは、順序を示す簡易な文章又は文字とする。

### 第二十四条第三項

<!-- segment: hashira id: kinsho-teigi-furei-art-24-p3-hashira -->
法第二条第三十四項に規定する主として信用評価以外の事項を勘案して定められる等級として内閣府令で定めるものは、次に掲げるものとする。
