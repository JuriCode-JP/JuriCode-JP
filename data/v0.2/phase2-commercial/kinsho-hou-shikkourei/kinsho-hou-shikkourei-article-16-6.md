---
law_id: 340CO0000000321
law_name_ja: 金融商品取引法施行令
law_name_en: (English name pending)
article_number: 16-6
article_id: kinsho-hou-shikkourei-art-16-6
version_date: '1965-09-30'
source_url: https://laws.e-gov.go.jp/law/340CO0000000321
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: kinsho-hou-shikkourei-art-16-6-p1-hashira
    type: hashira
    text: 法第四十条の二第一項に規定する政令で定めるものは、次に掲げるものとする。
    modality: gimu
  - id: kinsho-hou-shikkourei-art-16-6-p1-kou-1
    type: kou
    text: '一　有価証券の売買（次に掲げるものを除く。）


      　イ　上場株券等（金融商品取引所に上場されている株券、新株予約権付社債券その他の有価証券で内閣府令で定めるものをいう。第三項において同じ。）の売買（デリバティブ取引に該当するものを除く。以下この号及び第三項において同じ。）


      　ロ　店頭売買有価証券の売買


      　ハ　取扱有価証券の売買'
    modality: unspecified
    item_number: 1
  - id: kinsho-hou-shikkourei-art-16-6-p1-kou-2
    type: kou
    text: 二　デリバティブ取引
    modality: unspecified
    item_number: 2
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-hou-shikkourei-art-16-6-p2
    type: simple
    text: 法第四十条の二第一項の規定による最良執行方針等は、同項に規定する有価証券等取引について銘柄ごとに最良の取引の条件で執行するための方法及び当該方法を選択する理由を内閣府令で定めるところにより記載して定めなければならない。
    modality: gimu
    references:
    - 同項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: kinsho-hou-shikkourei-art-16-6-p3
    type: simple
    text: 法第四十条の二第四項に規定する政令で定める取引は、上場株券等及び店頭売買有価証券の売買とする。
    modality: gimu
cases: []
amendments: []
tags:
- phase2-commercial
- auto-generated
parent_section:
  shou: 4
  shou_name_ja: 第四章　金融商品取引業者等
---

# 金融商品取引法施行令 第16-6条(（最良執行方針等の適用除外等）)

## 原文 (日本語)

### 第十六条の六第一項

法第四十条の二第一項に規定する政令で定めるものは、次に掲げるものとする。

一　有価証券の売買（次に掲げるものを除く。）

　イ　上場株券等（金融商品取引所に上場されている株券、新株予約権付社債券その他の有価証券で内閣府令で定めるものをいう。第三項において同じ。）の売買（デリバティブ取引に該当するものを除く。以下この号及び第三項において同じ。）

　ロ　店頭売買有価証券の売買

　ハ　取扱有価証券の売買

二　デリバティブ取引

### 第十六条の六第二項

法第四十条の二第一項の規定による最良執行方針等は、同項に規定する有価証券等取引について銘柄ごとに最良の取引の条件で執行するための方法及び当該方法を選択する理由を内閣府令で定めるところにより記載して定めなければならない。

### 第十六条の六第三項

法第四十条の二第四項に規定する政令で定める取引は、上場株券等及び店頭売買有価証券の売買とする。
