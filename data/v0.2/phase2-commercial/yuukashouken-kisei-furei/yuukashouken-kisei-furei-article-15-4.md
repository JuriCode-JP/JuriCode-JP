---
law_id: 419M60000002059
law_name_ja: 有価証券の取引等の規制に関する内閣府令
law_name_en: (English name pending)
article_number: 15-4
article_id: yuukashouken-kisei-furei-art-15-4
version_date: '2007-08-08'
source_url: https://laws.e-gov.go.jp/law/419M60000002059
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: yuukashouken-kisei-furei-art-15-4-p1-hashira
    type: hashira
    text: 主たる金融商品取引所は、令第二十六条の五第五項の規定に基づき、当該主たる金融商品取引所の会員等から提供された残高情報のうち次に掲げる要件のいずれかに該当するものを取りまとめ、遅滞なく、その内容を公表しなければならない。
    modality: gimu
  - id: yuukashouken-kisei-furei-art-15-4-p1-kou-1
    type: kou
    text: 一　当該残高情報に係る空売り残高割合が〇・〇〇五以上であること。
    modality: unspecified
    item_number: 1
  - id: yuukashouken-kisei-furei-art-15-4-p1-kou-2
    type: kou
    text: 二　当該残高情報に係る空売り残高割合が〇・〇〇五未満又は当該残高情報に係る第十五条の二第七項に規定する空売り残高売買単位数が五十以下であり、かつ、当該残高情報に係る直近空売り残高割合が〇・〇〇五以上であること。
    modality: unspecified
    item_number: 2
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: yuukashouken-kisei-furei-art-15-4-p2
    type: simple
    text: 前項の公表は、残高情報の提供を受けた日から一年間、インターネットの利用その他の適切な方法により行わなければならない。
    modality: gimu
    references:
    - 前項
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: yuukashouken-kisei-furei-art-15-4-p3
    type: junyou
    text: 前二項の規定は、認可金融商品取引業協会について準用する。
    modality: unspecified
    references:
    - 前二項
cases: []
amendments: []
tags:
- phase2-commercial
- auto-generated
parent_section:
  shou: 4
  shou_name_ja: 第四章　有価証券の空売り
---

# 有価証券の取引等の規制に関する内閣府令 第15-4条(（金融商品取引所等による空売りに係る情報の公表）)

## 原文 (日本語)

### 第十五条の四第一項

主たる金融商品取引所は、令第二十六条の五第五項の規定に基づき、当該主たる金融商品取引所の会員等から提供された残高情報のうち次に掲げる要件のいずれかに該当するものを取りまとめ、遅滞なく、その内容を公表しなければならない。

一　当該残高情報に係る空売り残高割合が〇・〇〇五以上であること。

二　当該残高情報に係る空売り残高割合が〇・〇〇五未満又は当該残高情報に係る第十五条の二第七項に規定する空売り残高売買単位数が五十以下であり、かつ、当該残高情報に係る直近空売り残高割合が〇・〇〇五以上であること。

### 第十五条の四第二項

前項の公表は、残高情報の提供を受けた日から一年間、インターネットの利用その他の適切な方法により行わなければならない。

### 第十五条の四第三項

前二項の規定は、認可金融商品取引業協会について準用する。
