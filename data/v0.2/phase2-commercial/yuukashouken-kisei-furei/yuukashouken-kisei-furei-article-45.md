---
law_id: 419M60000002059
law_name_ja: 有価証券の取引等の規制に関する内閣府令
law_name_en: (English name pending)
article_number: '45'
article_id: yuukashouken-kisei-furei-art-45
version_date: '2007-08-08'
source_url: https://laws.e-gov.go.jp/law/419M60000002059
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: yuukashouken-kisei-furei-art-45-p1
    type: simple
    text: 法第百六十五条の二第十四項に規定する内閣府令で定める場合は、第四十条第四項各号に掲げる場合とする。
    modality: gimu
cases: []
amendments: []
tags:
- phase2-commercial
- auto-generated
parent_section:
  shou: 6
  shou_name_ja: 第六章　上場会社等の役員及び主要株主等が行う売買等
---

# 有価証券の取引等の規制に関する内閣府令 第45条(（特定組合等の組合員に係る短期売買利益の返還の適用除外）)

## 原文 (日本語)

### 第四十五条

<!-- segment: simple id: yuukashouken-kisei-furei-art-45-p1 -->
法第百六十五条の二第十四項に規定する内閣府令で定める場合は、第四十条第四項各号に掲げる場合とする。
