---
law_id: 419M60000002059
law_name_ja: 有価証券の取引等の規制に関する内閣府令
law_name_en: (English name pending)
article_number: '66'
article_id: yuukashouken-kisei-furei-art-66
version_date: '2007-08-08'
source_url: https://laws.e-gov.go.jp/law/419M60000002059
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: yuukashouken-kisei-furei-art-66-p1
    type: simple
    text: 令第四十三条の十第三項に規定する内閣府令で定める財務局長又は財務支局長は、関東財務局長とする。
    modality: gimu
cases: []
amendments: []
tags:
- phase2-commercial
- auto-generated
parent_section:
  shou: 9
  shou_name_ja: 第九章　雑則
---

# 有価証券の取引等の規制に関する内閣府令 第66条(（特定有価証券等の売買に関する報告書等に関する権限の関東財務局長への委任）)

## 原文 (日本語)

### 第六十六条

<!-- segment: simple id: yuukashouken-kisei-furei-art-66-p1 -->
令第四十三条の十第三項に規定する内閣府令で定める財務局長又は財務支局長は、関東財務局長とする。
