---
law_id: 336M50000100001
law_name_ja: 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則
law_name_en: (English name pending)
article_number: '200'
article_id: yakkihou-shikoukisoku-art-200
version_date: '1961-02-01'
source_url: https://laws.e-gov.go.jp/law/336M50000100001
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: yakkihou-shikoukisoku-art-200-p1
    type: simple
    text: 令第六十条第一項に規定する検査合格証明書は、様式第九十六によるものとする。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: yakkihou-shikoukisoku-art-200-p2-hashira
    type: hashira
    text: 検査機関は、検査合格証明書を交付したときは、当該医薬品、医療機器又は再生医療等製品に係る次に掲げる事項を検査機関のホームページに掲載しなければならない。
    modality: gimu
cases: []
amendments: []
tags:
- phase3-pharma
- auto-generated
parent_section:
  shou: 6
  shou_name_ja: 第六章　医薬品等の基準及び検査
---

# 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則 第200条(（検査合格証明書等）)

## 原文 (日本語)

### 第二百条第一項

<!-- segment: simple id: yakkihou-shikoukisoku-art-200-p1 -->
令第六十条第一項に規定する検査合格証明書は、様式第九十六によるものとする。

### 第二百条第二項

<!-- segment: hashira id: yakkihou-shikoukisoku-art-200-p2-hashira -->
検査機関は、検査合格証明書を交付したときは、当該医薬品、医療機器又は再生医療等製品に係る次に掲げる事項を検査機関のホームページに掲載しなければならない。
