---
law_id: 336M50000100001
law_name_ja: 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則
law_name_en: (English name pending)
article_number: '263'
article_id: yakkihou-shikoukisoku-art-263
version_date: '1961-02-01'
source_url: https://laws.e-gov.go.jp/law/336M50000100001
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: yakkihou-shikoukisoku-art-263-p1
    type: simple
    text: 令第七十三条の規定による調査の結果の通知は、厚生労働大臣に対し、様式第二十六による通知書によつて行うものとする。
    modality: gimu
cases: []
amendments: []
tags:
- phase3-pharma
- auto-generated
parent_section:
  shou: 14
  shou_name_ja: 第十四章　雑則
---

# 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則 第263条(（輸出用の医薬品等の製造管理又は品質管理の方法の基準に係る調査の結果の通知）)

## 原文 (日本語)

### 第二百六十三条

<!-- segment: simple id: yakkihou-shikoukisoku-art-263-p1 -->
令第七十三条の規定による調査の結果の通知は、厚生労働大臣に対し、様式第二十六による通知書によつて行うものとする。
