---
law_id: 336M50000100001
law_name_ja: 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則
law_name_en: (English name pending)
article_number: '10'
article_id: yakkihou-shikoukisoku-art-10
version_date: '1961-02-01'
source_url: https://laws.e-gov.go.jp/law/336M50000100001
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: yakkihou-shikoukisoku-art-10-p1
    type: simple
    text: 法第六条ただし書の規定により、薬局の名称を付することができる場所は、病院又は診療所の調剤所とする。
    modality: gimu
cases: []
amendments: []
tags:
- phase3-pharma
- auto-generated
parent_section:
  shou: 12
  shou_name_ja: 第一章の二　薬局
---

# 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則 第10条(（名称の使用の特例）)

## 原文 (日本語)

### 第十条

<!-- segment: simple id: yakkihou-shikoukisoku-art-10-p1 -->
法第六条ただし書の規定により、薬局の名称を付することができる場所は、病院又は診療所の調剤所とする。
