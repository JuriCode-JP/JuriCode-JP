---
law_id: 336M50000100001
law_name_ja: 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則
law_name_en: (English name pending)
article_number: '3'
article_id: yakkihou-shikoukisoku-art-3
version_date: '1961-02-01'
source_url: https://laws.e-gov.go.jp/law/336M50000100001
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: yakkihou-shikoukisoku-art-3-p1
    type: simple
    text: 薬局開設者は、薬局開設の許可証を薬局の見やすい場所に掲示しておかなければならない。
    modality: gimu
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  shou: 12
  shou_name_ja: 第一章の二　薬局
---

# 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則 第3条(（薬局開設の許可証の掲示）)

## 原文 (日本語)

### 第三条

<!-- segment: simple id: yakkihou-shikoukisoku-art-3-p1 -->
薬局開設者は、薬局開設の許可証を薬局の見やすい場所に掲示しておかなければならない。
