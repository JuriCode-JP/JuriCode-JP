---
law_id: 336M50000100001
law_name_ja: 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則
law_name_en: (English name pending)
article_number: '218'
article_id: yakkihou-shikoukisoku-art-218
version_date: '1961-02-01'
source_url: https://laws.e-gov.go.jp/law/336M50000100001
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: yakkihou-shikoukisoku-art-218-p1
    type: simple
    text: 法第五十条から第五十二条までに規定する事項の記載は、邦文でされていなければならない。
    modality: gimu
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  shou: 7
  shou_name_ja: 第七章　医薬品等の取扱い
---

# 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則 第218条(（邦文記載）)

## 原文 (日本語)

### 第二百十八条

<!-- segment: simple id: yakkihou-shikoukisoku-art-218-p1 -->
法第五十条から第五十二条までに規定する事項の記載は、邦文でされていなければならない。
