---
law_id: 336M50000100001
law_name_ja: 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則
law_name_en: (English name pending)
article_number: '202'
article_id: yakkihou-shikoukisoku-art-202
version_date: '1961-02-01'
source_url: https://laws.e-gov.go.jp/law/336M50000100001
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: yakkihou-shikoukisoku-art-202-p1
    type: simple
    text: 出願者は、検査を受けた医薬品、医療機器又は再生医療等製品について様式第九十七による検査記録表を作成しておかなければならない。
    modality: gimu
cases: []
amendments: []
tags:
- phase3-pharma
- auto-generated
parent_section:
  shou: 6
  shou_name_ja: 第六章　医薬品等の基準及び検査
---

# 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則 第202条(（検査記録表）)

## 原文 (日本語)

### 第二百二条

<!-- segment: simple id: yakkihou-shikoukisoku-art-202-p1 -->
出願者は、検査を受けた医薬品、医療機器又は再生医療等製品について様式第九十七による検査記録表を作成しておかなければならない。
