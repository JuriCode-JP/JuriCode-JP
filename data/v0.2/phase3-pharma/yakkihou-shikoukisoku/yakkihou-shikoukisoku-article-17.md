---
law_id: 336M50000100001
law_name_ja: 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則
law_name_en: (English name pending)
article_number: '17'
article_id: yakkihou-shikoukisoku-art-17
version_date: '1961-02-01'
source_url: https://laws.e-gov.go.jp/law/336M50000100001
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: yakkihou-shikoukisoku-art-17-p1-hashira
    type: hashira
    text: 令第二条の十三ただし書の厚生労働省令で定める場合は、次のとおりとする。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: yakkihou-shikoukisoku-art-17-p2
    type: simple
    text: 令第二条の十三の届出は、様式第七による届書を提出することによつて行うものとする。
    modality: gimu
cases: []
amendments: []
tags:
- phase3-pharma
- auto-generated
parent_section:
  shou: 12
  shou_name_ja: 第一章の二　薬局
---

# 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則 第17条(（取扱処方箋数の届出）)

## 原文 (日本語)

### 第十七条第一項

<!-- segment: hashira id: yakkihou-shikoukisoku-art-17-p1-hashira -->
令第二条の十三ただし書の厚生労働省令で定める場合は、次のとおりとする。

### 第十七条第二項

<!-- segment: simple id: yakkihou-shikoukisoku-art-17-p2 -->
令第二条の十三の届出は、様式第七による届書を提出することによつて行うものとする。
