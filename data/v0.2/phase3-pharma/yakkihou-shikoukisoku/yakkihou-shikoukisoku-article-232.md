---
law_id: 336M50000100001
law_name_ja: 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則
law_name_en: (English name pending)
article_number: '232'
article_id: yakkihou-shikoukisoku-art-232
version_date: '1961-02-01'
source_url: https://laws.e-gov.go.jp/law/336M50000100001
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: yakkihou-shikoukisoku-art-232-p1
    type: simple
    text: 第二百十一条（第二百二十条の三及び第二百二十一条の三において準用する場合を含む。）の規定にかかわらず、生物由来製品については、製造番号又は製造記号の記載を省略することができない。
    modality: kanou_negative
cases: []
amendments: []
tags:
- phase1-police
- auto-generated
parent_section:
  shou: 10
  shou_name_ja: 第十章　生物由来製品の特例
---

# 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則 第232条(（生物由来製品の表示の特例）)

## 原文 (日本語)

### 第二百三十二条

<!-- segment: simple id: yakkihou-shikoukisoku-art-232-p1 -->
第二百十一条（第二百二十条の三及び第二百二十一条の三において準用する場合を含む。）の規定にかかわらず、生物由来製品については、製造番号又は製造記号の記載を省略することができない。
