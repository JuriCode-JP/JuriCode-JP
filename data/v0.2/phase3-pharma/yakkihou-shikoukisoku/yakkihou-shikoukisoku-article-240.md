---
law_id: 336M50000100001
law_name_ja: 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則
law_name_en: (English name pending)
article_number: '240'
article_id: yakkihou-shikoukisoku-art-240
version_date: '1961-02-01'
source_url: https://laws.e-gov.go.jp/law/336M50000100001
source_format: e-gov-xml
last_verified: '2026-05-21'
license: MIT
translation_status: none
machine_translated: false
paragraphs:
- number: 1
  has_proviso: false
  has_items: true
  is_added_by_amendment: false
  segments:
  - id: yakkihou-shikoukisoku-art-240-p1-hashira
    type: hashira
    text: 生物由来製品承認取得者等は、法第六十八条の二十二第一項の規定による生物由来製品に関する記録を、次の各号に掲げる期間、保存しなければならない。
    modality: gimu
- number: 2
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: yakkihou-shikoukisoku-art-240-p2
    type: simple
    text: 薬局の管理者又は病院、診療所若しくは動物診療施設の管理者は、法第六十八条の二十二第三項の規定による特定生物由来製品に関する記録を、その使用した日から起算して少なくとも二十年間、これを保存しなければならない。
    modality: gimu
- number: 3
  has_proviso: false
  has_items: false
  is_added_by_amendment: false
  segments:
  - id: yakkihou-shikoukisoku-art-240-p3
    type: simple
    text: 前二項の規定にかかわらず、生物由来製品の承認取得者等又は薬局の管理者若しくは病院、診療所若しくは動物診療施設の管理者は、厚生労働大臣が指定する生物由来製品にあつては、法第六十八条の二十二第一項又は第三項の規定による記録を、厚生労働大臣が指定する期間、保存しなければならない。
    modality: gimu
    references:
    - 前二項
cases: []
amendments: []
tags:
- phase3-pharma
- auto-generated
parent_section:
  shou: 10
  shou_name_ja: 第十章　生物由来製品の特例
---

# 医薬品、医療機器等の品質、有効性及び安全性の確保等に関する法律施行規則 第240条(（記録の保存）)

## 原文 (日本語)

### 第二百四十条第一項

<!-- segment: hashira id: yakkihou-shikoukisoku-art-240-p1-hashira -->
生物由来製品承認取得者等は、法第六十八条の二十二第一項の規定による生物由来製品に関する記録を、次の各号に掲げる期間、保存しなければならない。

### 第二百四十条第二項

<!-- segment: simple id: yakkihou-shikoukisoku-art-240-p2 -->
薬局の管理者又は病院、診療所若しくは動物診療施設の管理者は、法第六十八条の二十二第三項の規定による特定生物由来製品に関する記録を、その使用した日から起算して少なくとも二十年間、これを保存しなければならない。

### 第二百四十条第三項

<!-- segment: simple id: yakkihou-shikoukisoku-art-240-p3 -->
前二項の規定にかかわらず、生物由来製品の承認取得者等又は薬局の管理者若しくは病院、診療所若しくは動物診療施設の管理者は、厚生労働大臣が指定する生物由来製品にあつては、法第六十八条の二十二第一項又は第三項の規定による記録を、厚生労働大臣が指定する期間、保存しなければならない。
